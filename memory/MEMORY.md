...
| 66 | V1+SR+Idio+F96+F168 | 1.673 | 1.08 | 35/5/20/15/25 |
| 67 | V1+SR+Idio+F144+F168 | 1.800 | 1.114 | 25/5/25/25/20 |
| 68 | V1+Idio+F144+F168 | 1.844 | 0.996 | 15/30/40/15 (AVG-max) |
| 68 | V1+SR+Idio+F144+F168 | 1.826 | 1.112 | 20/10/20/30/20 (balanced) |
...
| 73 | V1+I437_bw168+I600_bw168+F144+F168 | **1.909** | **1.170** | 15/15/20/45/5 (BEST BALANCED) |
...
- **Idio lookback**: 437h is CONFIRMED optimal. Curve: 168→0.419, 288→0.431, **437→1.200**, 600→1.060, 730→0.787
- **Beta window (idio)**: 72h default. Curve: 24→0.884, 48→1.156, 72→1.200, 168→1.222, 336→1.267. Longer bw improves AVG but may lower MIN
...
- **Beta window=168 for dual**: bw168 for both gives best MIN (1.170 vs 1.097 for bw72). bw336 for 437 gives higher AVG but worse MIN.
...
- PositioningAlphaV1 (proxy mode only): not competitive standalone
...
- taker↔idio = 0.091 (low but taker fails standalone)

## Best Ensemble Architecture
Current best balanced: `V1(15%) + Idio_437_bw168(15%) + Idio_600_bw168(20%) + F144(45%) + F168(5%)`
- AVG=1.909, MIN=1.170, all 5 years positive
- YbY: [3.464, 1.175, 1.170, 2.471, 1.267]
- F144 heavy (45%) is key — funding contrarian 144h is the strongest single signal
...
- Phase 73: Fine frontier mapping — bw168×168 gives AVG=1.909, MIN=1.170 (best balanced)
- Phase 74: (in progress) 2.5% grid + gap lookback search (475, 510, 550h)
