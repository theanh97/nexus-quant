...
def reflect_main(*, config_path: Path, artifacts_dir: Path, tail_events: int) -> int:
    reflect_and_update(config_path=config_path, artifacts_dir=artifacts_dir, tail_events=int(tail_events))
    return 0


def reflect_cli(argv: list[str]) -> int:
    p = argparse.ArgumentParser(prog="nexus_quant reflect", add_help=True)
