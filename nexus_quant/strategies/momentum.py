...
class MomentumCrossSectionV1Strategy(Strategy):
    def __init__(self, params: Dict[str, Any]) -> None:
        super().__init__(name="momentum_xs_v1", params=params)

    def should_rebalance(self, dataset: MarketDataset, idx: int) -> bool:
        interval = int(self.params.get("rebalance_interval_bars") or 24)
        lookback = int(self.params.get("lookback_bars") or 168)
        if idx <= lookback + 2:
            return False
    ...
    def target_weights(self, dataset: MarketDataset, idx: int, current: Weights) -> Weights:
        ...
        for s in dataset.symbols:
            ...
            scores[s] = (c1 / c0) - 1.0 if c0 != 0 else 0.0

        ranked = sorted(dataset.symbols, key=lambda s: scores[s], reverse=True)
        long_syms = ranked[:k]
        short_syms = ranked[-k:]
