"""
Phase 201 — Regime Weight Final Re-Opt (Correct Overlay Constants)
===================================================================
My P195-P200 used stale overlay constants:
  - VOL_SCALE=0.40, f168_boost=0.10 (parallel P198 changed to 0.30/0.00)
  - FTS short=12/long=96 (parallel P200 changed to short=16/long=72)

Re-optimizing regime weights with ALL correct v2.29.0 constants.
Current regime weights are from my P200 (may be suboptimal under real params).

Sweep: v1 × f168 per regime (2D grid)
Baseline: v2.29.0, regime weights from P200
"""
...
def _on_timeout(signum, frame):
    ...
    (out / "phase201_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
# ── CORRECT v2.29.0 OVERLAY CONSTANTS ──────────────────────────────────────
VOL_WINDOW    = 168; BRD_LB = 192; PCT_WINDOW = 336
TS_SHORT = 16; TS_LONG = 72      # ← parallel P200 updated from 12/96
FUND_DISP_PCT = 240
P_LOW = 0.30; P_HIGH = 0.60

VOL_THRESHOLD = 0.50
VOL_SCALE     = 0.30             # ← parallel P198 updated from 0.40
VOL_F168_BOOST = 0.00            # ← parallel P198 updated from 0.10

TS_RT = 0.65; TS_RS = 0.45      # from parallel P197
TS_BT = 0.25; TS_BS = 1.70
DISP_THR = 0.60; DISP_SCALE = 1.0

# ── SIGNAL PARAMS (v2.29.0) ─────────────────────────────────────────────────
V1_PARAMS = {
...
# Current regime weights (v2.29.0) — may be suboptimal under correct params
BASE_WEIGHTS = {
    "LOW":  {"v1": 0.50,   "i460": 0.0678, "i415": 0.1122, "f168": 0.32},
    "MID":  {"v1": 0.30,   "i460": 0.0186, "i415": 0.0314, "f168": 0.65},
...
# Sweep grids
SWEEPS = {
    "LOW":  {"v1":   [0.42, 0.46, 0.50, 0.53, 0.55],
             "f168": [0.22, 0.26, 0.30, 0.32, 0.35, 0.38]},
    "MID":  {"v1":   [0.20, 0.25, 0.30, 0.35],
             "f168": [0.55, 0.58, 0.60, 0.63, 0.65, 0.68]},
    "HIGH": {"v1":   [0.05, 0.06, 0.07, 0.08, 0.10],
...
def ensemble_returns(weights_by_regime, base_data):
    ...
    for i in range(min_len):
        ...
        if not np.isnan(bv[i]) and bv[i] > VOL_THRESHOLD:
            # VOL_F168_BOOST=0.00 means no boost adjustment needed
            ret_i *= VOL_SCALE
...
def main():
    t0 = time.time()
    print("=" * 65)
    print("Phase 201 — Regime Weight Final Re-Opt (Correct Constants)")
    print("Vol: scale=0.30 boost=0.00 | FTS: short=16 long=72")
    print("=" * 65)
    ...
    print(f"\n  Baseline OBJ = {base_obj:.4f}  (v2.29.0 weights w/ correct overlay params)")

    print("\n[2/3] Per-regime 2D sweep ...")
    best_per_regime = {}
    ...
    result = {"phase": 201, "baseline_obj": float(base_obj), "best_obj": float(joint_obj),
    ...
    out = Path("artifacts/phase201"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase201_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        print(f"\n  Config → v2.30.0  OBJ={round(joint_obj,4)}")
    else:
        # Even if no improvement, update OBJ to reflect true baseline
        cfg_path = ROOT / "configs" / "production_p91b_champion.json"
        cfg = json.load(open(cfg_path))
        ...
        with open(cfg_path, "w") as f:
            json.dump(cfg, f, indent=2, ensure_ascii=False)
        print(f"\n  Config OBJ updated to true baseline: {round(base_obj, 4)}")
