"""
Phase 321 — Per-Regime FTS Retune [VECTORIZED]
============================================================================
Baseline: v2.49.34, OBJ=6.2529 (P320 vol/disp retune — no change)

Continue R&D cycle: re-sweep FTS overlay params.

Current FTS (confirmed optimal in P296, P301, P306, P311, P316):
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase321_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase321_fts_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 321 — FTS Retune [VECTORIZED]")
    print("=" * 70)
    ...
    result = {
        "phase": "321", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    out = Path("artifacts/phase321_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase321_fts_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase321_fts/phase321_fts_report.json")
    ...
    if validated:
        ...
        old_vver = cfg.get("version", "v2.49.34").lstrip("v")
        ...
        msg = (f"feat: P321 FTS retune OBJ={reported_obj} "
