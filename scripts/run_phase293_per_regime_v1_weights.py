"""
Phase 293 — Per-Regime V1 Internal Weights Re-Sweep [VECTORIZED, MEM-EFFICIENT]
================================================================================
Baseline: v2.49.15, OBJ=5.4545 (Phase 290 FTS retune, LOYO=5/5)

New cycle after P287-P290 advanced OBJ 5.1177→5.4545. V1 internals may have
shifted after regime weights (P288), vol+disp (P289), FTS (P290) changes.

Current V1 weights:
  LOW:  wc=0.15, wm=0.45, wmr=0.40  (mean-reversion heavy)
  MID:  wc=0.45, wm=0.45, wmr=0.10  (carry+momentum)
  HIGH: wc=0.50, wm=0.40, wmr=0.10  (carry+momentum)
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase293"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase293_report.json").write_text(json.dumps(_partial, indent=2))
...
if __name__ == "__main__":
    t0 = time.time()
    print("=" * 70)
    print("Phase 293 — Per-Regime V1 Weights Re-Sweep [MEM-EFFICIENT]")
    print("Baseline: P290 FTS retune v2.49.15 OBJ=5.4545")
    print("=" * 70)
    ...
    result = {
        "phase": "293", "baseline_obj": float(obj_base), "stored_baseline": float(base_obj),
    ...
    out = Path("artifacts/phase293"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase293_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        msg = (f"feat: P293 per-regime V1 weights OBJ={reported_obj:.4f} "
