"""
Phase 287 — Per-Regime V1 Internal Weight Re-Sweep [VECTORIZED]
================================================================
Baseline: v2.49.9, OBJ=5.0336 (Phase 286 vol+disp resweep, LOYO=4/5)

V1 internal weights (carry/mom/mean_reversion) last optimized in P280
(+0.49 delta — biggest single win). Since then:
  P283: breadth boundary (p_high 0.68→0.62)
  P285: FTS resweep (rs→0.02 all regimes)
  P286: vol+disp resweep

The optimal V1 signal composition may have shifted. Re-sweep per regime.

Current (P280):
  LOW:  wc=0.10, wm=0.50, wmr=0.40  (mean-reversion heavy)
  MID:  wc=0.30, wm=0.60, wmr=0.10  (momentum heavy)
  HIGH: wc=0.60, wm=0.40, wmr=0.00  (carry dominant, zero MR)
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 287 — Per-Regime V1 Internal Weight Re-Sweep")
    print("Baseline: v2.49.9, OBJ=5.0336")
    print("=" * 70)
    ...
    # [7] Save report
    result = {
        "phase": "287", "title": "Per-Regime V1 Weight Re-Sweep",
    ...
    print(f"\n  Report → artifacts/phase287/phase287_report.json")
    ...
    if validated:
        ...
        old_ver = cfg.get("_version", "2.49.9")
        parts = old_ver.split(".")
        parts[-1] = str(int(parts[-1]) + 1)
        new_ver = ".".join(parts)
        cfg["_version"] = new_ver

        old_vver = cfg.get("version", "v2.49.9").lstrip("v")
        ...
        msg = (f"feat: P287 V1 weight resweep OBJ={reported_obj:.4f} "
