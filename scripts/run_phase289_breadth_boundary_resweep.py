"""
Phase 289 — Breadth Boundary Fine-tune (p_low / p_high)
========================================================
Baseline: v2.49.11, OBJ=5.1177 (post P282 regime weight reopt14)

Fine-tune p_low/p_high around current 0.30/0.68 with a denser grid.
After P279-P282 changed FTS, V1 weights, vol/disp, and regime weights,
optimal boundaries may have shifted.

Grid:
  p_low  ∈ [0.20, 0.25, 0.28, 0.30, 0.32, 0.35, 0.40]
  p_high ∈ [0.58, 0.62, 0.65, 0.68, 0.70, 0.73, 0.75]
  Constraint: p_high > p_low + 0.15

FIX: momentum_lookback_bars 336→312 to match config.
"""
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase289"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase289_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
P_LOW_SWEEP  = [0.20, 0.25, 0.28, 0.30, 0.32, 0.35, 0.40]
P_HIGH_SWEEP = [0.58, 0.62, 0.65, 0.68, 0.70, 0.73, 0.75]
...
def load_config_params():
    ...
    return {
        ...
        "fts_rs":  {r: sdget(fts.get("per_regime_rs"), r, 0.02)  for r in RNAMES},
        ...
        "vol_thr":   {r: sdget(vol.get("per_regime_threshold"), r, 0.53)  for r in RNAMES},
        "vol_scale": {r: sdget(vol.get("per_regime_scale"),     r, 0.40)  for r in RNAMES},
        "disp_thr":  {r: sdget(disp.get("per_regime_threshold"), r, 0.40) for r in RNAMES},
        "disp_scale":{r: sdget(disp.get("per_regime_scale"),     r, 0.70) for r in RNAMES},
...
def build_precomp_for_boundary(p, all_base_data, p_low, p_high):
    """Build fast eval data with specific p_low/p_high boundary."""
    fast_data = {}
...
if __name__ == "__main__":
    t0 = time.time()
    print("=" * 65)
    print("Phase 289 — Breadth Boundary Fine-tune (p_low / p_high)")
    ...
    for pl in P_LOW_SWEEP:
        for ph in P_HIGH_SWEEP:
            if ph <= pl + 0.15: continue
            fd = build_precomp_for_boundary(P, all_base_data, pl, ph)
            obj = fast_eval(P["regime_weights"], fd)
            delta_local = obj - obj_base
            results.append({"p_low": pl, "p_high": ph, "obj": round(obj, 4), "delta": round(delta_local, 4)})
            marker = " <-- BEST" if obj > best_obj else ""
            if obj > best_obj:
                best_obj = obj; best_plow = pl; best_phigh = ph
            print(f"  p_low={pl:.2f} p_high={ph:.2f}  OBJ={obj:.4f}  D={delta_local:+.4f}{marker}")

    delta = best_obj - obj_base
    print(f"\n  Best: p_low={best_plow:.2f}  p_high={best_phigh:.2f}  OBJ={best_obj:.4f}  D={delta:+.4f}")

    # Build report
    report = {
        "phase": 283,
        "baseline_version": P["config_version"],
        "baseline_obj_config": base_obj,
        "baseline_obj_measured": round(obj_base, 4),
        "best_obj": round(best_obj, 4),
        "delta": round(delta, 4),
        "best_p_low": best_plow,
        "best_p_high": best_phigh,
        "current_p_low": P["p_low"],
        "current_p_high": P["p_high"],
        "grid_results": results,
    }

    # [4] LOYO validation (only if improvement found)
    if delta < MIN_DELTA:
        report["validated"] = False
        report["reason"] = f"delta {delta:+.4f} < {MIN_DELTA}"
        out = Path("artifacts/phase289"); out.mkdir(parents=True, exist_ok=True)
        (out / "phase289_report.json").write_text(json.dumps(report, indent=2))
        print(f"\n  NO IMPROVEMENT (D={delta:+.4f} < {MIN_DELTA})")
    ...
    for yr in sorted(YEAR_RANGES.keys()):
        ...
        loyo_wins += int(win)
        loyo_detail.append({"year": yr, "base": round(base_loyo, 4), "cand": round(cand_loyo, 4), "win": win})
    ...
    report["loyo_wins"] = loyo_wins
    report["loyo_detail"] = loyo_detail
    ...
    if validated:
        ...
        old_ver = cfg.get("_version", "2.49.5")
        parts = old_ver.split(".")
        ...
        msg = (f"feat: P289 breadth boundary p_low={best_plow:.2f} p_high={best_phigh:.2f} "
        ...
        print(f"  Current boundary confirmed: p_low={P['p_low']:.2f}  p_high={P['p_high']:.2f}")
