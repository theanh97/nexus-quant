"""
Phase 347 — Per-Regime Vol + Disp Overlay Re-Sweep [VECTORIZED]
================================================================
Baseline: v2.49.35, OBJ=6.2892 (after P324 regime no change)
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase347"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase347_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 347 — Vol + Disp Overlay Re-Tune [VECTORIZED]")
    print("=" * 70)
    ...
    result = {
        "phase": "347", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    out = Path("artifacts/phase347"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase347_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        msg = (f"feat: P347 vol+disp retune OBJ={reported_obj:.4f} "
