"""
Phase 289 — Per-Regime Vol + Disp Overlay Re-Tune [VECTORIZED]
================================================================
Baseline: v2.49.13, OBJ=5.3574 (Phase 288 regime weights, LOYO=5/5)

After P287 (V1 internal weights changed) and P288 (regime weights changed),
the signal landscape has shifted. Re-tune both vol and disp overlays with
finer grids around current values.

Current (from P286):
  Vol thresh: LOW=0.53, MID=0.50, HIGH=0.58
  Vol scale:  LOW=0.40, MID=0.20, HIGH=0.05
  Disp thresh: LOW=0.75, MID=0.40, HIGH=0.30
  Disp scale:  LOW=0.50, MID=1.80, HIGH=0.30
...
# Grids — finer around current values (P286 winners)
# Current: vol_thr LOW=0.53 MID=0.50 HIGH=0.58, vol_scale LOW=0.40 MID=0.20 HIGH=0.05
VOL_THR_GRID   = [0.40, 0.45, 0.48, 0.50, 0.52, 0.53, 0.55, 0.57, 0.58, 0.60, 0.63, 0.65, 0.70]
VOL_SCALE_GRID = [0.03, 0.05, 0.08, 0.10, 0.15, 0.18, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50]
# Current: disp_thr LOW=0.75 MID=0.40 HIGH=0.30, disp_scale LOW=0.50 MID=1.80 HIGH=0.30
DISP_THR_GRID  = [0.25, 0.30, 0.35, 0.40, 0.45, 0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85]
DISP_SCALE_GRID = [0.20, 0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 1.0, 1.2, 1.4, 1.6, 1.8, 2.0, 2.2, 2.5]
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 289 — Vol + Disp Overlay Re-Tune [VECTORIZED]")
    print("=" * 70)
    ...
    result = {
        "phase": "289", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    out = Path("artifacts/phase289"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase289_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        old_ver = cfg.get("_version", "2.49.13")
        parts = old_ver.split(".")
        parts[-1] = str(int(parts[-1]) + 1)
        new_ver = ".".join(parts)
        cfg["_version"] = new_ver
        old_vver = cfg.get("version", "v2.49.13").lstrip("v")
        ...
        msg = (f"feat: P289 vol+disp retune OBJ={reported_obj:.4f} "
