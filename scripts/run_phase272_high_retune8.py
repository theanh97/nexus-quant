#!/usr/bin/env python3
"""
Phase 272 — HIGH Regime Re-retune Round 8
Parallel session again set HIGH: v1=0.5, i460=0.5, f168=0.0
Re-validate f168 contribution in HIGH. Sweeps f168, v1, idio_ratio.
...
def main():
    ...
    print(f"    Current HIGH: v1={base_weights['HIGH']['v1']} i460={base_weights['HIGH']['i460']} "
          f"i415={base_weights['HIGH']['i415']} f168={base_weights['HIGH']['f168']}")
    ...
    # --- Sweep f168 ---
    print("\n[3] f168 sweep ...")
    best_f168 = base_weights["HIGH"]["f168"]
    best_obj  = curr_obj
    v1_cur = base_weights["HIGH"]["v1"]
    iratio = 0.35

    for f168_w in [0.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50]:
        w = make_high_w(base_weights, f168_w, v1_cur, iratio)
    ...
    print("\n[4] v1 sweep ...")
    best_v1 = v1_cur
    for v1_w in [0.10, 0.15, 0.18, 0.20, 0.22, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50]:
    ...
    print("\n[5] idio_ratio sweep ...")
    best_ratio = iratio
    for ratio in [0.10, 0.20, 0.30, 0.35, 0.40, 0.45, 0.50, 0.55, 0.60, 0.70, 0.80, 0.90]:
    ...
    best_high = best_w["HIGH"]
    print(f"\n  LOYO: {wins}/5 wins  delta={delta:+.4f}")
    print(f"  Best HIGH: v1={best_high['v1']:.3f} i460={best_high['i460']:.3f} "
          f"i415={best_high['i415']:.3f} f168={best_high['f168']:.3f}")
    ...
    if wins >= 3 and delta > 0.005:
        ...
        rw2["HIGH"]["v1"] = best_high["v1"]
        if "i460bw168" in rw2["HIGH"]:
            rw2["HIGH"]["i460bw168"] = best_high["i460"]
        else:
            rw2["HIGH"]["i460"] = best_high["i460"]
        if "i415bw216" in rw2["HIGH"]:
            rw2["HIGH"]["i415bw216"] = best_high["i415"]
        else:
            rw2["HIGH"]["i415"] = best_high["i415"]
        rw2["HIGH"]["f168"] = best_high["f168"]
        ...
        print(f"  HIGH: v1={best_high['v1']:.3f} i460={best_high['i460']:.3f} "
              f"i415={best_high['i415']:.3f} f168={best_high['f168']:.3f}")
        print(f"  New OBJ={best_obj:.4f}")
        msg = (f"quant: {new_ver} — HIGH retune r8 "
               f"(v1={best_high['v1']:.2f} f168={best_high['f168']:.2f}) "
               f"OBJ={best_obj:.4f}")
        ...
        print(f"  HIGH stays: v1={base_weights['HIGH']['v1']} i460={base_weights['HIGH']['i460']} "
              f"i415={base_weights['HIGH']['i415']} f168={base_weights['HIGH']['f168']}")

    elapsed = time.time() - t0
    print(f"\n[DONE] Phase 272 complete.  ({elapsed:.1f}s)")
