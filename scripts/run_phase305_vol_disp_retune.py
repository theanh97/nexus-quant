"""
Phase 305 — Per-Regime Vol + Disp Overlay Re-Tune [VECTORIZED]
================================================================
Baseline: v2.49.26, OBJ=5.9117 (Phase 304 V1 weight resweep)

After P304 (V1 weights changed — LOW wc=0.15 wm=0.45 wmr=0.40),
re-tune both vol and disp overlays with finer grids around current values.

Current (from P300):
  Vol thresh: LOW=0.54, MID=0.50, HIGH=0.57
  Vol scale:  LOW=0.40, MID=0.20, HIGH=0.005
  Disp thresh: LOW=0.38, MID=0.40, HIGH=0.30
  Disp scale:  LOW=0.70, MID=2.00, HIGH=0.35
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase305"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase305_report.json").write_text(json.dumps(_partial, indent=2))
...
# Grids — finer around current values (P300 winners)
# Current: vol_thr LOW=0.54 MID=0.50 HIGH=0.57, vol_scale LOW=0.40 MID=0.20 HIGH=0.005
VOL_THR_GRID   = [0.44, 0.47, 0.49, 0.50, 0.52, 0.53, 0.54, 0.55, 0.56, 0.57, 0.58, 0.59, 0.60, 0.62]
VOL_SCALE_GRID = [0.003, 0.005, 0.01, 0.02, 0.05, 0.08, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.50]
# Current: disp_thr LOW=0.38 MID=0.40 HIGH=0.30, disp_scale LOW=0.70 MID=2.00 HIGH=0.35
DISP_THR_GRID  = [0.22, 0.25, 0.28, 0.30, 0.33, 0.35, 0.38, 0.40, 0.42, 0.45, 0.50, 0.55, 0.60, 0.70, 0.80]
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 305 — Vol + Disp Overlay Re-Tune [VECTORIZED]")
    print("=" * 70)
    ...
    result = {
        "phase": "305", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    out = Path("artifacts/phase305"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase305_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        old_ver = cfg.get("_version", "2.49.26")
        parts = old_ver.split(".")
        parts[-1] = str(int(parts[-1]) + 1)
        new_ver = ".".join(parts)
        cfg["_version"] = new_ver
        old_vver = cfg.get("version", "v2.49.26").lstrip("v")
        ...
        msg = (f"feat: P305 vol+disp retune OBJ={reported_obj:.4f} "
