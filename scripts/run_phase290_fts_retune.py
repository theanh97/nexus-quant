"""
Phase 290 — Per-Regime FTS Retune after P287-P289 Signal Shift [VECTORIZED]
============================================================================
Baseline: v2.49.14, OBJ=5.3868 (Phase 289 vol+disp retune, LOYO=3/5)

After P287 (V1 internal weights), P288 (regime weights), P289 (vol+disp),
the signal landscape has shifted. Re-tune FTS overlay params.

Current FTS (from P285):
  rs: LOW=0.02 MID=0.02 HIGH=0.02  (reduce leg nearly disabled)
  rt: LOW=0.80 MID=0.60 HIGH=0.55
  bs: LOW=3.50 MID=1.50 HIGH=2.50
  bt: LOW=0.33 MID=0.22 HIGH=0.22
...
# Grids — finer around current P285 values + test re-enabling reduce leg
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 290 — FTS Retune after P287-P289 Signal Shift [VECTORIZED]")
    print("Baseline: v2.49.14 OBJ=5.3868")
    print("=" * 70)
    ...
    result = {
        "phase": "290", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    if validated:
        ...
        old_vver = cfg.get("version", "v2.49.14").lstrip("v")
        ...
        msg = (f"feat: P290 FTS retune OBJ={reported_obj} "
