#!/usr/bin/env python3
"""
Phase 273 — LOW Regime Retune Round 3
Re-optimize LOW weights from current config (parallel session changed i460, i415, f168).
Sweeps f168, v1, idio_ratio for LOW regime.
Keeps MID and HIGH fixed.
...
def make_low_w(base_weights, f168_w, v1_w, idio_ratio):
...
def main():
    ...
    print(f"    Current LOW: v1={base_weights['LOW']['v1']} i460={base_weights['LOW']['i460']} "
          f"i415={base_weights['LOW']['i415']} f168={base_weights['LOW']['f168']}")

    print("[2] Loading yearly data (2021-2025) ...")
    yearly_data = {}
    for yr in YEARS:
    ...
    # --- Sweep f168 ---
    print("\n[3] f168 sweep ...")
    best_f168 = base_weights["LOW"]["f168"]
    best_obj  = curr_obj
    v1_cur = base_weights["LOW"]["v1"]
    iratio = 0.5

    for f168_w in [0.00, 0.10, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50, 0.55, 0.60]:
        w = make_low_w(base_weights, f168_w, v1_cur, iratio)
    ...
    print("\n[4] v1 sweep ...")
    best_v1 = v1_cur
    for v1_w in [0.00, 0.10, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50, 0.55, 0.60]:
        w = make_low_w(base_weights, best_f168, v1_w, iratio)
        obj = eval_weights(yearly_data, params, w)
        marker = " <-" if obj > best_obj else ""
    ...
    print("\n[5] idio_ratio sweep ...")
    best_ratio = iratio
    for ratio in [0.00, 0.10, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 1.00]:
        w = make_low_w(base_weights, best_f168, best_v1, ratio)
        obj = eval_weights(yearly_data, params, w)
        marker = " <-" if obj > best_obj else ""
    ...
    best_w = make_low_w(base_weights, best_f168, best_v1, best_ratio)
    ...
    print(f"  Best LOW: v1={best_low['v1']:.3f} i460={best_low['i460']:.3f} "
          f"i415={best_low['i415']:.3f} f168={best_low['f168']:.3f}")
    ...
    if wins >= 3 and delta > 0.005:
        ...
        rw2["LOW"]["v1"] = best_low["v1"]
        if "i460bw168" in rw2["LOW"]:
            rw2["LOW"]["i460bw168"] = best_low["i460"]
        else:
            rw2["LOW"]["i460"] = best_low["i460"]
        if "i415bw216" in rw2["LOW"]:
            rw2["LOW"]["i415bw216"] = best_low["i415"]
        else:
            rw2["LOW"]["i415"] = best_low["i415"]
        rw2["LOW"]["f168"] = best_low["f168"]
        ...
        print(f"  LOW: v1={best_low['v1']:.3f} i460={best_low['i460']:.3f} "
              f"i415={best_low['i415']:.3f} f168={best_low['f168']:.3f}")
        print(f"  New OBJ={best_obj:.4f}")
        msg = (f"quant: {new_ver} — LOW retune r3 "
               f"(v1={best_low['v1']:.2f} f168={best_low['f168']:.2f}) "
               f"OBJ={best_obj:.4f}")
        ...
        print(f"  LOW stays: v1={base_weights['LOW']['v1']} i460={base_weights['LOW']['i460']} "
              f"i415={base_weights['LOW']['i415']} f168={base_weights['LOW']['f168']}")

    elapsed = time.time() - t0
    print(f"\n[DONE] Phase 273 complete.  ({elapsed:.1f}s)")
