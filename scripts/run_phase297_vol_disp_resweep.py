"""
Phase 297 — Per-Regime Vol + Disp Overlay Re-Sweep [VECTORIZED]
================================================================
Baseline: v2.49.21, OBJ=5.6221 (latest after P293-P296)

After P293 (V1 weights), P294 (regime weights), P295 (boundary no change),
P296 (FTS not validated), re-sweep vol + disp overlays.
...
def _on_timeout(signum, frame):
    ...
    (out / "phase297_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 297 — Vol + Disp Overlay Re-Tune [VECTORIZED]")
    print("=" * 70)
    ...
    out = Path("artifacts/phase297"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase297_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        msg = (f"feat: P297 vol+disp retune OBJ={reported_obj:.4f} "
