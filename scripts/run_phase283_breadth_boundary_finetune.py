"""
Phase 283 — Breadth Boundary Fine-tune (p_low / p_high)
========================================================
Baseline: v2.49.5, OBJ=4.8089 (post P282 regime weight reopt14)
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase283"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase283_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
if __name__ == "__main__":
    t0 = time.time()
    print("=" * 65)
    print("Phase 283 — Breadth Boundary Fine-tune (p_low / p_high)")
    ...
    if delta < MIN_DELTA:
        ...
        out = Path("artifacts/phase283"); out.mkdir(parents=True, exist_ok=True)
        (out / "phase283_report.json").write_text(json.dumps(report, indent=2))
    ...
    if validated:
        ...
        msg = (f"feat: P283 breadth boundary p_low={best_plow:.2f} p_high={best_phigh:.2f} "
