#!/usr/bin/env python3
"""
Phase 263 — MID Regime Retune Round 3
After P261 p_high 0.60→0.68, MID now covers brd_pct 0.30-0.68 (wider).
Current MID: v1=0.10, i460bw168=0.00, i415bw216=0.25, f168=0.65
Sweep: A) f168, B) v1, C) i415 fraction of remaining non-f168/v1
"""
...
def make_mid_w(base_weights, f168_w, v1_w, i415_w):
...
def main():
    t0 = time.time()
    print("=" * 60)
    print("Phase 263 — MID Regime Retune Round 3")
    print("After P261: p_high changed 0.60 → 0.68")
    print("=" * 60)
    ...
    print(f"  Current MID: v1={curr_mid['v1']:.2f} i460={curr_mid['i460']:.2f} "
          f"i415={curr_mid['i415']:.2f} f168={curr_mid['f168']:.2f}")

    # [2] f168 sweep
    print("\n[2] f168 sweep ...")
    f168_candidates = [0.00, 0.10, 0.20, 0.30, 0.40, 0.50, 0.55, 0.60,
                       0.65, 0.70, 0.75, 0.80, 0.85, 0.90, 0.95, 1.00]
    best_f168 = curr_mid["f168"]; best_f168_obj = baseline_obj
    for fw in f168_candidates:
        w_test = make_mid_w(base_weights, fw, curr_mid["v1"], curr_mid["i415"])
    ...
    # [3] v1 sweep
    print("\n[3] v1 sweep ...")
    v1_candidates = [0.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40]
    best_v1 = curr_mid["v1"]; best_v1_obj = best_f168_obj
    for vw in v1_candidates:
        if vw + best_f168 > 1.0: continue
        w_test = make_mid_w(base_weights, best_f168, vw, curr_mid["i415"])
        obj = eval_weights(year_data, params, w_test)
    ...
    # [4] i415 sweep (with best f168 + v1)
    print("\n[4] i415 sweep ...")
    ...
    for iw in i415_candidates:
        ...
        w_test = make_mid_w(base_weights, best_f168, best_v1, iw)
        obj = eval_weights(year_data, params, w_test)
    ...
    # [5] LOYO validation
    print("\n[5] LOYO validation ...")
    best_weights = make_mid_w(base_weights, best_f168, best_v1, best_i415)
    wins = 0
    for yr in YEARS:
    ...
    print(f"  Best MID: v1={bw['v1']:.2f} i460={bw['i460']:.3f} i415={bw['i415']:.2f} f168={bw['f168']:.2f}")
    ...
    print(f"\n[DONE] Phase 263 complete.  ({time.time()-t0:.1f}s)")
...
def _update_config(cfg, bw, new_obj):
    ...
    subprocess.run(["git", "commit", "-m",
        f"quant: {new_ver} — MID regime retune r3 (v1={bw['v1']:.2f} f168={bw['f168']:.2f}) OBJ={new_obj:.4f}"],
