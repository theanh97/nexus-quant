#!/usr/bin/env python3
"""
Phase 257 — HIGH Regime Re-retune (post P256 V1 weight update)
Current HIGH: v1=0.12 f168=0.20 i460=0.204 i415=0.476
Sweep: A) f168, B) v1, C) idio_ratio
"""
...
def make_high_w(base, f168_w, v1_w, idio_ratio):
    ...
    return {"LOW": base["LOW"], "MID": base["MID"],
            "HIGH": {"v1": v1_w, "i460": i460_w, "i415": i415_w, "f168": f168_w}}

def main():
    t0 = time.time()
    print("=" * 60)
    print("Phase 257 — HIGH Regime Re-retune (post V1 weight update)")
    print("=" * 60)
    cfg, weights, params, v1p, i460p, i415p, f168p = get_config()
    cur_high = weights["HIGH"]
    cur_v1 = cur_high["v1"]; cur_f168 = cur_high["f168"]
    cur_i460 = cur_high["i460"]; cur_i415 = cur_high["i415"]
    cur_idio = cur_i460 + cur_i415
    cur_ratio = (cur_i460 / cur_idio) if cur_idio > 1e-9 else 0.3
    ...
    best_obj = base_obj; best_f168 = cur_f168; best_v1 = cur_v1; best_ratio = cur_ratio

    # Part A: f168
    print("\n[2] Part A: HIGH f168 sweep ...")
    for f168_w in [0.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35]:
        w_t = make_high_w(weights, f168_w, cur_v1, cur_ratio)
        o = eval_weights(year_data, params, w_t)
        marker = " <-" if o > best_obj + 1e-6 else ""
        print(f"  f168={f168_w:.2f}  OBJ={o:.4f}{marker}")
        if o > best_obj + 1e-6: best_obj = o; best_f168 = f168_w
    print(f"  Best f168: {best_f168}  OBJ={best_obj:.4f}")

    # Part B: v1
    print(f"\n[3] Part B: HIGH v1 sweep (f168={best_f168}) ...")
    for v1_w in [0.00, 0.04, 0.08, 0.10, 0.12, 0.15, 0.18, 0.20, 0.24, 0.28]:
        w_t = make_high_w(weights, best_f168, v1_w, cur_ratio)
        o = eval_weights(year_data, params, w_t)
        marker = " <-" if o > best_obj + 1e-6 else ""
        print(f"  v1={v1_w:.2f}  f168={best_f168:.2f}  OBJ={o:.4f}{marker}")
        if o > best_obj + 1e-6: best_obj = o; best_v1 = v1_w
    print(f"  Best v1: {best_v1}  OBJ={best_obj:.4f}")

    # Part C: idio_ratio
    print(f"\n[4] Part C: HIGH idio_ratio (v1={best_v1} f168={best_f168}) ...")
    for ratio in [0.0, 0.1, 0.2, 0.25, 0.3, 0.35, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0]:
        w_t = make_high_w(weights, best_f168, best_v1, ratio)
        o = eval_weights(year_data, params, w_t)
        hi = w_t["HIGH"]
        marker = " <-" if o > best_obj + 1e-6 else ""
        print(f"  ratio={ratio:.2f}  i460={hi['i460']:.3f}  i415={hi['i415']:.3f}  OBJ={o:.4f}{marker}")
        if o > best_obj + 1e-6: best_obj = o; best_ratio = ratio
    delta = best_obj - base_obj
    best_w = make_high_w(weights, best_f168, best_v1, best_ratio)
    best_hi = best_w["HIGH"]
    print(f"  Best ratio: {best_ratio}  D={delta:+.4f}")

    print(f"\n[5] LOYO validation ...")
    loyo_wins = 0
    for yr in YEARS:
        sb = yearly_sharpe(year_data[yr], params, weights)
        sc = yearly_sharpe(year_data[yr], params, best_w)
    ...
    if loyo_wins >= 3 and delta > 0.005:
        ...
        old_ver = cfg.get("version", "v2.42.8")
        ...
        try:
            ...
            msg = (f"feat: {new_ver} Phase257 HIGH retune2 v1={best_v1} f168={best_f168} ratio={best_ratio:.2f} "
