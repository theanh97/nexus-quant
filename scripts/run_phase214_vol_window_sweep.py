"""
Phase 214 — VOL_WINDOW Sweep (BTC realized vol calculation window)
===================================================================
VOL_WINDOW=168h (7-day) sets the lookback for computing BTC realized vol.
This determines when the vol damping overlay activates.
Never swept since early phases — might find a better signal window.
All signals pre-computed — rebuild vol signal on-the-fly (fast).

Baseline: v2.32.0, OBJ=3.0860
Current: VOL_WINDOW=168h, VOL_THRESHOLD=0.50, VOL_SCALE=0.30
Sweep VOL_WINDOW ∈ [48, 72, 96, 120, 144, 168, 192, 240, 288]
"""
...
def _on_timeout(signum, frame):
    ...
    (out / "phase214_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
BASELINE_OBJ   = 3.0860
VOL_WIN_SWEEP  = [48, 72, 96, 120, 144, 168, 192, 240, 288]
...
def load_year_data(year):
    ...
    # BTC returns (for vol calc with any window)
    btc_rets = np.zeros(n)
    for i in range(1, n):
    ...
    # Return btc_rets for vol recompute with different windows
    return btc_rets, fund_std_pct, ts_spread_pct, brd_pct, sig_rets, n

def compute_btc_vol(btc_rets, n, vol_window):
...
def ensemble_rets(btc_vol, fund_std_pct, ts_spread_pct, brd_pct, sig_rets):
    RNAMES = ["LOW", "MID", "HIGH"]
...
def evaluate(vol_window, all_data):
    yearly = {}
    for yr, (btc_rets, fsp, tsp, brd_pct, sig_rets, n) in all_data.items():
        btc_vol = compute_btc_vol(btc_rets, n, vol_window)
        ens = ensemble_rets(btc_vol, fsp, tsp, brd_pct, sig_rets)
...
def main():
    t0 = time.time()
    print("=" * 65)
    print("Phase 214 — VOL_WINDOW Sweep")
    print(f"Baseline: v2.32.0  OBJ={BASELINE_OBJ}")
    print(f"Current: VOL_WINDOW={VOL_WINDOW_BASE}h  (thr={VOL_THRESHOLD}  scale={VOL_SCALE})")
    print(f"Sweep: {VOL_WIN_SWEEP}")
    print("=" * 65)
    ...
    base_obj, base_yearly = evaluate(VOL_WINDOW_BASE, all_data)
    print(f"\n  Baseline OBJ = {base_obj:.4f}  (expected ≈ {BASELINE_OBJ})")
    _partial.update({"baseline_obj": float(base_obj)})

    print("\n[2/3] Sweep...")
    best_obj = base_obj; best_vw = VOL_WINDOW_BASE
    results = []
    for vw in VOL_WIN_SWEEP:
        o, _ = evaluate(vw, all_data)
        delta = o - base_obj
        marker = " ← BEST" if o > best_obj else ""
        print(f"  vol_window={vw:4d}h  OBJ={o:.4f}  Δ={delta:+.4f}{marker}")
        results.append((vw, o))
        if o > best_obj:
            best_obj = o; best_vw = vw

    print(f"\n  → Best: vol_window={best_vw}h  OBJ={best_obj:.4f}")

    print("\n[3/3] LOYO validation...")
    joint_obj, joint_yearly = evaluate(best_vw, all_data)
    ...
    print(f"\n{'✅ VALIDATED' if validated else '❌ NO IMPROVEMENT'} — vol_window={best_vw}h")

    result = {
        "phase": 214, "baseline_obj": float(base_obj), "best_obj": float(joint_obj),
        "delta": float(delta), "loyo_wins": loyo_wins, "loyo_total": len(joint_yearly),
        "validated": validated, "best_vol_window": best_vw,
        "sweep_results": results,
    }
    _partial.update(result)
    out = Path("artifacts/phase214"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase214_report.json").write_text(json.dumps(result, indent=2))

    if validated:
        cfg_path = ROOT / "configs" / "production_p91b_champion.json"
        cfg = json.load(open(cfg_path))
        vo = cfg.setdefault("vol_overlay_params", {})
        vo["vol_window_bars"] = best_vw
        cfg["_version"] = "2.33.0"
