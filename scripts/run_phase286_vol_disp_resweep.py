"""
Phase 286 — Per-Regime Vol + Disp Overlay Re-Sweep [VECTORIZED]
================================================================
Baseline: v2.49.4, OBJ=4.7319 (Phase 280 V1 weights, LOYO=5/5)

Vol and Disp overlays were tuned at much earlier OBJ levels. V1 internal
weights just changed significantly (P280). Re-sweep both overlays.

Current:
  Vol thresh: LOW=0.50, MID=0.50, HIGH=0.55
  Vol scale:  LOW=0.40, MID=0.15, HIGH=0.05
  Disp thresh: LOW=0.50, MID=0.70, HIGH=0.40
  Disp scale:  LOW=0.70, MID=1.80, HIGH=0.50

Approach: Coordinate descent sweep per regime (vol_thr→vol_scale→disp_thr→disp_scale)
"""
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase286"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase286_report.json").write_text(json.dumps(_partial, indent=2))
...
# Grids
VOL_THR_GRID   = [0.40, 0.45, 0.50, 0.53, 0.55, 0.58, 0.60, 0.65, 0.70]
VOL_SCALE_GRID = [0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.50, 0.60]
DISP_THR_GRID  = [0.30, 0.40, 0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80]
DISP_SCALE_GRID = [0.30, 0.50, 0.70, 0.80, 1.0, 1.2, 1.5, 1.8, 2.0, 2.5]
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 286 — Vol + Disp Overlay Re-Sweep [VECTORIZED]")
    print("=" * 70)
    ...
    result = {
        "phase": "281", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    out = Path("artifacts/phase286"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase286_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        old_ver = cfg.get("_version", "2.49.3")
        parts = old_ver.split(".")
        parts[-1] = str(int(parts[-1]) + 1)
        new_ver = ".".join(parts)
        cfg["_version"] = new_ver
        old_vver = cfg.get("version", "v2.49.4").lstrip("v")
        ...
        msg = (f"feat: P286 vol+disp resweep OBJ={reported_obj:.4f} "
