"""
Phase 310 — Per-Regime Vol + Disp Overlay Re-Tune [VECTORIZED]
================================================================
Baseline: v2.49.29, OBJ=5.9744 (parallel P307 vol+disp retune)

After parallel session P307 changed vol/disp overlays,
re-tune with finer grids around new current values.

Current (from parallel P307):
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase310"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase310_report.json").write_text(json.dumps(_partial, indent=2))
...
# Grids — finer around current values (parallel P307 winners)
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 310 — Vol + Disp Overlay Re-Tune [VECTORIZED]")
    print("=" * 70)
    ...
    out = Path("artifacts/phase310"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase310_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        msg = (f"feat: P310 vol+disp retune OBJ={reported_obj:.4f} "
