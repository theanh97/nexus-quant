"""
Phase 227 — Regime Weight Re-Opt (5th pass, post FTS BS/RS change)
====================================================================
FTS overlay changed significantly in P225 (BS: 1.70→2.50, RS: 0.45→0.35).
Higher BS means low-TS periods are 2.5x amplified vs 1.7x before.
This changes how much the regime weight drives total return in each
FTS state. The optimal regime weights may have shifted.

Same approach as P220 (4th pass):
  - Pre-compute all 4 signals
  - Per-regime iterative sweep of v1 × f168 (idio ratio fixed)
  - 2 passes

Grid: v1  ∈ [0.00, 0.04, ..., 0.60]  (16 values)
      f168 ∈ [0.00, 0.04, ..., 0.90] (24 values)
      Per regime → pure NumPy (very fast)
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase227"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase227_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
IDIO_RATIO_460 = {
    "LOW":  0.0716 / (0.0716 + 0.1184),
    "MID":  0.0074 / (0.0074 + 0.0126),
    "HIGH": 0.2821 / (0.2821 + 0.5079),
}
...
def find_best_weights_for_regime(regime, all_data, other_rw):
    ...
    return best_v1, best_f168, best_obj_val

def main():
    t0 = time.time()
    print("=" * 65)
    print("Phase 227 — Regime Weight Re-Opt (5th pass, FTS BS=2.50)")
    print(f"Baseline: v2.36.0  OBJ={BASELINE_OBJ}")
    print(f"Grid: V1={len(V1_SWEEP)} × F168={len(F168_SWEEP)} per regime × 2 passes")
    print("=" * 65)
    ...
    base_obj_val, base_yearly = evaluate_rw(REGIME_WEIGHTS_BASE, all_data)
    print(f"\n  Baseline OBJ = {base_obj_val:.4f}  (expected ≈ {BASELINE_OBJ})")
    _partial.update({"baseline_obj": float(base_obj_val)})

    print("\n[2/3] Per-regime weight sweep (2 passes)...")
    current_rw = {r: dict(w) for r, w in REGIME_WEIGHTS_BASE.items()}

    for pass_num in range(2):
        print(f"\n  === Pass {pass_num+1} ===")
        for regime in ["LOW", "MID", "HIGH"]:
            other_rw = {r: current_rw[r] for r in current_rw if r != regime}
            bv1, bf168, bo = find_best_weights_for_regime(regime, all_data, other_rw)
            idio = 1.0 - bv1 - bf168
            ratio = IDIO_RATIO_460[regime]
            current_rw[regime] = {
    ...
    result = {
        "phase": 227, "baseline_obj": float(base_obj_val), "best_obj": float(joint_obj_val),
    ...
    out = Path("artifacts/phase227"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase227_report.json").write_text(json.dumps(result, indent=2))
