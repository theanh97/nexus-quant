#!/usr/bin/env python3
"""
Phase 261 — Breadth Threshold Re-sweep Round 2
Sweep p_low + p_high with updated v2.42.9 config.
Not retested since P251 (before V1 weights/regime changes in P255-257).
"""
...
def main():
    t0 = time.time()
    print("=" * 60)
    print("Phase 261 — Breadth Threshold Re-sweep Round 2")
    print("Config: v2.42.9 (or updated by P260)")
    print("=" * 60)
    ...
    # [2] p_low sweep (hold p_high fixed)
    print("\n[2] p_low sweep ...")
    low_candidates = [0.15, 0.20, 0.25, 0.28, 0.30, 0.33, 0.35, 0.38, 0.40, 0.45]
    ...
    # [3] p_high sweep (with best p_low)
    print("\n[3] p_high sweep ...")
    high_candidates = [0.45, 0.50, 0.55, 0.57, 0.60, 0.63, 0.65, 0.68, 0.70, 0.75, 0.80]
    ...
    print(f"\n[DONE] Phase 261 complete.  ({time.time()-t0:.1f}s)")
...
def _update_config(cfg, best_low, best_high, new_obj):
    ...
    subprocess.run(["git", "commit", "-m",
        f"quant: {new_ver} — breadth threshold r2 (p_low={best_low:.2f} p_high={best_high:.2f}) OBJ={new_obj:.4f}"],
