"""
Phase 290 — Per-Regime FTS Resweep against P284 Baseline [VECTORIZED]
=====================================================================
...
Grids same as P279.to seed search:
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase290_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase290_fts_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 290 — FTS Resweep against P284 Baseline [VECTORIZED]")
    print("Baseline: P284 OBJ=4.8412")
    print("=" * 70)
    ...
    out = Path("artifacts/phase290_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase290_fts_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase290_fts/phase290_fts_report.json")
    ...
    if validated:
        ...
        msg = (f"feat: P290 FTS resweep OBJ={reported_obj} "
