#!/usr/bin/env python3
"""
Phase 266 — MID Regime Re-retune Round 4
Current MID: v1=0.10, i460=0.00, i415=0.25, f168=0.65
After recent supervisor changes — verify and fine-tune MID
"""
...
def make_mid_w(base_weights, f168_w, v1_w, i415_frac):
...
def main():
    t0 = time.time()
    print("=" * 60)
    print("Phase 266 — MID Regime Re-retune Round 4")
    print("=" * 60)
    ...
    curr_rem = curr_mid["i415"] + curr_mid["i460"]
    curr_i415_frac = curr_mid["i415"] / curr_rem if curr_rem > 0 else 1.0

    # [2] f168 sweep
    print("\n[2] f168 sweep ...")
    f168_candidates = [0.00, 0.10, 0.20, 0.30, 0.40, 0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.90, 1.00]
    best_f168 = curr_mid["f168"]; best_f168_obj = baseline_obj
    for fw in f168_candidates:
        w_test = make_mid_w(base_weights, fw, curr_mid["v1"], curr_i415_frac)
    ...
    # [3] v1 sweep
    print("\n[3] v1 sweep ...")
    v1_candidates = [0.00, 0.05, 0.08, 0.10, 0.12, 0.15, 0.20, 0.25, 0.30]
    best_v1 = curr_mid["v1"]; best_v1_obj = best_f168_obj
    for vw in v1_candidates:
        if vw + best_f168 > 1.0: continue
        w_test = make_mid_w(base_weights, best_f168, vw, curr_i415_frac)
        obj = eval_weights(year_data, params, w_test)
    ...
    for frac in frac_candidates:
        w_test = make_mid_w(base_weights, best_f168, best_v1, frac)
        obj = eval_weights(year_data, params, w_test)
    ...
    # [5] LOYO validation
    print("\n[5] LOYO validation ...")
    best_weights = make_mid_w(base_weights, best_f168, best_v1, best_frac)
    wins = 0
    for yr in YEARS:
...
def _update_config(cfg, bw, new_obj):
    ...
    subprocess.run(["git", "commit", "-m",
        f"quant: {new_ver} — MID regime retune r4 (v1={bw['v1']:.2f} f168={bw['f168']:.2f}) OBJ={new_obj:.4f}"],
