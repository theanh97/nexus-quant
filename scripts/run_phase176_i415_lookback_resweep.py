"""
Phase 176 — I415 Lookback Re-Sweep (bw=144 locked)
=====================================================
P175 changed I415 beta_window from 216 → 144. The original lb=415 was
calibrated with bw=216. With bw=144 (more aggressive de-trending), the
optimal lookback may have shifted — same logic as P173 for I460.

Sweep: lookback_bars ∈ [300, 340, 380, 415, 455, 495, 530]
Baseline (v2.15.0): I415 lb=415, bw=144, OBJ≈2.5396

Design: V1, I460bw120lb460, F168lb168 run once per year (shared).
        I415 re-run for each lookback variant (bw=144 fixed).

Validate: LOYO ≥ 3/5 AND delta > 0.005 AND best_lb != 415 → update v2.16.0
"""
...
# P172b optimized regime weights (production v2.14.0 / unchanged in v2.15.0)
WEIGHTS = {
...
BASELINE_LB = 415
I415_BW     = 144   # locked from P175
I460_LB     = 460   # locked P173
I460_BW     = 120   # locked P172a
LB_SWEEP    = [300, 340, 380, 415, 455, 495, 530]
...
def _on_timeout(signum, frame):
    ...
    (OUT_DIR / "phase176_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
print("=" * 68)
print("PHASE 176 — I415 Lookback Re-Sweep (bw=144 locked)")
print("=" * 68)
print(f"  Baseline lb={BASELINE_LB}, bw={I415_BW}, OBJ≈2.5396 (v2.15.0)")
print(f"  Sweep lb: {LB_SWEEP}")
print(f"  I460: lb={I460_LB}, bw={I460_BW} locked\n")

print("[1/3] Loading data + signals + shared strategy returns...")
shared_rets: dict = {"v1": {}, "i460bw168": {}, "f168": {}}
i415_rets_by_lb: dict = {lb: {} for lb in LB_SWEEP}
signals_data: dict = {}
...
for year, (start, end) in YEAR_RANGES.items():
    ...
    # I415 variants: one run per lookback (bw=144 locked)
    for lb in LB_SWEEP:
        bt_cfg = BacktestConfig(costs=COST_MODEL)
        strat  = make_strategy({"name": "idio_momentum_alpha", "params": {
            "k_per_side": 4, "lookback_bars": lb, "beta_window_bars": I415_BW,
            "target_gross_leverage": 0.3, "rebalance_interval_bars": 48,
        }})
        result = BacktestEngine(bt_cfg).run(dataset, strat)
        i415_rets_by_lb[lb][year] = np.array(result.returns)

    print(f". ✓ [{len(LB_SWEEP)} lb variants]")

_partial.update({"phase": 176, "description": "I415 Lookback Re-Sweep (bw=144)", "partial": True})
...
for lb in LB_SWEEP:
    ...
    for year in YEARS:
        sr = {
            ...
            "i415bw216": i415_rets_by_lb[lb][year],
...
print("\n" + "=" * 68)
if validated:
    print(f"✅ VALIDATED — I415 lb={best_lb} bw={I415_BW}: OBJ={best_obj:.4f} (Δ={best_delta:+.4f}) LOYO {loyo_wins}/5")
else:
    print(f"❌ NO IMPROVEMENT — I415 lb={BASELINE_LB} bw={I415_BW} OBJ={baseline_obj:.4f} optimal")
    print(f"   Best: lb={best_lb} Δ={best_delta:+.4f} | LOYO {loyo_wins}/5")
print("=" * 68)

verdict = (
    f"VALIDATED — I415 lb={best_lb} bw={I415_BW} OBJ={best_obj:.4f} Δ={best_delta:+.4f} LOYO {loyo_wins}/5"
    if validated else
    f"NO IMPROVEMENT — I415 lb={BASELINE_LB} bw={I415_BW} OBJ={baseline_obj:.4f} optimal"
)

report = {
    "phase": 176,
    "description": "I415 Lookback Re-Sweep (bw=144 locked)",
    "elapsed_seconds": round(time.time() - _start, 1),
    "i415_bw_locked": I415_BW,
...
out_path = OUT_DIR / "phase176_report.json"
...
if validated:
    ...
    cfg["_validated"] += (
        f"; I415 lb sweep P176: lb={BASELINE_LB}→{best_lb} LOYO {loyo_wins}/5 "
        f"Δ={best_delta:+.4f} OBJ={best_obj:.4f} — PRODUCTION v2.16.0"
    )
    ...
    print(f"  Config: {prev} → v2.16.0 (I415 lb={BASELINE_LB}→{best_lb})")
else:
    print("\n❌ NO IMPROVEMENT — I415 lb=415 bw=144 remains optimal.")
