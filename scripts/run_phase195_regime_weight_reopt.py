"""
Phase 195 — Full Regime Weight Re-Optimization
================================================
After P194 changed i460 beta_window 120→168 (LOYO 3/5), and P190 changed
V1 vol_lb 168→192 (LOYO 4/5), re-optimize all regime weights jointly:
  - Sweep v1 weight + f168 weight per regime (2D grid, independent first)
  - i460/i415 absorb remainder in current proportion
  - LOYO validation on joint best combo

Baseline: v2.24.0, OBJ=2.8797 (with vol_lb=192 + i460_bw=168 now applied)
"""
...
def _on_timeout(signum, frame):
    ...
    (out / "phase195_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
# Overlay constants (from v2.24.0 config)
VOL_WINDOW    = 168
BRD_LB        = 192
PCT_WINDOW    = 336
...
TS_RT = 0.60; TS_RS = 0.40
TS_BT = 0.35; TS_BS = 1.60   # v2.24.0 config values (P176 updated)
DISP_THR   = 0.60
DISP_SCALE = 1.0

# v2.24.0 signal params (with fixes applied)
V1_PARAMS = {
    "k_per_side": 2, "w_carry": 0.35, "w_mom": 0.40, "w_mean_reversion": 0.25,
    "momentum_lookback_bars": 336, "mean_reversion_lookback_bars": 84,
    "vol_lookback_bars": 192,          # ← FIXED (was 168)
    "target_gross_leverage": 0.35, "rebalance_interval_bars": 60,
}
I460_PARAMS = {
    "k_per_side": 4, "lookback_bars": 460, "beta_window_bars": 168,  # ← FIXED (was 120)
...
# Current regime weights (v2.24.0)
BASE_WEIGHTS = {
...
# Fixed i460/(i460+i415) split ratio per regime
...
# Sweep grids: v1 weight × f168 weight per regime
SWEEPS = {
    "LOW":  {
        "v1":   [0.40, 0.44, 0.48, 0.50, 0.52, 0.55],
        "f168": [0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45],
    },
    "MID":  {
        "v1":   [0.25, 0.30, 0.35, 0.38, 0.40, 0.45],
        "f168": [0.15, 0.20, 0.25, 0.30, 0.35, 0.40],
    },
    "HIGH": {
        "v1":   [0.04, 0.06, 0.08, 0.10, 0.12, 0.15],
        "f168": [0.05, 0.08, 0.10, 0.12, 0.15, 0.18, 0.20],
    },
}
...
def main():
    t0 = time.time()
    print("=" * 65)
    print("Phase 195 — Regime Weight Full Re-Optimization")
    print(f"Baseline: v2.24.0  OBJ={BASELINE_OBJ}")
    print(f"Sweep:    v1 × f168 per regime (independent + joint LOYO)")
    print("=" * 65)

    # Load per-year data
    print("\n[1/4] Loading per-year data + pre-computing signals ...")
    all_data = {}
    ...
    # ── Per-regime sweep ────────────────────────────────────────────────────
    print("\n[3/4] Per-regime 2D sweep (v1 × f168) ...")
    best_per_regime = {}

    for regime in ["LOW", "MID", "HIGH"]:
        print(f"\n  Regime={regime}")
        v1_vals   = SWEEPS[regime]["v1"]
        f168_vals = SWEEPS[regime]["f168"]

        best_obj_r  = base_obj
        best_v1_r   = BASE_WEIGHTS[regime]["v1"]
        best_f168_r = BASE_WEIGHTS[regime]["f168"]

        for v1_w in v1_vals:
            for f168_w in f168_vals:
                w_r = make_weights(regime, v1_w, f168_w)
                if w_r is None: continue
                # Override only this regime
    ...
    print(f"\n{status} — Δ={delta:+.4f}  LOYO {loyo_wins}/{len(joint_yearly)}")
    ...
    out = Path("artifacts/phase195"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase195_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        print(f"\n  Config updated → v2.25.0  OBJ={round(joint_obj,4)}")

    print(f"\nReport → artifacts/phase195/phase195_report.json")
