"""
Phase 295 — Per-Regime Vol + Disp Overlay Re-Tune [VECTORIZED]
================================================================
Baseline: v2.49.20, OBJ=5.5998 (Phase 293 V1 weights + P294 regime weights)

After P293 (V1 internal weights changed) and P294 (regime weights changed),
the signal landscape has shifted. Re-tune both vol and disp overlays with
finer grids around current values.

Current (from P289):
  Vol thresh: LOW=0.53, MID=0.50, HIGH=0.57
  Vol scale:  LOW=0.40, MID=0.18, HIGH=0.03
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase295"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase295_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 295 — Vol + Disp Overlay Re-Tune [VECTORIZED]")
    print("=" * 70)
    ...
    result = {
        "phase": "295", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    out = Path("artifacts/phase295"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase295_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        msg = (f"feat: P295 vol+disp retune OBJ={reported_obj:.4f} "
