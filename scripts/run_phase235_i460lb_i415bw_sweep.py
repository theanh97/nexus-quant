"""
Phase 235 — I460 lookback_bars + I415 beta_window_bars re-sweep

With updated signal leverages (0.20), FTS params, and regime weights from phases 213-221,
optimal lookback/beta-window may have shifted. Re-confirm peaks.

I460 current: lb=480  → sweep [360, 400, 420, 440, 460, 480, 500, 520, 540]
I415 current: bw=144  → sweep [96, 120, 144, 168, 192, 216] (with best I460 lb)
...
def _on_timeout(signum, frame):
    ...
    (out / "phase235_report.json").write_text(json.dumps(_partial, indent=2, default=str))
    sys.exit(0)
...
I460_LB_SWEEP = [360, 400, 420, 440, 460, 480, 500, 520, 540]
I415_BW_SWEEP = [96, 120, 144, 168, 192, 216]
...
def load_year_base(year, params):
    """Load auxiliary data + dataset + V1 + F168 (fixed). I460/I415 re-run per sweep."""
    p = params
...
# ── MAIN ────────────────────────────────────────────────────────────────────
print("=" * 60)
print("Phase 235 — I460 lb + I415 beta_window re-sweep")
print("=" * 60)
_start = time.time()

cfg, WEIGHTS, PARAMS, v1p, i460p, i415p, f168p, baseline_obj = get_config()
ver_now = cfg.get("_version", "2.40.0")
curr_i460_lb = i460p.get("lookback_bars", 480)
curr_i415_bw = i415p.get("beta_window_bars", 144)
print(f"  Config: {ver_now}  baseline_obj(str)={baseline_obj:.4f}")
print(f"  I460 lb={curr_i460_lb}  I415 bw={curr_i415_bw}")

# [1] Load base data for all years
...
# ── Part A: I460 lb sweep ─────────────────────────────────────────────────
print(f"\n[3] Part A: I460 lb sweep (bw=168 fixed) ...")
best_i460_lb = curr_i460_lb
...
for lb in I460_LB_SWEEP:
    ...
    obj = obj_fn(yr_s)
    i460_lb_results[lb] = obj
    marker = " ←" if obj > best_i460_obj else ""
    print(f"  I460 lb={lb:4d}  OBJ={obj:.4f}{marker}")
    if obj > best_i460_obj:
        best_i460_obj = obj
        best_i460_lb = lb
print(f"  Best I460 lb: {best_i460_lb}  OBJ={best_i460_obj:.4f}")

# Re-run I460 with best lb
best_i460_rets = {}
for yr in YEARS:
    ds = base_data[yr]["dataset"]
    best_i460_rets[yr] = run_signal(ds, "idio_momentum_alpha", {
        "k_per_side": i460p.get("k_per_side", 4),
        "lookback_bars": best_i460_lb,
        "beta_window_bars": i460p.get("beta_window_bars", 168),
        "target_gross_leverage": i460p.get("target_gross_leverage", 0.20),
        "rebalance_interval_bars": i460p.get("rebalance_interval_bars", 48),
    })

# ── Part B: I415 beta_window sweep ───────────────────────────────────────
print(f"\n[4] Part B: I415 bw sweep (I460 lb={best_i460_lb}) ...")
best_i415_bw = curr_i415_bw
...
for bw in I415_BW_SWEEP:
    ...
    for yr in YEARS:
        ...
        yr_s[yr] = sharpe(ens, b["n"])
    obj = obj_fn(yr_s)
    i415_bw_results[bw] = obj
    marker = " ←" if obj > best_i415_obj else ""
    print(f"  I415 bw={bw:3d}  OBJ={obj:.4f}{marker}")
    if obj > best_i415_obj:
        best_i415_obj = obj
        best_i415_bw = bw
delta = best_i415_obj - baseline_computed
print(f"  Best I415 bw: {best_i415_bw}  OBJ={best_i415_obj:.4f}  Δ={delta:+.4f}")
...
for yr in YEARS:
    b = base_data[yr]; s = baseline_sigs[yr]
    ens = compute_ensemble(
        b, s["v1"], best_i460_rets[yr], best_i415_rets[yr], s["f168"], WEIGHTS, PARAMS)
...
out = Path("artifacts/phase235"); out.mkdir(parents=True, exist_ok=True)
report = {
    "phase": 235, "elapsed_s": elapsed,
    "best_i460_lb": best_i460_lb, "best_i415_bw": best_i415_bw,
    "best_obj": best_i415_obj, "delta": delta,
    "loyo_wins": loyo_wins, "validated": validated,
    "i460_lb_results": {str(k): v for k, v in i460_lb_results.items()},
    "i415_bw_results": {str(k): v for k, v in i415_bw_results.items()},
}
(out / "phase235_report.json").write_text(json.dumps(report, indent=2))
_partial.update(report)

if validated:
    print(f"\n✅ VALIDATED — updating config I460 lb={best_i460_lb} I415 bw={best_i415_bw}")
    cfg["ensemble"]["signals"]["i460bw168"]["params"]["lookback_bars"] = best_i460_lb
    cfg["ensemble"]["signals"]["i415bw216"]["params"]["beta_window_bars"] = best_i415_bw
    ...
    cfg["_validated"] = (old_val +
        f" | v{new_ver} I460lb={best_i460_lb} I415bw={best_i415_bw} "
        f"OBJ={best_i415_obj:.4f} LOYO={loyo_wins}/5")
    Path("configs/production_p91b_champion.json").write_text(json.dumps(cfg, indent=2))
    msg = (f"phase235: I460lb+I415bw -- VALIDATED I460lb={best_i460_lb} "
           f"I415bw={best_i415_bw} LOYO {loyo_wins}/5 delta={delta:+.4f} OBJ={best_i415_obj:.4f}")
    os.system(f"git add configs/production_p91b_champion.json artifacts/phase235/ && "
    ...
    msg = (f"phase235: I460lb+I415bw -- WEAK LOYO {loyo_wins}/5 delta={delta:+.4f} "
           f"lb={best_i460_lb} bw={best_i415_bw}")
    os.system(f"git add artifacts/phase235/ && "
              f"git stash && git pull --rebase && git stash pop && "
              f"git commit -m '{msg}' && git push")

print(f"  Report saved  ({elapsed:.1f}s)")
print(f"\n[DONE] Phase 235 complete.")
