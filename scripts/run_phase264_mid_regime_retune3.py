#!/usr/bin/env python3
"""
Phase 264 — MID Regime Re-retune Round 3
Current MID: v1=0.1, i460=0.0, i415=0.25, f168=0.65 (from parallel P260)
MID now covers brd_pct 0.30-0.68 (wider than pre-P261).
Sweep: A) f168 weight, B) v1 weight, C) i415 weight (allow non-zero idio)
"""
...
def make_mid_w(base_weights, v1_w, f168_w, i415_w):
    """MID: v1 + f168 + i415 (allow non-zero i415); i460=0 in MID."""
    i460_w = 0.0
    total = v1_w + f168_w + i415_w + i460_w
    if total > 1.0:
        return None  # invalid
    return {"LOW": base_weights["LOW"], "HIGH": base_weights["HIGH"],
            "MID": {"v1": v1_w, "i460": i460_w, "i415": i415_w, "f168": f168_w}}

def main():
    t0 = time.time()
    print("=" * 60)
    print("Phase 264 — MID Regime Re-retune Round 3")
    print("MID now covers brd_pct 0.30-0.68 (broader)")
    print("=" * 60)
    ...
    print(f"  p_high: {params['p_high']:.2f}")
    print(f"  Current MID: v1={curr_mid['v1']:.3f} i460={curr_mid['i460']:.3f} "
          f"i415={curr_mid['i415']:.3f} f168={curr_mid['f168']:.3f}")
    ...
    # [2] f168 sweep (hold v1 and i415 fixed)
    print("\n[2] f168 sweep ...")
    f168_candidates = [0.00, 0.05, 0.10, 0.15, 0.20, 0.30, 0.40, 0.50,
                       0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90, 0.95, 1.00]
    best_f168 = curr_mid["f168"]; best_f168_obj = baseline_obj
    for fw in f168_candidates:
        total = curr_mid["v1"] + fw + curr_mid["i415"]
        if total > 1.001: continue
        w_test = make_mid_w(base_weights, curr_mid["v1"], fw, curr_mid["i415"])
    ...
    # [3] v1 sweep (with best f168)
    print("\n[3] v1 sweep ...")
    v1_candidates = [0.00, 0.02, 0.05, 0.08, 0.10, 0.12, 0.15, 0.18, 0.20, 0.25, 0.30]
    best_v1 = curr_mid["v1"]; best_v1_obj = best_f168_obj
    for vw in v1_candidates:
        total = vw + best_f168 + curr_mid["i415"]
        if total > 1.001: continue
        w_test = make_mid_w(base_weights, vw, best_f168, curr_mid["i415"])
        if w_test is None: continue
        obj = eval_weights(year_data, params, w_test)
    ...
    # [4] i415 sweep (with best f168 and v1)
    print("\n[4] i415 sweep ...")
    i415_candidates = [0.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40]
    best_i415 = curr_mid["i415"]; best_i415_obj = best_v1_obj
    for iw in i415_candidates:
        total = best_v1 + best_f168 + iw
        if total > 1.001: continue
        w_test = make_mid_w(base_weights, best_v1, best_f168, iw)
        if w_test is None: continue
        obj = eval_weights(year_data, params, w_test)
    ...
    # [5] LOYO validation
    print("\n[5] LOYO validation ...")
    best_weights = make_mid_w(base_weights, best_v1, best_f168, best_i415)
    wins = 0
    for yr in YEARS:
    ...
    print(f"  Best MID: v1={bm['v1']:.3f} i460={bm['i460']:.3f} i415={bm['i415']:.3f} f168={bm['f168']:.3f}")

    if wins >= 3 and delta > 0.005:
        print(f"\n  VALIDATED  (LOYO {wins}/5 delta={delta:+.4f})")
        _update_config(cfg, bm, best_i415_obj)
    else:
        print(f"\n  CONFIRMED OPTIMAL  (LOYO {wins}/5 delta={delta:+.4f})")

    print(f"\n[DONE] Phase 264 complete.  ({time.time()-t0:.1f}s)")

def _update_config(cfg, bm, new_obj):
    old_ver = cfg.get("version", "v2.42.14")
    parts = old_ver.lstrip("v").split(".")
    parts[-1] = str(int(parts[-1]) + 1)
    new_ver = "v" + ".".join(parts)
    rw = cfg["breadth_regime_switching"]["regime_weights"]["MID"]
    rw["v1"]        = round(bm["v1"], 4)
    rw["i460bw168"] = round(bm["i460"], 4)
    rw["i415bw216"] = round(bm["i415"], 4)
    rw["f168"]      = round(bm["f168"], 4)
    cfg["version"] = new_ver
    p = Path("configs/production_p91b_champion.json")
    p.write_text(json.dumps(cfg, indent=2))
    print(f"  Config updated: {old_ver} → {new_ver}")
    print(f"  MID: v1={bm['v1']:.3f} i460={bm['i460']:.3f} i415={bm['i415']:.3f} f168={bm['f168']:.3f}")
    print(f"  New OBJ={new_obj:.4f}")
    subprocess.run(["git", "add", str(p)], cwd="/Users/truonglys/projects/quant")
    subprocess.run(["git", "commit", "-m",
        f"quant: {new_ver} — MID retune r3 (v1={bm['v1']:.2f} f168={bm['f168']:.2f} i415={bm['i415']:.2f}) OBJ={new_obj:.4f}"],
