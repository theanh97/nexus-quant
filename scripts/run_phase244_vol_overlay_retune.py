"""
Phase 244 — Vol Overlay Re-tune (post P242/P243 regime/FTS changes)
====================================================================
The regime weights and FTS params changed significantly in P242-243.
Vol overlay may need re-tuning.

Current: vol_thr=0.50, vol_scale=0.30

Parts:
  A: vol_thr  in [0.30, 0.35, 0.40, 0.45, 0.50, 0.55, 0.60, 0.65, 0.70]
  B: vol_scale in [0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.50]
...
def main():
    print("=" * 60)
    print("Phase 244 — Vol Overlay Re-tune (post P242/P243)")
    print("=" * 60)
    ...
    thr_grid = [0.30, 0.35, 0.40, 0.45, 0.50, 0.55, 0.60, 0.65, 0.70]
    best_thr_obj = -999.0
    best_thr = params["vol_thr"]
    for thr in thr_grid:
        p = dict(best_params); p["vol_thr"] = thr
        o = obj_fn(eval_all(year_data, weights, p))
        marker = " <-" if o > best_thr_obj else ""
        print(f"  vol_thr={thr:.2f}  OBJ={o:.4f}{marker}")
        if o > best_thr_obj: best_thr_obj = o; best_thr = thr
    best_params["vol_thr"] = best_thr
    print(f"  Best vol_thr: {best_thr:.2f}  OBJ={best_thr_obj:.4f}")

    # Part B: vol_scale
    print(f"\n[3] Part B: vol_scale sweep ...")
    scale_grid = [0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.50]
    best_scale_obj = -999.0
    best_scale = params["vol_scale"]
    for scale in scale_grid:
        p = dict(best_params); p["vol_scale"] = scale
        o = obj_fn(eval_all(year_data, weights, p))
        marker = " <-" if o > best_scale_obj else ""
        print(f"  vol_scale={scale:.2f}  OBJ={o:.4f}{marker}")
        if o > best_scale_obj: best_scale_obj = o; best_scale = scale
    best_params["vol_scale"] = best_scale
    delta = best_scale_obj - baseline_obj
    print(f"  Best vol_scale: {best_scale:.2f}  OBJ={best_scale_obj:.4f}  D={delta:+.4f}")
    ...
    if wins >= 3 and delta > 0.005:
        ...
        cfg2["vol_regime_overlay"]["threshold"]    = best_params["vol_thr"]
        cfg2["vol_regime_overlay"]["scale_factor"] = best_params["vol_scale"]

        old_ver = cfg2.get("version", "2.42.2")
        ...
        script_name = "scripts/run_phase244_vol_overlay_retune.py"
        commit_msg = (f"feat(p244): vol retune thr={best_thr} scale={best_scale} "
                      f"OBJ={best_scale_obj:.4f} (D={delta:+.4f} LOYO {wins}/5) [{new_ver}]")
        subprocess.run(
