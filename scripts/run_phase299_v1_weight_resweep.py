"""
Phase 299 — Per-Regime V1 Internal Weight Re-Sweep [VECTORIZED]
================================================================
Baseline: v2.49.22, OBJ=5.6545 (Phase 298 regime weight reopt19)

V1 internal weights (carry/mom/mean_reversion) last optimized in P293.
Since then: P295 vol/disp, P298 regime weights.
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase299"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase299_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 299 — Per-Regime V1 Internal Weight Re-Sweep")
    print("Baseline: v2.49.22, OBJ=5.6545")
    print("=" * 70)
    ...
    # [7] Save report
    result = {
        "phase": "299", "title": "Per-Regime V1 Weight Re-Sweep",
    ...
    out = Path("artifacts/phase299"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase299_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase299/phase299_report.json")
    ...
    if validated:
        ...
        old_ver = cfg.get("_version", "2.49.22")
        parts = old_ver.split(".")
        parts[-1] = str(int(parts[-1]) + 1)
        new_ver = ".".join(parts)
        cfg["_version"] = new_ver

        old_vver = cfg.get("version", "v2.49.22").lstrip("v")
        ...
        msg = (f"feat: P299 V1 weight resweep OBJ={reported_obj:.4f} "
