"""
Phase 284 — Regime Weight Re-opt (15th Pass) [VECTORIZED]
=========================================================
Baseline: v2.49.6, OBJ=4.8279

After P279 (FTS resweep), P280 (per-regime V1 weights — BIG WIN),
and P281 (vol+disp resweep), the optimal ensemble weights per regime
may have shifted substantially. This phase re-sweeps regime weights
from the new baseline.

FIX from P271: v1_HIGH was incorrectly mapped to v1_MID.
Now each regime uses its own V1 returns.
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase284"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase284_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
if __name__ == "__main__":
    t0 = time.time()
    print("=" * 65)
    print("Phase 284 — Regime Weight Re-opt (15th Pass) [VECTORIZED]")
    print("FIX: v1_HIGH now correctly mapped (was v1_MID in P271)")
    ...
    report = {
        ...
        "v1_high_fix": "v1_HIGH now correctly mapped (was v1_MID in P271)",
        "runtime_seconds": round(time.time() - t0, 1),
    }
    out = Path("artifacts/phase284"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase284_report.json").write_text(json.dumps(report, indent=2))
    ...
    if validated:
        ...
        msg = (f"feat: P284 regime weight reopt15 OBJ={reported_obj:.4f} "
