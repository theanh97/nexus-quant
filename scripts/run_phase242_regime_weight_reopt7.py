"""
Phase 242 — Regime Weight Re-Opt (7th Pass)
=============================================
P241: FTS joint re-sweep showed LOYO 2/5 (overfitting pattern — near ceiling).
P238 validated per-regime ts_pct_win (LOW=240, MID=288, HIGH=400).

Since P233 (last weight re-opt), the ts_pct_win per regime changed.
Different percentile windows shift the distribution of the FTS trigger signal,
which could change which regime benefits more/less from FTS overlay.
Regime weights should be re-evaluated with updated ts_pct_win.

Same structure as P233: iterative per-regime v1×f168 sweep (2 passes).
Fixed idio total per regime; sweep v1 and f168 (complementary).

v1 grid: [0.02 .. 0.60 step 0.02]
idio_frac (i460/(i460+i415)): 12 steps [0..1]
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase242"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase242_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
BASELINE_OBJ = 3.7924
V1_GRID = [round(x * 0.02, 2) for x in range(1, 31)]
IDIO_STEPS = 12
...
def main():
    t0 = time.time()
    print("=" * 65)
    print("Phase 242 — Regime Weight Re-Opt (7th Pass)")
    ...
    idio_fracs = [round(x / (IDIO_STEPS - 1), 3) for x in range(IDIO_STEPS)]
    ...
    result = {
        "phase": 242, "baseline_obj": float(base_obj_val), "best_obj": float(joint_obj_val),
    ...
    out = Path("artifacts/phase242"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase242_report.json").write_text(json.dumps(result, indent=2))

    if validated:
        cfg_path = ROOT / "configs" / "production_p91b_champion.json"
        cfg = json.load(open(cfg_path))
        brs_w = cfg["breadth_regime_switching"]["regime_weights"]
        for rname, wts in best_weights.items():
            brs_w[rname]["v1"] = wts["v1"]
            brs_w[rname]["i460bw168"] = wts["i460"]
            brs_w[rname]["i415bw216"] = wts["i415"]
            brs_w[rname]["f168"] = wts["f168"]
        cfg["_version"] = "2.43.0"
