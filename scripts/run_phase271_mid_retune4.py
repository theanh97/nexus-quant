#!/usr/bin/env python3
"""
Phase 271 — MID Regime Retune Round 4
Parallel session set MID v1=0.15, f168=0.7 — re-optimize from current state.
Sweeps f168, v1, idio_ratio for MID regime.
Keeps LOW and HIGH fixed at current config values.
...
def main():
    ...
    for f168_w in [0.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40,
    ...
    if wins >= 3 and delta > 0.005:
        ...
        msg = (f"quant: {new_ver} — MID retune r4 "
    ...
    print(f"\n[DONE] Phase 271 complete.  ({elapsed:.1f}s)")
