#!/usr/bin/env python3
"""
Phase 273 — MID Regime Retune Round 6
Current MID (supervisor v2.49.x): v1=0.15, i460=0.00, i415=0.15, f168=0.70
My earlier finding (P270): v1=0.00, f168=1.10 (pure amplified contrarian)
Systematic sweep to find the LOYO-validated optimum under current full config.
...
def try_mid(yearly_data, params, base_weights, mv1, mi460, mi415, mf168):
    w = {
        "HIGH": base_weights["HIGH"],
        "MID":  {"v1": mv1, "i460": mi460, "i415": mi415, "f168": mf168},
...
def main():
    ...
    print(f"    Current MID: v1={cv1} i460={ci460} i415={ci415} f168={cf168}")
    print(f"    HIGH: {weights['HIGH']}  LOW: {weights['LOW']}")
    ...
    # [3] Sweep f168 (dominant signal for MID contrarian)
    print("\n[3] f168 sweep ...")
    for f in [0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 1.00, 1.10, 1.20, 1.30]:
        o = try_mid(yearly_data, params, weights, best_v1, best_i460, best_i415, f)
    ...
    # [4] Sweep v1
    print("\n[4] v1 sweep ...")
    for v in [0.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30]:
        o = try_mid(yearly_data, params, weights, v, best_i460, best_i415, best_f168)
        marker = " <--" if o > best_obj else ""
    ...
    # [5] Sweep i415
    print("\n[5] i415 sweep ...")
    for a in [0.00, 0.05, 0.10, 0.15, 0.20, 0.25]:
        o = try_mid(yearly_data, params, weights, best_v1, best_i460, a, best_f168)
        marker = " <--" if o > best_obj else ""
    ...
    print("\n[6] i460 sweep ...")
    for a in [0.00, 0.05, 0.10, 0.15, 0.20, 0.25]:
        o = try_mid(yearly_data, params, weights, best_v1, a, best_i415, best_f168)
        marker = " <--" if o > best_obj else ""
    ...
    print("\n[7] Fine-tune f168 ...")
    fine_f = sorted(set([round(best_f168 + d, 2) for d in [-0.10, -0.05, 0.0, 0.05, 0.10]]))
    for f in fine_f:
        if f <= 0.0: continue
        o = try_mid(yearly_data, params, weights, best_v1, best_i460, best_i415, f)
        marker = " <--" if o > best_obj else ""
    ...
    best_weights = {
        "HIGH": weights["HIGH"],
        "MID":  {"v1": best_v1, "i460": best_i460, "i415": best_i415, "f168": best_f168},
        "LOW":  weights["LOW"],
    }
    wins = 0; deltas = []
    for y in YEARS:
    ...
    if wins >= 3 and delta > 0.005:
        ...
        # Re-read config fresh to avoid clobbering supervisor changes
        cfg_fresh = json.loads(Path("configs/production_p91b_champion.json").read_text())
        old_ver = cfg_fresh.get("version", "v2.49.3")
        parts   = old_ver.split(".")
        new_ver = f"{parts[0]}.{parts[1]}.{int(parts[2]) + 1}"
        cfg_fresh["version"] = new_ver
        brs = cfg_fresh["breadth_regime_switching"]
        mid = brs["regime_weights"]["MID"]
        mid["v1"]        = round(float(best_v1), 4)
        mid["i460bw168"] = round(float(best_i460), 4)
        mid["i415bw216"] = round(float(best_i415), 4)
        mid["f168"]      = round(float(best_f168), 4)
        for k in ["i460", "i415"]: mid.pop(k, None)
        ...
        print(f"  MID: v1={best_v1} i460={best_i460} i415={best_i415} f168={best_f168}")
        print(f"  New OBJ={best_obj:.4f}")
        msg = (f"quant: {new_ver} — MID regime retune r6 "
        ...
        print(f"  Keeping MID: v1={cv1} i460={ci460} i415={ci415} f168={cf168}")

    print(f"\n[DONE] Phase 273 complete.  ({time.time()-t0:.1f}s)")
