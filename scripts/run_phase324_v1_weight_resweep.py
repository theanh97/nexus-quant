"""
Phase 324 — Per-Regime V1 Internal Weight Re-Sweep [VECTORIZED]
================================================================
Baseline: v2.49.35, OBJ=6.2892 (parallel session advances)

V1 internal weights (carry/mom/mean_reversion) — re-sweep after
P320-P323 confirmed all other params optimal.

Current (from P323 confirmed optimal):
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase324"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase324_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 324 — Per-Regime V1 Internal Weight Re-Sweep")
    print("=" * 70)
    ...
    result = {
        "phase": "324", "title": "Per-Regime V1 Weight Re-Sweep",
    ...
    out = Path("artifacts/phase324"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase324_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase324/phase324_report.json")
    ...
    if validated:
        ...
        old_vver = cfg.get("version", "v2.49.35").lstrip("v")
