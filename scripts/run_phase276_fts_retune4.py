#!/usr/bin/env python3
"""
Phase 276 — FTS Overlay Retune Round 4
After MID/HIGH/vol/breadth updates, re-sweep FTS params.
...
def compute_ensemble(data, params, weights, ts_rt=None, ts_rs=None, ts_bt=None, ts_bs=None):
    ...
    rt = ts_rt if ts_rt is not None else p["ts_rt"]
    rs = ts_rs if ts_rs is not None else p["ts_rs"]
    bt = ts_bt if ts_bt is not None else p["ts_bt"]
    bs = ts_bs if ts_bs is not None else p["ts_bs"]
...
def yearly_sharpe(data, params, weights, ts_rt=None, ts_rs=None, ts_bt=None, ts_bs=None):
...
def eval_obj(yearly_data, params, weights, ts_rt=None, ts_rs=None, ts_bt=None, ts_bs=None):
    sharpes = [yearly_sharpe(yearly_data[yr], params, weights, ts_rt, ts_rs, ts_bt, ts_bs)
               for yr in YEARS]
...
def main():
    ...
    print(f"    FTS: rt={curr_rt} rs={curr_rs} bt={curr_bt} bs={curr_bs}")
    ...
    # [3] Sweep ts_bt (boost threshold — contango → boost)
    print("\n[3] ts_bt sweep ...")
    best_bt  = curr_bt
    best_obj = baseline
    for bt in [0.10, 0.12, 0.15, 0.17, 0.18, 0.19, 0.20, 0.21, 0.22, 0.23,
               0.25, 0.27, 0.28, 0.30]:
        o = eval_obj(yearly_data, params, weights, ts_bt=bt)
        marker = " <--" if o > best_obj else ""
        print(f"  ts_bt={bt:.2f}  OBJ={o:.4f}{marker}", flush=True)
        if o > best_obj:
            best_obj = o; best_bt = bt
    print(f"  Best ts_bt: {best_bt}  D={best_obj-baseline:+.4f}")

    # [4] Sweep ts_bs (boost scale)
    print("\n[4] ts_bs sweep ...")
    best_bs = curr_bs
    for bs in [1.20, 1.40, 1.60, 1.80, 2.00, 2.20, 2.40, 2.60, 2.80, 3.00, 3.50, 4.00]:
        o = eval_obj(yearly_data, params, weights, ts_bt=best_bt, ts_bs=bs)
        marker = " <--" if o > best_obj else ""
        print(f"  ts_bs={bs:.2f}  OBJ={o:.4f}{marker}", flush=True)
        if o > best_obj:
            best_obj = o; best_bs = bs
    print(f"  Best ts_bs: {best_bs}  D={best_obj-baseline:+.4f}")

    # [5] Sweep ts_rt (reduce threshold — backwardation → reduce)
    print("\n[5] ts_rt sweep ...")
    best_rt = curr_rt
    for rt in [0.50, 0.52, 0.55, 0.58, 0.60, 0.62, 0.65, 0.68, 0.70, 0.75, 0.80]:
        o = eval_obj(yearly_data, params, weights, ts_rt=rt, ts_bt=best_bt, ts_bs=best_bs)
        marker = " <--" if o > best_obj else ""
        print(f"  ts_rt={rt:.2f}  OBJ={o:.4f}{marker}", flush=True)
        if o > best_obj:
            best_obj = o; best_rt = rt
    print(f"  Best ts_rt: {best_rt}  D={best_obj-baseline:+.4f}")

    # [6] Sweep ts_rs (reduce scale)
    print("\n[6] ts_rs sweep ...")
    best_rs = curr_rs
    for rs in [0.00, 0.02, 0.05, 0.08, 0.10, 0.15, 0.20, 0.25, 0.30, 0.40, 0.50]:
        o = eval_obj(yearly_data, params, weights, ts_rt=best_rt, ts_rs=rs, ts_bt=best_bt, ts_bs=best_bs)
        marker = " <--" if o > best_obj else ""
        print(f"  ts_rs={rs:.2f}  OBJ={o:.4f}{marker}", flush=True)
        if o > best_obj:
            best_obj = o; best_rs = rs
    print(f"  Best ts_rs: {best_rs}  D={best_obj-baseline:+.4f}")

    # [7] LOYO validation
    print(f"\n[7] LOYO (rt={best_rt} rs={best_rs} bt={best_bt} bs={best_bs}) ...")
    wins = 0
    for yr in YEARS:
        s_best = yearly_sharpe(yearly_data[yr], params, weights, best_rt, best_rs, best_bt, best_bs)
        s_curr = yearly_sharpe(yearly_data[yr], params, weights, curr_rt, curr_rs, curr_bt, curr_bs)
    ...
    if wins >= 3 and delta > 0.005:
        ...
        fts2 = cfg2.setdefault("funding_term_structure_overlay", {})
        fts2["reduce_threshold"] = round(float(best_rt), 4)
        fts2["reduce_scale"]     = round(float(best_rs), 4)
        fts2["boost_threshold"]  = round(float(best_bt), 4)
        fts2["boost_scale"]      = round(float(best_bs), 4)
        ...
        msg = (f"quant: {new_ver} — FTS retune r4 "
               f"(rt={best_rt} rs={best_rs} bt={best_bt} bs={best_bs}) "
        ...
        print(f"  FTS stays: rt={curr_rt} rs={curr_rs} bt={curr_bt} bs={curr_bs}")
