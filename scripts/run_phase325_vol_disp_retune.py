"""
Phase 325 — Per-Regime Vol + Disp Overlay Re-Tune [VECTORIZED]
================================================================
Baseline: v2.49.36, OBJ=6.4480 (P324 V1 weight resweep — VALIDATED D=+0.1588)

After full cycle P320-P324 with P324 validated (V1 weights),
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase325"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase325_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 325 — Vol + Disp Overlay Re-Tune [VECTORIZED]")
    print("=" * 70)
    ...
    result = {
        "phase": "325", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    out = Path("artifacts/phase325"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase325_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        old_vver = cfg.get("version", "v2.49.36").lstrip("v")
        ...
        msg = (f"feat: P325 vol+disp retune OBJ={reported_obj:.4f} "
