#!/usr/bin/env python3
"""
Phase 271 — FTS Overlay Retune Round 3
Regime structure now stable:
  HIGH: v1=0.20 i460=0.20 i415=0.30 f168=0.30
  MID:  pure f168=1.10 (amplified contrarian)
  LOW:  v1=0.30 i460=0.10 i415=0.20 f168=0.40
Not retested since P259 (before major regime changes)
...
def main():
    t0 = time.time()
    print("=" * 60)
    print("Phase 271 — FTS Overlay Retune Round 3")
    print("Current: rt=0.55 rs=0.05 bt=0.22 bs=2.00")
    print("=" * 60)
    ...
    # [2] ts_rt sweep
    print("\n[2] ts_rt sweep ...")
    rt_cands = [0.40, 0.45, 0.50, 0.52, 0.55, 0.57, 0.60, 0.65, 0.70]
    best_rt = params["ts_rt"]; best_rt_obj = baseline_obj
    for rt in rt_cands:
        obj = eval_fts(year_data, params, weights, rt, params["ts_rs"],
                       params["ts_bt"], params["ts_bs"])
        marker = " <-" if obj > best_rt_obj else ""
        print(f"  ts_rt={rt:.2f}  OBJ={obj:.4f}{marker}")
        if obj > best_rt_obj:
            best_rt_obj = obj; best_rt = rt
    print(f"  Best ts_rt: {best_rt:.2f}  D={best_rt_obj-baseline_obj:+.4f}")

    # [3] ts_rs sweep
    print("\n[3] ts_rs sweep ...")
    rs_cands = [0.00, 0.02, 0.05, 0.08, 0.10, 0.12, 0.15, 0.20, 0.25, 0.30]
    best_rs = params["ts_rs"]; best_rs_obj = best_rt_obj
    for rs in rs_cands:
        obj = eval_fts(year_data, params, weights, best_rt, rs,
                       params["ts_bt"], params["ts_bs"])
        marker = " <-" if obj > best_rs_obj else ""
        print(f"  ts_rs={rs:.2f}  OBJ={obj:.4f}{marker}")
        if obj > best_rs_obj:
            best_rs_obj = obj; best_rs = rs
    print(f"  Best ts_rs: {best_rs:.2f}  D={best_rs_obj-baseline_obj:+.4f}")

    # [4] ts_bt sweep
    print("\n[4] ts_bt sweep ...")
    bt_cands = [0.15, 0.18, 0.20, 0.22, 0.25, 0.28, 0.30, 0.35]
    best_bt = params["ts_bt"]; best_bt_obj = best_rs_obj
    for bt in bt_cands:
        obj = eval_fts(year_data, params, weights, best_rt, best_rs,
                       bt, params["ts_bs"])
        marker = " <-" if obj > best_bt_obj else ""
        print(f"  ts_bt={bt:.2f}  OBJ={obj:.4f}{marker}")
        if obj > best_bt_obj:
            best_bt_obj = obj; best_bt = bt
    print(f"  Best ts_bt: {best_bt:.2f}  D={best_bt_obj-baseline_obj:+.4f}")

    # [5] ts_bs sweep
    print("\n[5] ts_bs sweep ...")
    bs_cands = [1.00, 1.25, 1.50, 1.75, 2.00, 2.25, 2.50, 2.75, 3.00, 3.50]
    ...
    print(f"  Best ts_bs: {best_bs:.2f}  D={best_bs_obj-baseline_obj:+.4f}")

    # [6] LOYO validation
    print("\n[6] LOYO validation ...")
    wins = 0
    for yr in YEARS:
        sh_best = yearly_sharpe(year_data[yr],
                                {**params, "ts_rt": best_rt, "ts_rs": best_rs,
                                 "ts_bt": best_bt, "ts_bs": best_bs}, weights)
    ...
    print(f"\n[DONE] Phase 271 complete.  ({time.time()-t0:.1f}s)")

def _update_config(cfg, rt, rs, bt, bs, new_obj):
    old_ver = cfg.get("version", "v2.42.20")
    ...
    subprocess.run(["git", "commit", "-m",
        f"quant: {new_ver} — FTS retune r3 (rt={rt:.2f} bs={bs:.2f}) OBJ={new_obj:.4f}"],
