"""
Phase 226 — BS Extension Sweep (find true peak beyond 2.50)
============================================================
In P225, BS=2.50 was still rising (monotonic from 1.70→2.50).
The true optimal BS may be higher. Extend sweep: [2.50, 2.75, 3.00,
3.25, 3.50, 4.00, 4.50, 5.00, 6.00].
Also fine-tune RS around 0.35: [0.25, 0.30, 0.35, 0.40, 0.50].
All with RT=0.65, BT=0.25.
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase226"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase226_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
TS_RS_BASE = 0.35; TS_BS_BASE = 2.50  # from P225
...
BASELINE_OBJ = 3.2437
BS_EXT_SWEEP = [2.50, 2.75, 3.00, 3.25, 3.50, 4.00, 4.50, 5.00, 6.00]
RS_FINE_SWEEP = [0.25, 0.30, 0.35, 0.40, 0.50]
...
def ensemble_rets(ts_rs, ts_bs, base_data):
...
def evaluate(ts_rs, ts_bs, all_data):
    yearly = {yr: sharpe(ensemble_rets(ts_rs, ts_bs, d)) for yr, d in all_data.items()}
    return obj(yearly), yearly

def main():
    t0 = time.time()
    print("=" * 65)
    print("Phase 226 — BS Extension Sweep (find peak beyond 2.50)")
    print(f"Baseline: v2.36.0  OBJ={BASELINE_OBJ}")
    print(f"Current: RS={TS_RS_BASE}  BS={TS_BS_BASE}  RT={TS_RT}  BT={TS_BT}")
    print(f"BS ext: {BS_EXT_SWEEP}")
    print(f"RS fine: {RS_FINE_SWEEP}")
    print("=" * 65)
    ...
    base_obj_val, base_yearly = evaluate(TS_RS_BASE, TS_BS_BASE, all_data)
    print(f"\n  Baseline OBJ = {base_obj_val:.4f}  (expected ≈ {BASELINE_OBJ})")
    _partial.update({"baseline_obj": float(base_obj_val)})

    print("\n[2/3] 2D Sweep (RS × BS)...")
    best_obj_val = base_obj_val; best_rs = TS_RS_BASE; best_bs = TS_BS_BASE
    results = []
    for rs in RS_FINE_SWEEP:
        for bs in BS_EXT_SWEEP:
            o, _ = evaluate(rs, bs, all_data)
            delta = o - base_obj_val
            marker = " ← BEST" if o > best_obj_val else ""
            print(f"  RS={rs:.2f}  BS={bs:.2f}  OBJ={o:.4f}  Δ={delta:+.4f}{marker}")
            results.append((rs, bs, o))
            if o > best_obj_val:
                best_obj_val = o; best_rs = rs; best_bs = bs

    print(f"\n  → Best: RS={best_rs}  BS={best_bs}  OBJ={best_obj_val:.4f}")
    ...
    print(f"\n{'✅ VALIDATED' if validated else '❌ NO IMPROVEMENT'} — RS={best_rs}  BS={best_bs}")

    result = {
        "phase": 226, "baseline_obj": float(base_obj_val), "best_obj": float(joint_obj_val),
        "delta": float(delta), "loyo_wins": loyo_wins, "loyo_total": len(joint_yearly),
        "validated": validated, "best_rs": best_rs, "best_bs": best_bs,
        "top5": sorted(results, key=lambda x: -x[2])[:5],
    }
    _partial.update(result)
    out = Path("artifacts/phase226"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase226_report.json").write_text(json.dumps(result, indent=2))
