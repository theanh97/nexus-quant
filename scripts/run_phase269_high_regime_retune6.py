...
Supervisor reverted HIGH to f168=0.00 (pure i460+v1)
My P267 found f168=0.35 / i415=0.315 with LOYO 4/5 D=+0.5782
Re-verify: does HIGH still benefit from f168 and i415?
Current: v1=0.35, i460=0.65, i415=0.00, f168=0.00
"""
...
def make_high_w(base_weights, v1_w, f168_w, i415_frac):
...
def main():
    t0 = time.time()
    print("=" * 60)
    print("Phase 269 — HIGH Regime Re-retune Round 6")
    print("Current: v1=0.35, i460=0.65, i415=0.00, f168=0.00")
    print("Re-verify: P267 found f168=0.35/i415=0.315 LOYO 4/5")
    print("=" * 60)
    ...
    print(f"  Current HIGH: v1={curr['v1']:.2f} i460={curr['i460']:.3f} i415={curr['i415']:.3f} f168={curr['f168']:.2f}")

    # Grid: v1 × f168 × i415_frac (coarse first)
    print("\n[2] Joint grid: v1 × f168 × i415_frac ...")
    v1_cands    = [0.15, 0.20, 0.25, 0.30, 0.35]
    f168_cands  = [0.00, 0.15, 0.25, 0.35, 0.45]
    i415_cands  = [0.00, 0.30, 0.50, 0.70, 1.00]

    best_obj = baseline_obj
    best_v1 = curr["v1"]; best_f168 = curr["f168"]; best_i415f = 0.0

    for v1_w in v1_cands:
        for f168_w in f168_cands:
            if v1_w + f168_w >= 1.0: continue
            for i415f in i415_cands:
                w_test = make_high_w(base_weights, v1_w, f168_w, i415f)
    ...
    # [3] Fine tune around best
    print("\n[3] Fine-tune f168 ...")
    f168_fine = [max(0.0, best_f168 - 0.10 + i*0.05) for i in range(5)]
    f168_fine = sorted(set(round(v, 2) for v in f168_fine))
    for fw in f168_fine:
        w_test = make_high_w(base_weights, best_v1, fw, best_i415f)
        obj = eval_weights(year_data, params, w_test)
    ...
    print("\n[4] Fine-tune i415 fraction ...")
    i415_fine = [round(i * 0.1, 1) for i in range(11)]
    for frac in i415_fine:
        if best_v1 + best_f168 >= 1.0: continue
        w_test = make_high_w(base_weights, best_v1, best_f168, frac)
        obj = eval_weights(year_data, params, w_test)
        marker = " <-" if obj > best_obj else ""
    ...
    # [5] LOYO
    print("\n[5] LOYO validation ...")
    best_weights = make_high_w(base_weights, best_v1, best_f168, best_i415f)
    wins = 0
    for yr in YEARS:
...
def _update_config(cfg, bw, new_obj):
    ...
    subprocess.run(["git", "commit", "-m",
        f"quant: {new_ver} — HIGH regime retune r6 (v1={bw['v1']:.2f} f168={bw['f168']:.2f}) OBJ={new_obj:.4f}"],
