"""
Phase 400 — Per-Regime Vol + Disp Overlay Re-Tune [VECTORIZED]
================================================================
Baseline: v2.49.40, OBJ=6.7382 (P354 V1 weight resweep — no change)

After full cycle P350-P354 with P350-P354 P353 validated (regime wts),
restart cycle with vol/disp retune.
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase400"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase400_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 400 — Vol + Disp Overlay Re-Tune [VECTORIZED]")
    print("=" * 70)
    ...
    result = {
        "phase": "355", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    out = Path("artifacts/phase400"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase400_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        old_ver = cfg.get("_version", "2.49.33")
        parts = old_ver.split(".")
        parts[-1] = str(int(parts[-1]) + 1)
        new_ver = ".".join(parts)
        cfg["_version"] = new_ver
        old_vver = cfg.get("version", "v2.49.40").lstrip("v")
        ...
        msg = (f"feat: P400 vol+disp retune OBJ={reported_obj:.4f} "
