"""
Phase 334 — Per-Regime V1 Internal Weight Re-Sweep [VECTORIZED]
================================================================
Baseline: v2.49.39, OBJ=6.5794 (P333 regime weight reopt — no change)

V1 internal weights (carry/mom/mean_reversion) — re-sweep after
P330-P333 confirmed all other params optimal.

Current (from P333 confirmed optimal):
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase334"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase334_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 334 — Per-Regime V1 Internal Weight Re-Sweep")
    print("=" * 70)
    ...
    result = {
        "phase": "334", "title": "Per-Regime V1 Weight Re-Sweep",
    ...
    out = Path("artifacts/phase334"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase334_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase334/phase334_report.json")
    ...
    if validated:
        ...
        old_vver = cfg.get("version", "v2.49.39").lstrip("v")
