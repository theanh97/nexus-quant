"""
Phase 271 — Regime Weight Re-opt (12th Pass) [VECTORIZED]
=========================================================
Baseline: v2.49.0, OBJ=4.3173

P252 (parallel session) improved DISP overlay (Δ=+0.0321, LOYO=3/5).
With updated DISP params, optimal ensemble weights per regime may have shifted.
This phase re-sweeps regime weights from the new baseline.

All params loaded dynamically from config (same pattern as Phase 249).
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase271"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase271_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
def precompute_fast_data(P, all_base_data, all_v1_raw):
    ...
    VOL direction: bv > thr → dampen (same as P262n: high-vol dampening).
    """
    fast_data = {}
    for yr, base_data in sorted(all_base_data.items()):
        ...
        # Pre-scaled signals (overlay fully applied)
        scaled = {
...
def fast_evaluate(regime_weights, fast_data):
    ...
    for yr, fd in sorted(fast_data.items()):
        ...
        for ridx, rname in enumerate(RNAMES):
            ...
            vk = "v1_LOW" if rname == "LOW" else "v1_MID"
...
if __name__ == "__main__":
    t0 = time.time()
    print("=" * 65)
    print("Phase 271 — Regime Weight Re-opt (12th Pass) [VECTORIZED]")
        ...
        def obj_subset(yrs, weights):
            ...
            for y in yrs:
                ...
                for ri, rn in enumerate(RNAMES):
                    ...
                    vk = "v1_LOW" if rn == "LOW" else "v1_MID"
                    ens[m] = (w["v1"]*sc[vk][m] + w["i460"]*sc["i460"][m]
                             + w["i415"]*sc["i415"][m] + w["f168"]*sc["f168"][m])
                r = ens[~np.isnan(ens)]
                ann = float(np.mean(r)/np.std(r,ddof=1)*np.sqrt(8760)) if len(r) > 1 else 0.0
    ...
    if validated:
        ...
        # Use stored config OBJ + delta (not vectorized absolute) to avoid overwriting
        # true OBJ with lower vectorized approximation
        ...
        msg = (f"feat: P271 regime weight reopt12 OBJ={reported_obj:.4f} "
        ...
        print(f"  Current weights confirmed: LOW={P['regime_weights']['LOW']}")
        print(f"                            MID={P['regime_weights']['MID']}")
        print(f"                           HIGH={P['regime_weights']['HIGH']}")
