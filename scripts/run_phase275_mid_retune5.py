#!/usr/bin/env python3
"""
Phase 275 — MID Regime Retune Round 5
Parallel session set MID v1=0.1, f168=0.65 (different from my P271 v1=0.0, f168=0.8).
Re-optimize from current state with updated HIGH (P274).
Sweeps f168, v1, idio_ratio for MID regime.
...
def make_mid_w(base_weights, f168_w, v1_w, idio_ratio):
...
def main():
    ...
    print(f"    Current MID: v1={base_weights['MID']['v1']} i460={base_weights['MID']['i460']} "
          f"i415={base_weights['MID']['i415']} f168={base_weights['MID']['f168']}")
    ...
    print("\n[3] f168 sweep ...")
    best_f168 = base_weights["MID"]["f168"]
    best_obj  = curr_obj
    v1_cur = base_weights["MID"]["v1"]
    iratio = 0.5

    for f168_w in [0.00, 0.10, 0.20, 0.30, 0.40, 0.50, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90]:
        w = make_mid_w(base_weights, f168_w, v1_cur, iratio)
    ...
    print("\n[4] v1 sweep ...")
    best_v1 = v1_cur
    for v1_w in [0.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40]:
        w = make_mid_w(base_weights, best_f168, v1_w, iratio)
        obj = eval_weights(yearly_data, params, w)
        marker = " <-" if obj > best_obj else ""
    ...
    print("\n[5] idio_ratio sweep ...")
    best_ratio = iratio
    for ratio in [0.00, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 1.00]:
        w = make_mid_w(base_weights, best_f168, best_v1, ratio)
        obj = eval_weights(yearly_data, params, w)
        marker = " <-" if obj > best_obj else ""
    ...
    best_w = make_mid_w(base_weights, best_f168, best_v1, best_ratio)
    ...
    print(f"  Best MID: v1={best_mid['v1']:.3f} i460={best_mid['i460']:.3f} "
          f"i415={best_mid['i415']:.3f} f168={best_mid['f168']:.3f}")
    ...
    if wins >= 3 and delta > 0.005:
        ...
        rw2["MID"]["v1"] = best_mid["v1"]
        if "i460bw168" in rw2["MID"]:
            rw2["MID"]["i460bw168"] = best_mid["i460"]
        else:
            rw2["MID"]["i460"] = best_mid["i460"]
        if "i415bw216" in rw2["MID"]:
            rw2["MID"]["i415bw216"] = best_mid["i415"]
        else:
            rw2["MID"]["i415"] = best_mid["i415"]
        rw2["MID"]["f168"] = best_mid["f168"]
        ...
        print(f"  MID: v1={best_mid['v1']:.3f} i460={best_mid['i460']:.3f} "
              f"i415={best_mid['i415']:.3f} f168={best_mid['f168']:.3f}")
        print(f"  New OBJ={best_obj:.4f}")
        msg = (f"quant: {new_ver} — MID retune r5 "
               f"(v1={best_mid['v1']:.2f} f168={best_mid['f168']:.2f}) "
               f"OBJ={best_obj:.4f}")
        ...
        print(f"  MID stays: v1={base_weights['MID']['v1']} i460={base_weights['MID']['i460']} "
              f"i415={base_weights['MID']['i415']} f168={base_weights['MID']['f168']}")

    elapsed = time.time() - t0
    print(f"\n[DONE] Phase 275 complete.  ({elapsed:.1f}s)")
