"""
Phase 285 — Per-Regime FTS Resweep against P284 Baseline [VECTORIZED]
=====================================================================
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase285_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase285_fts_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 285 — FTS Resweep against P284 Baseline [VECTORIZED]")
    ...
    out = Path("artifacts/phase285_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase285_fts_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase285_fts/phase285_fts_report.json")
    ...
    if validated:
        ...
        msg = (f"feat: P285 FTS resweep OBJ={reported_obj} "
