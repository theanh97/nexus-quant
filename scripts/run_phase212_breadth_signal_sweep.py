"""
Phase 212 — Breadth Signal Parameter Sweep (BRD_LB × PCT_WINDOW)
==================================================================
Breadth signal defines regime boundaries. Last swept at P158 (OBJ=2.21).
Now at OBJ=3.09 with very different regime weights (F168-heavy MID).
Optimal BRD_LB and PCT_WINDOW might have shifted.
All signals pre-computed. Rebuild breadth percentile on-the-fly.

Baseline: v2.32.0, OBJ=3.0860
Current: BRD_LB=192, PCT_WINDOW=336
Sweep: brd_lb ∈ [96, 120, 144, 168, 192, 240, 288, 336]
       pct_win ∈ [168, 240, 288, 336, 400, 480]
"""
...
def _on_timeout(signum, frame):
    ...
    (out / "phase212_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
VOL_WINDOW    = 168; PCT_WINDOW_BASE = 336
...
PCT_WIN_SWEEP = [168, 240, 288, 336, 400, 480]
...
def load_year_data(year):
    ...
    for i in range(PCT_WINDOW_BASE, n):
        ts_spread_pct[i] = float(np.mean(ts_raw[i-PCT_WINDOW_BASE:i] <= ts_raw[i]))
    # Pre-compute all raw breadth (for all brd_lb values)
    # Store raw breadth fraction per bar and close_mat for breadth recompute
    sig_rets = {}
...
def ensemble_rets_with_brd(brd_pct, close_mat, btc_vol, fund_std_pct, ts_spread_pct, sig_rets, n):
    RNAMES = ["LOW", "MID", "HIGH"]
...
def evaluate(brd_lb, pct_win, all_data):
    yearly = {}
    for yr, (close_mat, btc_vol, fsp, tsp, sig_rets, n) in all_data.items():
        brd_pct = compute_brd_pct(close_mat, n, brd_lb, pct_win)
        ens = ensemble_rets_with_brd(brd_pct, close_mat, btc_vol, fsp, tsp, sig_rets, n)
...
def main():
    t0 = time.time()
    print("=" * 65)
    print("Phase 212 — Breadth Signal Parameter Sweep")
    print(f"Baseline: v2.32.0  OBJ={BASELINE_OBJ}")
    print(f"Current: brd_lb={BRD_LB_BASE}  pct_win={PCT_WINDOW_BASE}")
    ...
    base_obj, base_yearly = evaluate(BRD_LB_BASE, PCT_WINDOW_BASE, all_data)
    print(f"\n  Baseline OBJ = {base_obj:.4f}  (expected ≈ {BASELINE_OBJ})")
    _partial.update({"baseline_obj": float(base_obj)})

    print("\n[2/3] 2D Sweep...")
    best_obj = base_obj; best_brd_lb = BRD_LB_BASE; best_pct_win = PCT_WINDOW_BASE
    results = []
    for brd_lb in BRD_LB_SWEEP:
        for pct_win in PCT_WIN_SWEEP:
            ...
            print(f"  brd_lb={brd_lb:4d}  pct_win={pct_win:4d}  OBJ={o:.4f}  Δ={delta:+.4f}{marker}")
            results.append((brd_lb, pct_win, o))
            if o > best_obj:
                best_obj = o; best_brd_lb = brd_lb; best_pct_win = pct_win

    print(f"\n  → Best: brd_lb={best_brd_lb}  pct_win={best_pct_win}  OBJ={best_obj:.4f}")

    print("\n[3/3] LOYO validation...")
    joint_obj, joint_yearly = evaluate(best_brd_lb, best_pct_win, all_data)
    ...
    print(f"\n{'✅ VALIDATED' if validated else '❌ NO IMPROVEMENT'} — brd_lb={best_brd_lb}  pct_win={best_pct_win}")

    result = {
        "phase": 212, "baseline_obj": float(base_obj), "best_obj": float(joint_obj),
    ...
    out = Path("artifacts/phase212"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase212_report.json").write_text(json.dumps(result, indent=2))
