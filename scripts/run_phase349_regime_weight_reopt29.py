"""
Phase 349 — Regime Weight Re-opt (29th Pass) [VECTORIZED]
=========================================================
Baseline: v2.49.43, OBJ=6.8295 (after P339 regime weights +0.0363)

After P343 V1 weights confirmed at exact local optimum,
re-sweep ensemble weights to check if regime allocations can improve.
...
if __name__ == "__main__":
    t0 = time.time()
    print("=" * 65)
    print("Phase 349 — Regime Weight Re-opt (29th Pass) [VECTORIZED]")
    print("After P348 V1 weights (no change) — re-sweep ensemble")
    ...
    report = {
        ...
        "context": "After P348 V1 weights (no change) — re-sweep ensemble",
        "runtime_seconds": round(time.time() - t0, 1),
    }
    out = Path("artifacts/phase349"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase349_report.json").write_text(json.dumps(report, indent=2))
    ...
    if validated:
        ...
        msg = (f"feat: P349 regime weight reopt29 OBJ={reported_obj:.4f} "
