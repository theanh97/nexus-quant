"""
Phase 372 — Global I460 Lookback + Beta Window Sweep [VECTORIZED]
==================================================================
Baseline: v2.49.45, OBJ=7.0152 (P370-P371 cycle fully converged)

I460 (idio_momentum_alpha lb=480 bw=168) is the PRIMARY signal for the HIGH
regime (w=0.50). Last swept in P194 (bw=120→168). Never swept lb since P172.

Current I460 params: lb=480, bw=168, k_per_side=4, tgl=0.3, reb=48
Current I415 params: lb=415, bw=144, k_per_side=4, tgl=0.3, reb=48

Approach:
  Pass 1: Sweep I460 lb ∈ [360, 420, 480, 540, 600] × bw ∈ [120, 144, 168, 192, 240]
           = 25 combos
  Pass 2: Refine around winner
  Then: Sweep I415 lb ∈ [360, 415, 480] × bw ∈ [120, 144, 168]
         = 9 combos

Note: These are GLOBAL params (same for all regimes), so the sweep
      re-runs the full 5-year backtest for each combination.
"""
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase372"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase372_report.json").write_text(json.dumps(_partial, indent=2))
    print("\n⏰ TIMEOUT — partial saved.", flush=True)
    sys.exit(0)
_signal.signal(_signal.SIGALRM, _on_timeout)
_signal.alarm(14400)  # 4h timeout
...
# I460 sweep grid
I460_LB_GRID = [360, 420, 480, 540, 600]
I460_BW_GRID = [120, 144, 168, 192, 240]
# I415 sweep grid
I415_LB_GRID = [360, 415, 480]
I415_BW_GRID = [120, 144, 168]
...
_i460_cache: dict = {}  # (year, lb, bw) → returns array
_i415_cache: dict = {}
...
def get_i460(year, dataset, lb, bw):
    key = (year, lb, bw)
    if key not in _i460_cache:
        _i460_cache[key] = run_signal(dataset, "idio_momentum_alpha",
            {"k_per_side": 4, "lookback_bars": lb, "beta_window_bars": bw,
             "target_gross_leverage": 0.3, "rebalance_interval_bars": 48})
    return _i460_cache[key]


def get_i415(year, dataset, lb, bw):
    key = (year, lb, bw)
    if key not in _i415_cache:
        _i415_cache[key] = run_signal(dataset, "idio_momentum_alpha",
            {"k_per_side": 4, "lookback_bars": lb, "beta_window_bars": bw,
             "target_gross_leverage": 0.3, "rebalance_interval_bars": 48})
    return _i415_cache[key]


def eval_ensemble(p, all_base, datasets, v1_by_year, f168_by_year, i460_lb, i460_bw, i415_lb, i415_bw):
    yearly = {}
    for yr in sorted(YEAR_RANGES):
        btc_vol, fund_std_pct, ts_raw, brd_pct, fixed_n = all_base[yr]
        n = fixed_n
        i460_ret = get_i460(yr, datasets[yr], i460_lb, i460_bw)
        i415_ret = get_i415(yr, datasets[yr], i415_lb, i415_bw)
        f168_ret = f168_by_year[yr]
        v1_ret = v1_by_year[yr]  # dict: {rn: array}
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 372 — Global I460/I415 Lookback + Beta Window Sweep")
    print("=" * 70)
    ...
    for rn in RNAMES:
        w = p["regime_weights"][rn]
        print(f"  {rn}: v1={w['v1']} i460={w['i460']} i415={w['i415']} f168={w['f168']}")

    print(f"\nI460 sweep: lb={I460_LB_GRID} × bw={I460_BW_GRID} = {len(I460_LB_GRID)*len(I460_BW_GRID)} combos")
    print(f"I415 sweep: lb={I415_LB_GRID} × bw={I415_BW_GRID} = {len(I415_LB_GRID)*len(I415_BW_GRID)} combos")
    ...
    print("\n[2] Pre-computing V1 + F168...", flush=True)
    v1_by_year = {}
    f168_by_year = {}
    for yr in sorted(YEAR_RANGES):
        ds = datasets[yr]
        ...
        f168_by_year[yr] = run_signal(ds, "funding_momentum_alpha",
            {"k_per_side": 2, "funding_lookback_bars": 168, "direction": "contrarian",
             "target_gross_leverage": 0.25, "rebalance_interval_bars": 36})
        print(f"  {yr}: done.", flush=True)

    # Compute baseline (current I460 lb=480 bw=168, I415 lb=415 bw=144)
    print("\n[3] Baseline evaluation...", flush=True)
    base_obj, base_yr = eval_ensemble(p, all_base, datasets, v1_by_year, f168_by_year,
                                       480, 168, 415, 144)
    ...
    # Current best
    best_i460_lb = 480; best_i460_bw = 168
    best_i415_lb = 415; best_i415_bw = 144
    best_obj = base_obj

    print("\n[4] I460 parameter sweep...", flush=True)
    for lb in I460_LB_GRID:
        for bw in I460_BW_GRID:
            obj, yr_vals = eval_ensemble(p, all_base, datasets, v1_by_year, f168_by_year,
                                          lb, bw, best_i415_lb, best_i415_bw)
            if obj > best_obj:
                best_obj = obj
                best_i460_lb = lb
                best_i460_bw = bw
                print(f"  I460 NEW BEST: lb={lb} bw={bw}  OBJ={obj:.4f}  Δ={obj-base_obj:+.4f}")
    print(f"  I460 sweep done. Best: lb={best_i460_lb} bw={best_i460_bw}  OBJ={best_obj:.4f}  Δ={best_obj-base_obj:+.4f}")
    _partial["best_i460"] = {"lb": best_i460_lb, "bw": best_i460_bw, "obj": float(best_obj)}

    print("\n[5] I415 parameter sweep...", flush=True)
    for lb in I415_LB_GRID:
        for bw in I415_BW_GRID:
            obj, yr_vals = eval_ensemble(p, all_base, datasets, v1_by_year, f168_by_year,
                                          best_i460_lb, best_i460_bw, lb, bw)
            if obj > best_obj:
                best_obj = obj
                best_i415_lb = lb
                best_i415_bw = bw
                print(f"  I415 NEW BEST: lb={lb} bw={bw}  OBJ={obj:.4f}  Δ={obj-base_obj:+.4f}")
    print(f"  I415 sweep done. Best: lb={best_i415_lb} bw={best_i415_bw}  OBJ={best_obj:.4f}  Δ={best_obj-base_obj:+.4f}")
    _partial["best_i415"] = {"lb": best_i415_lb, "bw": best_i415_bw, "obj": float(best_obj)}

    # Final LOYO validation
    print("\n[6] Final LOYO validation...", flush=True)
    final_obj, final_yr = eval_ensemble(p, all_base, datasets, v1_by_year, f168_by_year,
                                         best_i460_lb, best_i460_bw, best_i415_lb, best_i415_bw)
    ...
    print(f"\n  Changes:")
    print(f"    I460: lb={480}→{best_i460_lb}  bw={168}→{best_i460_bw}"
          f"{'  ← CHANGED' if best_i460_lb != 480 or best_i460_bw != 168 else '  (unchanged)'}")
    print(f"    I415: lb={415}→{best_i415_lb}  bw={144}→{best_i415_bw}"
          f"{'  ← CHANGED' if best_i415_lb != 415 or best_i415_bw != 144 else '  (unchanged)'}")

    result = {
        "phase": "372", "title": "Global I460/I415 LB+BW Sweep",
        ...
        "best_i460_lb": best_i460_lb, "best_i460_bw": best_i460_bw,
        "best_i415_lb": best_i415_lb, "best_i415_bw": best_i415_bw,
        "per_year_baseline": {str(k): v for k, v in base_yr.items()},
        "per_year_final": {str(k): v for k, v in final_yr.items()},
    }
    _partial.update(result)
    out = Path("artifacts/phase372"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase372_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase372/phase372_report.json")

    if validated:
        cfg = json.load(open(CFG_PATH))
        # Update I460 and I415 params in config
        # Store in a new section
        cfg.setdefault("signal_params", {})
        cfg["signal_params"]["i460"] = {
            "lookback_bars": best_i460_lb, "beta_window_bars": best_i460_bw,
            "k_per_side": 4, "target_gross_leverage": 0.3, "rebalance_interval_bars": 48
        }
        cfg["signal_params"]["i415"] = {
            "lookback_bars": best_i415_lb, "beta_window_bars": best_i415_bw,
            "k_per_side": 4, "target_gross_leverage": 0.3, "rebalance_interval_bars": 48
        }
        ...
        msg = (f"feat: P372 I460/I415 lb+bw sweep OBJ={reported_obj:.4f} "
    ...
    print(f"\nI460 cache: {len(_i460_cache)} | I415 cache: {len(_i415_cache)}")
