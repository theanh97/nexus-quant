"""
Phase 250 — Per-Regime FTS ts_pct_win Re-Sweep
================================================
Baseline: v2.47.0, OBJ=4.2699

Phase 248 changed FTS RS dramatically (0.50 → 0.05 for LOW/MID).
When RS≈0 (near-total dampening in contango), the threshold RT=0.80
means: when FTS percentile > 80th → dampen to 5%.
The ts_pct_win (lookback for computing FTS percentile) likely needs re-tuning.
Shorter window → more sensitive/reactive percentile; longer → more stable.

No new strategy runs needed — all signal arrays pre-computed.
Only ts_pct_win changes → very fast sweep (all numpy).
"""
...
VOL_SCALE = {"LOW": 0.40, "MID": 0.50, "HIGH": 0.35}
...
# ts_pct_win sweep — wider range given RS changed
TS_PCT_WIN_SWEEP = [120, 168, 216, 240, 288, 336, 400, 480, 600, 720]
...
def build_fast_data_for_wins(ts_pct_wins, all_base_data, all_v1_raw):
    """
    ts_pct_wins: {"LOW": w1, "MID": w2, "HIGH": w3}
    Builds fast eval data with given ts_pct_win per regime.
    """
    fast_data = {}
    for yr, base_data in sorted(all_base_data.items()):
        ...
        # Pre-compute ts_pct for each unique window needed
...
def main():
    t0 = time.time()
    print("=" * 65)
    print("Phase 250 — Per-Regime FTS ts_pct_win Re-Sweep")
    ...
    # Baseline check
    base_wins = {"LOW": 240, "MID": 288, "HIGH": 400}
    base_fast_data = build_fast_data_for_wins(base_wins, all_base_data, all_v1_raw)
    ...
    if validated:
        ...
        cur_ver = cfg.get("_version", "2.47.0")
        parts = cur_ver.split(".")
