"""
Phase 303 — Per-Regime V1 Internal Weights Re-Sweep [VECTORIZED, MEM-EFFICIENT]
================================================================================
Baseline: v2.49.25, OBJ=5.8110 (P298-P302 cycle complete)

New cycle P303-P307 after P298-P302 advanced OBJ 5.6221→5.8110. V1 internals
may have shifted after regime weights (P299), vol+disp (P300-ext).

Current V1 weights:
  LOW:  wc=0.10, wm=0.40, wmr=0.50  (mean-reversion heavy)
  MID:  wc=0.40, wm=0.40, wmr=0.20  (carry+momentum)
  HIGH: wc=0.50, wm=0.40, wmr=0.10  (carry-heavy)
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase303"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase303_report.json").write_text(json.dumps(_partial, indent=2))
...
if __name__ == "__main__":
    t0 = time.time()
    print("=" * 70)
    print("Phase 303 — Per-Regime V1 Weights Re-Sweep [MEM-EFFICIENT]")
    print("Baseline: v2.49.25 OBJ=5.8110 (P298-P302 cycle)")
    print("=" * 70)
    ...
    result = {
        "phase": "303", "baseline_obj": float(obj_base), "stored_baseline": float(base_obj),
    ...
    out = Path("artifacts/phase303"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase303_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        msg = (f"feat: P303 per-regime V1 weights OBJ={reported_obj:.4f} "
