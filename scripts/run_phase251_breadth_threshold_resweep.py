#!/usr/bin/env python3
"""
Phase 251 — Breadth Threshold Re-sweep
Current: p_low=0.30, p_high=0.60
Sweep: p_low ∈ [0.20..0.40], p_high ∈ [0.50..0.75]
"""
...
def main():
    t0 = time.time()
    print("=" * 60)
    print("Phase 251 — Breadth Threshold Re-sweep")
    print("=" * 60)

    cfg, weights, params, v1p, i460p, i415p, f168p = get_config()
    print(f"  Current: p_low={params['p_low']} p_high={params['p_high']}")
    ...
    # Part A: p_high sweep
    print("\n[2] Part A: p_high sweep (p_low fixed=0.30) ...")
    ph_cands = [0.45, 0.50, 0.53, 0.55, 0.57, 0.60, 0.62, 0.65, 0.68, 0.70, 0.75]
    best_ph = params["p_high"]
    for ph in ph_cands:
        if ph <= best_p["p_low"]: continue
        o = eval_params(year_data, p_with(best_p, p_high=ph), weights)
        marker = " <-" if o > best_obj + 1e-6 else ""
        print(f"  p_high={ph:.2f}  OBJ={o:.4f}{marker}")
        if o > best_obj + 1e-6:
            best_obj = o; best_ph = ph; best_p = p_with(best_p, p_high=ph)
    print(f"  Best p_high: {best_ph}  OBJ={best_obj:.4f}")

    # Part B: p_low sweep
    print(f"\n[3] Part B: p_low sweep (p_high={best_ph}) ...")
    pl_cands = [0.15, 0.18, 0.20, 0.22, 0.25, 0.28, 0.30, 0.33, 0.35, 0.38, 0.40]
    best_pl = best_p["p_low"]
    for pl in pl_cands:
        if pl >= best_ph: continue
        o = eval_params(year_data, p_with(best_p, p_low=pl), weights)
        marker = " <-" if o > best_obj + 1e-6 else ""
        print(f"  p_low={pl:.2f}  OBJ={o:.4f}{marker}")
        if o > best_obj + 1e-6:
            best_obj = o; best_pl = pl; best_p = p_with(best_p, p_low=pl)
    delta = best_obj - base_obj
    print(f"  Best p_low: {best_pl}  OBJ={best_obj:.4f}  D={delta:+.4f}")
    ...
    print(f"  Best thresholds: p_low={best_p['p_low']} p_high={best_p['p_high']}")

    if loyo_wins >= 3 and delta > 0.005:
        print("\n  VALIDATED — updating config ...")
        cfg["breadth_regime_switching"]["p_low"]  = best_p["p_low"]
        cfg["breadth_regime_switching"]["p_high"] = best_p["p_high"]
        ...
        try:
            ...
            msg = (f"feat: {new_ver} Phase251 breadth thresholds — "
                   f"p_low={best_p['p_low']} p_high={best_p['p_high']} "
