#!/usr/bin/env python3
"""
Phase 252 — F168 Signal Lookback Sweep
Current: funding_lookback_bars=168
Sweep: lb ∈ [96, 120, 144, 168, 192, 216, 240, 288, 336]
"""
...
def load_year_with_f168lb(year, params, v1p, i460p, i415p, f168p, f168_lb):
    """Load year data with a specific f168 lookback_bars."""
    p = params
...
def main():
    t0 = time.time()
    print("=" * 60)
    print("Phase 252 — F168 Signal Lookback Sweep")
    print("=" * 60)

    cfg, weights, params, v1p, i460p, i415p, f168p = get_config()
    cur_lb = f168p.get("funding_lookback_bars", 168)
    print(f"  Current f168 lookback_bars={cur_lb}")

    LB_CANDS = [96, 120, 144, 168, 192, 216, 240, 288, 336]

    print("\n[1] Sweeping f168 lookback_bars ...")
    baseline_data = {}
    best_obj = None; base_obj = None
    results = []

    for lb in LB_CANDS:
        sharpes = []
        for yr in YEARS:
            d = load_year_with_f168lb(yr, params, v1p, i460p, i415p, f168p, lb)
            s = yearly_sharpe(d, params, weights)
            sharpes.append(s)
            if lb == cur_lb:
                baseline_data[yr] = d
        arr = np.array(sharpes)
        obj = float(np.mean(arr)) - 0.5 * float(np.std(arr))
        results.append((lb, obj, sharpes))
        if lb == cur_lb: base_obj = obj

    # find best
    best_lb = cur_lb
    best_obj = base_obj
    for lb, obj, _ in results:
        print(f"  lb={lb:3d}  OBJ={obj:.4f}")
        if obj > best_obj + 1e-6:
            best_obj = obj; best_lb = lb

    print(f"\n  Baseline lb={cur_lb}  OBJ={base_obj:.4f}")
    print(f"  Best lb:  {best_lb}  OBJ={best_obj:.4f}")
    delta = best_obj - base_obj
    print(f"  D={delta:+.4f}")

    # LOYO
    best_r = next(r for lb, _, r in results if lb == best_lb)
    base_r = next(r for lb, _, r in results if lb == cur_lb)
    print(f"\n[2] LOYO validation ...")
    loyo_wins = 0
    for i, yr in enumerate(YEARS):
        result = "WIN " if best_r[i] > base_r[i] else "LOSS"
        if best_r[i] > base_r[i]: loyo_wins += 1
        print(f"  {yr}: best={best_r[i]:.4f} curr={base_r[i]:.4f} {result}")

    print(f"\n  LOYO: {loyo_wins}/5 wins  delta={delta:+.4f}")
    print(f"  Best f168_lb: {best_lb}")

    if loyo_wins >= 3 and delta > 0.005:
        print("\n  VALIDATED — updating config ...")
        sig_key = "f144" if "f144" in cfg["ensemble"]["signals"] else "f168"
        cfg["ensemble"]["signals"][sig_key]["params"]["funding_lookback_bars"] = best_lb
        ...
        try:
            ...
            msg = (f"feat: {new_ver} Phase252 f168_lb={best_lb} — "
