#!/usr/bin/env python3
"""
Phase 248 — FTS Overlay Re-tune (post P247 HIGH update)
Current: ts_rt=0.55, ts_rs=0.20, ts_bt=0.25, ts_bs=2.30
Sweep: A) ts_rt, B) ts_rs, C) ts_bt, D) ts_bs
"""
...
def main():
    t0 = time.time()
    print("=" * 60)
    print("Phase 248 — FTS Overlay Re-tune")
    print("=" * 60)
    ...
    # Part A: ts_rt sweep
    print("\n[2] Part A: ts_rt sweep ...")
    rt_cands = [0.45, 0.50, 0.52, 0.55, 0.57, 0.60, 0.62, 0.65, 0.70]
    ...
    for rt in rt_cands:
        ...
        if o > best_obj + 1e-6:
            best_obj = o; best_rt = rt; best_p = p_with(best_p, ts_rt=rt)
    print(f"  Best ts_rt: {best_rt}  OBJ={best_obj:.4f}")

    # Part B: ts_rs sweep
    print(f"\n[3] Part B: ts_rs sweep (ts_rt={best_rt}) ...")
    rs_cands = [0.05, 0.10, 0.12, 0.15, 0.17, 0.20, 0.22, 0.25, 0.30]
    best_rs = best_p["ts_rs"]
    for rs in rs_cands:
        o = eval_params(year_data, p_with(best_p, ts_rs=rs), weights)
        marker = " <-" if o > best_obj + 1e-6 else ""
        print(f"  ts_rs={rs:.2f}  OBJ={o:.4f}{marker}")
        if o > best_obj + 1e-6:
            best_obj = o; best_rs = rs; best_p = p_with(best_p, ts_rs=rs)
    print(f"  Best ts_rs: {best_rs}  OBJ={best_obj:.4f}")

    # Part C: ts_bt sweep
    print(f"\n[4] Part C: ts_bt sweep ...")
    bt_cands = [0.15, 0.18, 0.20, 0.22, 0.25, 0.28, 0.30, 0.33, 0.35]
    best_bt = best_p["ts_bt"]
    for bt in bt_cands:
        o = eval_params(year_data, p_with(best_p, ts_bt=bt), weights)
        marker = " <-" if o > best_obj + 1e-6 else ""
        print(f"  ts_bt={bt:.2f}  OBJ={o:.4f}{marker}")
        if o > best_obj + 1e-6:
            best_obj = o; best_bt = bt; best_p = p_with(best_p, ts_bt=bt)
    print(f"  Best ts_bt: {best_bt}  OBJ={best_obj:.4f}")

    # Part D: ts_bs sweep
    print(f"\n[5] Part D: ts_bs sweep ...")
    bs_cands = [1.50, 1.75, 2.00, 2.10, 2.20, 2.30, 2.40, 2.50, 2.75, 3.00]
    best_bs = best_p["ts_bs"]
    for bs in bs_cands:
        o = eval_params(year_data, p_with(best_p, ts_bs=bs), weights)
        marker = " <-" if o > best_obj + 1e-6 else ""
        print(f"  ts_bs={bs:.2f}  OBJ={o:.4f}{marker}")
        if o > best_obj + 1e-6:
            best_obj = o; best_bs = bs; best_p = p_with(best_p, ts_bs=bs)
    delta = best_obj - base_obj
    print(f"  Best ts_bs: {best_bs}  OBJ={best_obj:.4f}  D={delta:+.4f}")
    ...
    print(f"  Best FTS: rt={best_p['ts_rt']} rs={best_p['ts_rs']} "
          f"bt={best_p['ts_bt']} bs={best_p['ts_bs']}")
    ...
    if loyo_wins >= 3 and delta > 0.005:
        ...
        old_ver = cfg.get("version", "v2.42.5")
        ...
        try:
            ...
            msg = (f"feat: {new_ver} Phase248 FTS retune — "
                   f"rt={best_p['ts_rt']} rs={best_p['ts_rs']} "
                   f"bt={best_p['ts_bt']} bs={best_p['ts_bs']} "
