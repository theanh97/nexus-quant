"""
Phase 349 — Per-Regime V1 Internal Weight Re-Sweep [VECTORIZED]
================================================================
Baseline: v2.49.40, OBJ=6.7382 (P348 regime weight reopt — no change)

V1 internal weights (carry/mom/mean_reversion) — re-sweep after
P345-P348 confirmed all other params optimal.

Current (from P348 confirmed optimal):
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase349"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase349_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 349 — Per-Regime V1 Internal Weight Re-Sweep")
    print("=" * 70)
    ...
    result = {
        "phase": "349", "title": "Per-Regime V1 Weight Re-Sweep",
    ...
    out = Path("artifacts/phase349"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase349_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase349/phase349_report.json")
