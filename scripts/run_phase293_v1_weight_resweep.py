"""
Phase 293 — Per-Regime V1 Internal Weight Re-Sweep [VECTORIZED]
================================================================
Baseline: v2.49.17, OBJ=5.4871 (Phase 292 regime weight reopt17)

V1 internal weights (carry/mom/mean_reversion) last optimized in P287.
Since then: P289 vol/disp, P290 FTS, P291 breadth boundary, P292 regime weights.
The optimal V1 signal composition may have shifted. Re-sweep per regime.

Current (P287):
  LOW:  wc=0.15, wm=0.45, wmr=0.40
  MID:  wc=0.45, wm=0.45, wmr=0.10
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 293 — Per-Regime V1 Internal Weight Re-Sweep")
    print("Baseline: v2.49.17, OBJ=5.4871")
    print("=" * 70)
    ...
    # [7] Save report
    result = {
        "phase": "293", "title": "Per-Regime V1 Weight Re-Sweep",
    ...
    print(f"\n  Report → artifacts/phase293/phase293_report.json")
    ...
    if validated:
        ...
        old_ver = cfg.get("_version", "2.49.17")
        parts = old_ver.split(".")
        parts[-1] = str(int(parts[-1]) + 1)
        new_ver = ".".join(parts)
        cfg["_version"] = new_ver

        old_vver = cfg.get("version", "v2.49.17").lstrip("v")
        ...
        msg = (f"feat: P293 V1 weight resweep OBJ={reported_obj:.4f} "
