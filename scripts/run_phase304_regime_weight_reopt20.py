"""
Phase 304 — Regime Weight Re-opt (20th Pass) [VECTORIZED]
=========================================================
Baseline: v2.49.27, OBJ=5.9352 (after P303 V1 weights — no change)

After P303 confirmed V1 internal weights at exact local optimum,
...
if __name__ == "__main__":
    t0 = time.time()
    print("=" * 65)
    print("Phase 304 — Regime Weight Re-opt (20th Pass) [VECTORIZED]")
    print("After P303 V1 weights (no change) — re-sweep ensemble")
    ...
    report = {
        ...
        "context": "After P303 V1 weights (no change) — re-sweep ensemble",
        "runtime_seconds": round(time.time() - t0, 1),
    }
    out = Path("artifacts/phase304"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase304_report.json").write_text(json.dumps(report, indent=2))
    ...
    if validated:
        ...
        msg = (f"feat: P304 regime weight reopt20 OBJ={reported_obj:.4f} "
