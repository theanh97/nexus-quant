"""
Phase 386 — Per-Regime FTS Retune [VECTORIZED]
============================================================================
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase386_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase386_fts_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 386 — FTS Retune [VECTORIZED]")
    print("=" * 70)
    ...
    out = Path("artifacts/phase386_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase386_fts_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase386_fts/phase386_fts_report.json")
    ...
    if validated:
        ...
        msg = (f"feat: P386 FTS retune OBJ={reported_obj} "
