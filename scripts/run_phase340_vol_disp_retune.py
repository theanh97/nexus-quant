"""
Phase 340 — Per-Regime Vol + Disp Overlay Re-Tune [VECTORIZED]
================================================================
Baseline: v2.49.40, OBJ=6.7382 (P339 V1 weight resweep — no change)

After full cycle P335-P339 with P335-P339 full convergence,
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase340"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase340_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 340 — Vol + Disp Overlay Re-Tune [VECTORIZED]")
    print("=" * 70)
    ...
    result = {
        "phase": "340", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    out = Path("artifacts/phase340"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase340_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        msg = (f"feat: P340 vol+disp retune OBJ={reported_obj:.4f} "
