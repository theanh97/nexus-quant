"""
Phase 308 — Per-Regime V1 Internal Weights Re-Sweep [VECTORIZED, MEM-EFFICIENT]
================================================================================
Baseline: v2.49.29, OBJ=5.9744 (P303-P307 cycle complete)

New cycle P308-P312 after P303-P307 advanced OBJ 5.9352→5.9744. V1 internals
may have shifted after regime weights (P304), vol+disp (P307).
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase308"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase308_report.json").write_text(json.dumps(_partial, indent=2))
...
if __name__ == "__main__":
    t0 = time.time()
    print("=" * 70)
    print("Phase 308 — Per-Regime V1 Weights Re-Sweep [MEM-EFFICIENT]")
    print("Baseline: v2.49.29 OBJ=5.9744 (P303-P307 cycle)")
    print("=" * 70)
    ...
    result = {
        "phase": "308", "baseline_obj": float(obj_base), "stored_baseline": float(base_obj),
    ...
    out = Path("artifacts/phase308"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase308_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        msg = (f"feat: P308 per-regime V1 weights OBJ={reported_obj:.4f} "
