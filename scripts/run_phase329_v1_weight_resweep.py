"""
Phase 329 — Per-Regime V1 Internal Weight Re-Sweep [VECTORIZED]
================================================================
Baseline: v2.49.37, OBJ=6.4749 (P328 regime weight reopt VALIDATED)

V1 internal weights (carry/mom/mean_reversion) — re-sweep after
P325-P328 confirmed all other params optimal.

Current (from P328 regime weight reopt):
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase329"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase329_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 329 — Per-Regime V1 Internal Weight Re-Sweep")
    print("=" * 70)
    ...
    result = {
        "phase": "329", "title": "Per-Regime V1 Weight Re-Sweep",
    ...
    out = Path("artifacts/phase329"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase329_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase329/phase329_report.json")
    ...
    if validated:
        ...
        msg = (f"feat: P319 V1 weight resweep OBJ={reported_obj:.4f} "
