"""
Phase 297 — Breadth Boundary Re-Tune (p_low / p_high)
=======================================================
Baseline: v2.49.21, OBJ=5.6221 (Phase 295 vol+disp retune, LOYO=4/5)

After P293-P295 (V1 weights, regime weights, overlays all changed),
re-sweep the breadth regime boundaries. Current: p_low=0.30, p_high=0.60.
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase297"); out.mkdir(parents=True, exist_ok=True)
...
# Finer grid around current p_low=0.30 p_high=0.62
P_LOW_SWEEP  = [0.20, 0.22, 0.25, 0.27, 0.28, 0.30, 0.32, 0.33, 0.35, 0.38, 0.40]
P_HIGH_SWEEP = [0.50, 0.55, 0.58, 0.60, 0.62, 0.64, 0.65, 0.68, 0.70, 0.75]
...
if __name__ == "__main__":
    t0 = time.time()
    print("=" * 65)
    print("Phase 297 — Regime Boundary Re-Sweep (p_low / p_high)")
    ...
    if validated:
        ...
        old_ver = cfg.get("_version", "2.49.15")
        parts = old_ver.split(".")
        ...
        msg = (f"feat: P297 boundary resweep p_low={best_plow:.2f} p_high={best_phigh:.2f} "
