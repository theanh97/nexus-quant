#!/usr/bin/env python3
"""
Phase 263 — HIGH Regime Re-retune Round 4
Current config has HIGH: f168=0.0 v1=0.2 i460=0.8 i415=0.0 (from parallel P260 session)
Those weights were optimized under P_HIGH=0.60, but current config has p_high=0.68.
Re-validate + re-optimize HIGH under current p_high=0.68.
...
def main():
    t0 = time.time()
    print("=" * 60)
    print("Phase 263 — HIGH Regime Re-retune Round 4")
    print("Re-validating under current p_high=0.68")
    print("=" * 60)
    ...
    curr_idio = curr_high["i460"] + curr_high["i415"]
    curr_ratio = curr_high["i460"] / curr_idio if curr_idio > 0 else 0.5

    print(f"\n  Config: {cfg.get('version')}")
    print(f"  p_high: {params['p_high']:.2f}")
    print(f"  Current HIGH: v1={curr_high['v1']:.3f} i460={curr_high['i460']:.3f} "
          f"i415={curr_high['i415']:.3f} f168={curr_high['f168']:.3f}")
    ...
    for yr, sh in zip(YEARS, baseline_sharpes):
        print(f"    {yr}: Sharpe={sh:.4f}")

    # [2] f168 sweep
    print("\n[2] f168 sweep ...")
    f168_candidates = [0.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50]
    best_f168 = curr_high["f168"]; best_f168_obj = baseline_obj
    for fw in f168_candidates:
        w_test = make_high_w(base_weights, fw, curr_high["v1"], curr_ratio)
    ...
    # [3] v1 sweep
    print("\n[3] v1 sweep ...")
    v1_candidates = [0.05, 0.08, 0.10, 0.12, 0.15, 0.18, 0.20, 0.22, 0.25, 0.28, 0.30, 0.35]
    best_v1 = curr_high["v1"]; best_v1_obj = best_f168_obj
    for vw in v1_candidates:
        if vw + best_f168 >= 1.0: continue
        w_test = make_high_w(base_weights, best_f168, vw, curr_ratio)
        obj = eval_weights(year_data, params, w_test)
    ...
    # [4] idio_ratio sweep
    print("\n[4] idio_ratio sweep ...")
    ratio_candidates = [0.10, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50,
                        0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.90]
    best_ratio = curr_ratio; best_ratio_obj = best_v1_obj
    for ratio in ratio_candidates:
        if best_v1 + best_f168 >= 1.0: continue
        w_test = make_high_w(base_weights, best_f168, best_v1, ratio)
        obj = eval_weights(year_data, params, w_test)
        marker = " <-" if obj > best_ratio_obj else ""
        print(f"  ratio={ratio:.2f}  OBJ={obj:.4f}{marker}")
        if obj > best_ratio_obj:
            best_ratio_obj = obj; best_ratio = ratio
    print(f"  Best idio_ratio: {best_ratio:.2f}  OBJ={best_ratio_obj:.4f}  D={best_ratio_obj-baseline_obj:+.4f}")

    # [5] LOYO validation
    print("\n[5] LOYO validation ...")
    best_weights = make_high_w(base_weights, best_f168, best_v1, best_ratio)
    wins = 0
    for yr in YEARS:
    ...
    print(f"  Best HIGH: v1={bw['v1']:.3f} i460={bw['i460']:.3f} i415={bw['i415']:.3f} f168={bw['f168']:.3f}")

    if wins >= 3 and delta > 0.005:
        print(f"\n  VALIDATED  (LOYO {wins}/5 delta={delta:+.4f})")
        _update_config(cfg, bw, best_ratio_obj)
    else:
...
def _update_config(cfg, bw, new_obj):
    ...
    subprocess.run(["git", "commit", "-m",
        f"quant: {new_ver} — HIGH retune r4 (v1={bw['v1']:.2f} f168={bw['f168']:.2f}) OBJ={new_obj:.4f}"],
