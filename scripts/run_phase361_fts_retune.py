"""
Phase 361 — Per-Regime FTS Retune [VECTORIZED]
============================================================================
Baseline: v2.49.40, OBJ=6.7382 (P355 vol/disp retune — no change)
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase361_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase361_fts_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 361 — FTS Retune [VECTORIZED]")
    print("=" * 70)
    ...
    out = Path("artifacts/phase361_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase361_fts_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase361_fts/phase361_fts_report.json")
    ...
    if validated:
        ...
        msg = (f"feat: P361 FTS retune OBJ={reported_obj} "
