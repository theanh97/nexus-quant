#!/usr/bin/env python3
"""Phase 278 — HIGH Regime Re-retune Round 11.
Parallel set HIGH v1=0.10, i415=0.0, f168=0.37 and p_high=0.60.
Re-optimize HIGH weights under current params.
"""
...
def get_config():
    ...
    params = {
        ...
        "vol_thr": vol_ov.get("threshold", 0.53), "vol_scale": vol_ov.get("scale_factor", 0.40),
    }
...
def load_year_all(year, params, v1p, i460p, i415p, f168p):
    ...
    print(f"  n={n}")
    return {"n": n, "btc_vol": btc_vol, "ts_spread_pct": ts_spread_pct, "brd_pct": brd_pct, "v1": v1_r, "i460": i460_r, "i415": i415_r, "f168": f168_r}
...
def make_high_w(base_weights, f168_w, v1_w, idio_ratio):
    ...
    return {"LOW": base_weights["LOW"], "MID": base_weights["MID"],
            "HIGH": {"v1": v1_w, "i460": round(remainder * idio_ratio, 4), "i415": round(remainder * (1 - idio_ratio), 4), "f168": f168_w}}
...
def main():
    ...
    print(f"    p_high={params['p_high']} vol_thr={params['vol_thr']}")
    print(f"    Current HIGH: v1={base_weights['HIGH']['v1']} i460={base_weights['HIGH']['i460']} i415={base_weights['HIGH']['i415']} f168={base_weights['HIGH']['f168']}")
    ...
    best_f168 = base_weights["HIGH"]["f168"]; best_obj = curr_obj; iratio = 0.40
    best_v1 = base_weights["HIGH"]["v1"]; best_ratio = iratio

    print("\n[3] f168 sweep ...")
    for f168_w in [0.00, 0.10, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50]:
        w = make_high_w(base_weights, f168_w, base_weights["HIGH"]["v1"], iratio)
        obj = eval_weights(yearly_data, params, w)
        marker = " <-" if obj > best_obj else ""
        print(f"  f168={f168_w:.2f}  OBJ={obj:.4f}{marker}", flush=True)
        if obj > best_obj: best_obj = obj; best_f168 = f168_w
    print(f"  Best f168: {best_f168:.2f}  OBJ={best_obj:.4f}  D={best_obj-curr_obj:+.4f}")

    print("\n[4] v1 sweep ...")
    for v1_w in [0.05, 0.10, 0.15, 0.18, 0.20, 0.22, 0.25, 0.30, 0.35, 0.40, 0.50]:
        w = make_high_w(base_weights, best_f168, v1_w, iratio)
        obj = eval_weights(yearly_data, params, w)
        marker = " <-" if obj > best_obj else ""
        print(f"  v1={v1_w:.2f}  OBJ={obj:.4f}{marker}", flush=True)
        if obj > best_obj: best_obj = obj; best_v1 = v1_w
    print(f"  Best v1: {best_v1:.2f}  OBJ={best_obj:.4f}  D={best_obj-curr_obj:+.4f}")

    print("\n[5] idio_ratio sweep ...")
    for ratio in [0.20, 0.30, 0.35, 0.40, 0.45, 0.50, 0.60, 0.70, 0.80, 1.00]:
        w = make_high_w(base_weights, best_f168, best_v1, ratio)
        obj = eval_weights(yearly_data, params, w)
        marker = " <-" if obj > best_obj else ""
        print(f"  ratio={ratio:.2f}  OBJ={obj:.4f}{marker}", flush=True)
        if obj > best_obj: best_obj = obj; best_ratio = ratio
    print(f"  Best idio_ratio: {best_ratio:.2f}  OBJ={best_obj:.4f}  D={best_obj-curr_obj:+.4f}")
    ...
    print(f"  Best HIGH: v1={bh['v1']:.3f} i460={bh['i460']:.3f} i415={bh['i415']:.3f} f168={bh['f168']:.3f}")
    ...
    if wins >= 3 and delta > 0.005:
        ...
        rw2["HIGH"]["f168"] = bh["f168"]; cfg2["version"] = new_ver
        p = Path("configs/production_p91b_champion.json")
        p.write_text(json.dumps(cfg2, indent=2))
        print(f"  Config updated: {old_ver} → {new_ver}")
        print(f"  HIGH: v1={bh['v1']:.3f} i460={bh['i460']:.3f} i415={bh['i415']:.3f} f168={bh['f168']:.3f}")
        print(f"  New OBJ={best_obj:.4f}")
        msg = f"quant: {new_ver} — HIGH retune r11 (v1={bh['v1']:.2f} f168={bh['f168']:.2f}) OBJ={best_obj:.4f}"
    ...
    print(f"\n[DONE] Phase 278 complete.  ({elapsed:.1f}s)")
