"""
Phase 300 — Per-Regime Vol + Disp Overlay Re-Tune [VECTORIZED]
================================================================
Baseline: v2.49.23, OBJ=5.7195 (Phase 298 V1 weights)

New R&D cycle. After P298 (V1 weights + regime weights changed),
re-tune both vol and disp overlays with finer grids around current values.

Current (from P295):
  Vol thresh: LOW=0.54, MID=0.50, HIGH=0.57
  Vol scale:  LOW=0.45, MID=0.15, HIGH=0.01
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase300"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase300_report.json").write_text(json.dumps(_partial, indent=2))
...
# Grids — finer around current values (P295 winners)
# Current: vol_thr LOW=0.54 MID=0.50 HIGH=0.57, vol_scale LOW=0.45 MID=0.15 HIGH=0.01
VOL_THR_GRID   = [0.42, 0.46, 0.48, 0.50, 0.52, 0.53, 0.54, 0.55, 0.56, 0.57, 0.58, 0.59, 0.60, 0.63]
VOL_SCALE_GRID = [0.005, 0.01, 0.02, 0.03, 0.05, 0.08, 0.10, 0.12, 0.15, 0.20, 0.30, 0.40, 0.45, 0.50]
# Current: disp_thr LOW=0.75 MID=0.40 HIGH=0.30, disp_scale LOW=0.60 MID=2.00 HIGH=0.40
DISP_THR_GRID  = [0.22, 0.25, 0.28, 0.30, 0.33, 0.35, 0.38, 0.40, 0.45, 0.50, 0.55, 0.60, 0.70, 0.75, 0.80]
DISP_SCALE_GRID = [0.20, 0.30, 0.35, 0.40, 0.50, 0.60, 0.70, 0.80, 1.0, 1.2, 1.5, 1.8, 2.0, 2.2, 2.5, 3.0]
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 300 — Vol + Disp Overlay Re-Tune [VECTORIZED]")
    print("=" * 70)
    ...
    result = {
        "phase": "300", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    out = Path("artifacts/phase300"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase300_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        old_ver = cfg.get("_version", "2.49.23")
        parts = old_ver.split(".")
        parts[-1] = str(int(parts[-1]) + 1)
        new_ver = ".".join(parts)
        cfg["_version"] = new_ver
        old_vver = cfg.get("version", "v2.49.23").lstrip("v")
        ...
        msg = (f"feat: P300 vol+disp retune OBJ={reported_obj:.4f} "
