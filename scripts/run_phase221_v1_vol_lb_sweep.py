"""
Phase 221 — V1 vol_lookback_bars Sweep
========================================
V1 uses vol_lookback_bars=192 for volatility normalization (Sharpe-like
position sizing within the strategy). This was set very early and
never re-swept.

Pre-compute I460/I415/F168 once per year, only vary V1 vol_lb.
Sweep: [48, 72, 96, 120, 144, 168, 192, 240, 288, 336]
...
def _on_timeout(signum, frame):
    ...
    (out / "phase221_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
BASELINE_OBJ = 3.1534
VOL_LB_BASE  = 192
VOL_LB_SWEEP = [48, 72, 96, 120, 144, 168, 192, 240, 288, 336]
...
def main():
    t0 = time.time()
    print("=" * 65)
    print("Phase 221 — V1 vol_lookback_bars Sweep")
    print(f"Baseline: v2.34.0  OBJ={BASELINE_OBJ}")
    print(f"Current: vol_lb={VOL_LB_BASE}")
    print(f"Sweep: {VOL_LB_SWEEP}")
    print("=" * 65)

    print("\n[1/3] Loading per-year data & pre-computing fixed signals...")
    all_data = {}
    ...
    for yr, (dataset, btc_vol, fsp, tsp, brd_pct, fixed_rets, n) in all_data.items():
        v1_r = np.array(BacktestEngine(BacktestConfig(costs=COST_MODEL)).run(
            dataset, make_strategy({"name": "nexus_alpha_v1", "params": V1_BASE})).returns)
        sig_rets = dict(fixed_rets); sig_rets["v1"] = v1_r
        ens = ensemble_rets(sig_rets, btc_vol, fsp, tsp, brd_pct, n)
    ...
    print("\n[2/3] Sweep...")
    best_obj_val = base_obj_val; best_vl = VOL_LB_BASE
    results = []
    for vl in VOL_LB_SWEEP:
        params = dict(V1_BASE, vol_lookback_bars=vl)
        yearly = {}
        for yr, (dataset, btc_vol, fsp, tsp, brd_pct, fixed_rets, n) in all_data.items():
            v1_r = np.array(BacktestEngine(BacktestConfig(costs=COST_MODEL)).run(
                dataset, make_strategy({"name": "nexus_alpha_v1", "params": params})).returns)
            sig_rets = dict(fixed_rets); sig_rets["v1"] = v1_r
            ens = ensemble_rets(sig_rets, btc_vol, fsp, tsp, brd_pct, n)
        ...
        print(f"  vol_lb={vl:4d}  OBJ={o:.4f}  Δ={delta:+.4f}{marker}")
        results.append((vl, o))
        if o > best_obj_val:
            best_obj_val = o; best_vl = vl

    print(f"\n  → Best: vol_lb={best_vl}  OBJ={best_obj_val:.4f}")

    print("\n[3/3] LOYO validation...")
    params = dict(V1_BASE, vol_lookback_bars=best_vl)
    joint_yearly = {}
    for yr, (dataset, btc_vol, fsp, tsp, brd_pct, fixed_rets, n) in all_data.items():
        v1_r = np.array(BacktestEngine(BacktestConfig(costs=COST_MODEL)).run(
            dataset, make_strategy({"name": "nexus_alpha_v1", "params": params})).returns)
    ...
    print(f"\n{'✅ VALIDATED' if validated else '❌ NO IMPROVEMENT'} — vol_lb={best_vl}")

    result = {
        "phase": 221, "baseline_obj": float(base_obj_val), "best_obj": float(joint_obj_val),
        "delta": float(delta), "loyo_wins": loyo_wins, "loyo_total": len(joint_yearly),
        "validated": validated, "best_vol_lb": best_vl, "sweep_results": results,
    }
    _partial.update(result)
    out = Path("artifacts/phase221"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase221_report.json").write_text(json.dumps(result, indent=2))

    if validated:
        cfg_path = ROOT / "configs" / "production_p91b_champion.json"
        cfg = json.load(open(cfg_path))
        cfg["ensemble"]["signals"]["v1"]["params"]["vol_lookback_bars"] = best_vl
        cfg["_version"] = "2.35.0"
