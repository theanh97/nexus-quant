"""
Phase 201 — Walk-Forward Validation of v2.28.0 vs p91b_flat
=============================================================
Milestone WF check after crossing OBJ=3.0 barrier.
Significant changes since last WF (P195 @ v2.24.0):
  - P195-parallel: Regime weights full re-opt (f168 weight surge: LOW=0.45, MID=0.55!)
  - P197: FTS thresholds (rt=0.65, rs=0.45, bt=0.25, bs=1.70)
  - P198: Vol overlay retune (scale=0.30, boost=0.00)
  - P200: FTS windows (short=12->16, long=96->72)
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase201"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase201_report.json").write_text(json.dumps(_partial, indent=2, default=str))
    sys.exit(0)
...
# ── MAIN ────────────────────────────────────────────────────────────────────
print("=" * 60)
print("Phase 201 — WF Validation v2.28.0 (OBJ>3.0 Milestone)")
print("=" * 60)
_start = time.time()

prod_cfg, WEIGHTS_V228, PARAMS = get_config()
ver_now = prod_cfg.get("_version", "2.28.0")
print(f"  Config: {ver_now}")
print(f"  Weights: {WEIGHTS_V228}")
...
is_yr_v228 = {}; is_yr_flat = {}
for yr in YEARS:
    ens_v = compute_ensemble(year_data[yr], WEIGHTS_V228, PARAMS, use_overlay=True)
    ens_f = compute_ensemble(year_data[yr], WEIGHTS_FLAT, PARAMS, use_overlay=False)
    n = year_data[yr][-1]
    is_yr_v228[yr] = sharpe(ens_v, n)
    is_yr_flat[yr] = sharpe(ens_f, n)
    print(f"  {yr}: v228={is_yr_v228[yr]:.4f}  flat={is_yr_flat[yr]:.4f}  Δ={is_yr_v228[yr]-is_yr_flat[yr]:+.4f}")

is_obj_v228 = obj_fn(is_yr_v228)
is_obj_flat = obj_fn(is_yr_flat)
print(f"\n  IS OBJ: v228={is_obj_v228:.4f}  flat={is_obj_flat:.4f}  Δ={is_obj_v228-is_obj_flat:+.4f}")
...
for window in WF_WINDOWS:
    ...
    test_yr = window["test"]
    is_v = [is_yr_v228[y] for y in train_yrs]
    is_f = [is_yr_flat[y] for y in train_yrs]
    is_v228_wf = float(np.mean(is_v) - 0.5 * np.std(is_v))
    is_flat_wf = float(np.mean(is_f) - 0.5 * np.std(is_f))
    oos_v = is_yr_v228[test_yr]
    ...
    wf_results.append({
        ...
        "is_v228": is_v228_wf, "is_flat": is_flat_wf,
        "oos_v228": oos_v, "oos_flat": oos_f, "oos_delta": delta, "win": win
    })
    print(f"  Train={train_yrs} Test={test_yr}: OOS v228={oos_v:.4f} flat={oos_f:.4f} Δ={delta:+.4f} {'✅WIN' if win else '❌LOSS'}")

avg_oos_delta = float(np.mean([r["oos_delta"] for r in wf_results]))
degrad = is_obj_v228 / is_obj_flat if is_obj_flat > 0 else 0
...
report = {
    "phase": 201,
    "description": f"WF Validation of {ver_now} vs p91b_flat (OBJ>3.0 Milestone)",
    "elapsed_seconds": round(time.time() - _start, 1),
    "is_obj_v228": is_obj_v228, "is_obj_flat": is_obj_flat,
    "is_delta": is_obj_v228 - is_obj_flat,
    "is_yearly_v228": is_yr_v228, "is_yearly_flat": is_yr_flat,
    ...
    "verdict": f"{verdict} — {ver_now} OBJ={is_obj_v228:.4f} | WF {wf_wins}/3 wins | avg_OOS_delta={avg_oos_delta:+.4f} | IS_delta={is_obj_v228-is_obj_flat:+.4f}",
    "partial": False,
    "timestamp": datetime.now(UTC).isoformat(),
}
(out / "phase201_report.json").write_text(json.dumps(report, indent=2, default=str))
print(f"  Saved: artifacts/phase201/phase201_report.json")

cfg = json.loads(Path("configs/production_p91b_champion.json").read_text())
cfg["_validated"] += f"; WF Validation P201: {verdict} {ver_now} OBJ={is_obj_v228:.4f} WF {wf_wins}/3 avg_OOS_delta={avg_oos_delta:+.4f}"
Path("configs/production_p91b_champion.json").write_text(json.dumps(cfg, indent=2))

wf_str = "CONFIRMED" if wf_confirmed else "WEAK"
cm = "phase201: WF Validation %s -- %s %d/3 wins avg_OOS_delta=%+.4f IS_OBJ=%.4f IS_delta=%+.4f" % (ver_now, wf_str, wf_wins, avg_oos_delta, is_obj_v228, is_obj_v228-is_obj_flat)
os.system("git add configs/production_p91b_champion.json artifacts/phase201/ scripts/run_phase201_wf_validation_v228.py")
os.system('git commit -m "' + cm + '"')
os.system("git pull --rebase && git push")
print("\n[DONE] Phase 201 complete.")
