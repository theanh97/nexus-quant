"""
Phase 370 — Per-Regime V1 Lookback Sweep [VECTORIZED]
=======================================================
Baseline: v2.49.45, OBJ=7.0152 (P365-P369 cycle confirmed all dims optimal)

NEW DIMENSION: Sweep V1 momentum_lookback_bars and mean_reversion_lookback_bars
per regime. Currently these are global constants (ml=312, mrl=84) hardcoded
in all evaluation scripts — never swept per regime.

Hypothesis: Different market regimes may benefit from different lookback horizons.
  LOW regime (defensive): shorter lookbacks may be more adaptive
  HIGH regime (momentum): longer lookbacks may smooth noise better

Current global values:
  momentum_lookback_bars: 312  (all regimes)
  mean_reversion_lookback_bars: 84  (all regimes)

Grid:
  ML candidates:  [192, 240, 288, 312, 336, 384, 432, 480]
  MRL candidates: [48, 60, 72, 84, 96, 120, 144]
  Combos per regime: 56 (coarse)

Approach: Coordinate descent per regime (LOW → MID → HIGH), 2 passes.
"""
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase370"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase370_report.json").write_text(json.dumps(_partial, indent=2))
...
ML_GRID  = [192, 240, 288, 312, 336, 384, 432, 480]  # momentum lookback
MRL_GRID = [48, 60, 72, 84, 96, 120, 144]            # mean-reversion lookback
...
_v1_cache: dict = {}  # key: (year, ml, mrl, wc, wm, wmr)
...
def load_config_params():
    ...
    def v1p(rn):
        ...
        return {
            ...
            "momentum_lookback_bars": int(d.get("momentum_lookback_bars", 312)),
            "mean_reversion_lookback_bars": int(d.get("mean_reversion_lookback_bars", 84)),
...
def compute_v1_with_lb(year, dataset, wc, wm, wmr, ml, mrl):
    key = (year, round(wc, 2), round(wm, 2), round(wmr, 2), ml, mrl)
    if key in _v1_cache:
        return _v1_cache[key]
    params = {
        "k_per_side": 2,
        "w_carry": wc, "w_mom": wm, "w_mean_reversion": wmr,
        "momentum_lookback_bars": ml,
        "mean_reversion_lookback_bars": mrl,
        "vol_lookback_bars": 192, "target_gross_leverage": 0.35,
        "rebalance_interval_bars": 60,
    }
    res = BacktestEngine(BacktestConfig(costs=COST_MODEL)).run(
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 370 — Per-Regime V1 Lookback Sweep")
    print("=" * 70)

    p = load_config_params()
    STORED_OBJ = p["baseline_obj"]
    print(f"Config: v{p['config_version']}  stored OBJ={STORED_OBJ:.4f}")
    print(f"V1 per-regime params:")
    for rn in RNAMES:
        vp = p["v1_params"][rn]
        print(f"  {rn}: wc={vp['w_carry']:.2f} wm={vp['w_mom']:.2f} wmr={vp['w_mean_reversion']:.2f} "
              f"ml={vp['momentum_lookback_bars']} mrl={vp['mean_reversion_lookback_bars']}")

    total_combos = len(ML_GRID) * len(MRL_GRID)
    print(f"\nLookback sweep grid: ML={ML_GRID}")
    print(f"                     MRL={MRL_GRID}")
    print(f"  Total combos per regime: {total_combos}")
    ...
    for yr in sorted(YEAR_RANGES):
        ...
        datasets[yr] = result[6]
        all_base[yr] = result[:6]
    ...
    for yr in sorted(YEAR_RANGES):
        ...
        for rn in RNAMES:
            vp = p["v1_params"][rn]
            v1_per_regime[rn] = compute_v1_with_lb(
                yr, ds, vp["w_carry"], vp["w_mom"], vp["w_mean_reversion"],
                vp["momentum_lookback_bars"], vp["mean_reversion_lookback_bars"])
    ...
    print("\n[4] V1 lookback sweep per regime (2 passes)...", flush=True)

    # Current best per regime
    best_lb = {}
    for rn in RNAMES:
        vp = p["v1_params"][rn]
        best_lb[rn] = (vp["momentum_lookback_bars"], vp["mean_reversion_lookback_bars"])
    ...
    for pass_num in range(1, 3):
        ...
        for rn in RNAMES:
            tw = time.time()
            best_for_regime = best_lb[rn]
            vp = p["v1_params"][rn]
            wc, wm, wmr = vp["w_carry"], vp["w_mom"], vp["w_mean_reversion"]

            for ml_v in ML_GRID:
                for mrl_v in MRL_GRID:
                    v1_new = {}
                    for yr in sorted(YEAR_RANGES):
                        v1_new[yr] = compute_v1_with_lb(yr, datasets[yr], wc, wm, wmr, ml_v, mrl_v)
                    obj, _ = eval_with_v1(p, fast, rn, v1_new)
                    if obj > best_global:
                        best_global = obj
                        best_for_regime = (ml_v, mrl_v)

            print(f"    {rn}: ml={best_for_regime[0]} mrl={best_for_regime[1]}  "
                  f"OBJ={best_global:.4f}  Δ={best_global-base_obj:+.4f}  [{int(time.time()-tw)}s]",
                  flush=True)

            if best_lb[rn] != best_for_regime:
                best_lb[rn] = best_for_regime
                # Update fast data with new best V1 for this regime
                for yr in sorted(YEAR_RANGES):
                    ml_v, mrl_v = best_for_regime
                    new_ret = compute_v1_with_lb(yr, datasets[yr], wc, wm, wmr, ml_v, mrl_v)
                    fast[yr]["v1_bl_sc"][rn] = fast[yr]["overlay"] * new_ret[:fast[yr]["ml"]]

            regime_results[rn] = {"momentum_lookback_bars": best_lb[rn][0],
                                   "mean_reversion_lookback_bars": best_lb[rn][1]}
            _partial[f"best_lb_{rn}"] = regime_results[rn]
    ...
    for yr in sorted(YEAR_RANGES):
        ...
        for rn in RNAMES:
            vp = p["v1_params"][rn]
            ml_v, mrl_v = best_lb[rn]
            final_v1[yr][rn] = compute_v1_with_lb(
                yr, datasets[yr], vp["w_carry"], vp["w_mom"], vp["w_mean_reversion"], ml_v, mrl_v)
    ...
    for rn in RNAMES:
        vp = p["v1_params"][rn]
        old_ml = vp["momentum_lookback_bars"]
        old_mrl = vp["mean_reversion_lookback_bars"]
        new_ml, new_mrl = best_lb[rn]
        changed = (old_ml != new_ml or old_mrl != new_mrl)
        tag = " ← CHANGED" if changed else " (unchanged)"
        print(f"    {rn}: ml={old_ml}→{new_ml}  mrl={old_mrl}→{new_mrl}{tag}")

    result = {
        "phase": "370", "title": "Per-Regime V1 Lookback Sweep",
        ...
        "best_lookbacks": {rn: {"momentum_lookback_bars": best_lb[rn][0],
                                "mean_reversion_lookback_bars": best_lb[rn][1]}
                           for rn in RNAMES},
        "original_lookbacks": {rn: {"momentum_lookback_bars": p["v1_params"][rn]["momentum_lookback_bars"],
                                     "mean_reversion_lookback_bars": p["v1_params"][rn]["mean_reversion_lookback_bars"]}
                                for rn in RNAMES},
    ...
    out = Path("artifacts/phase370"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase370_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase370/phase370_report.json")
    ...
    if validated:
        ...
        for rn in RNAMES:
            ...
            if rn not in v1prw:
                v1prw[rn] = {}
            v1prw[rn]["momentum_lookback_bars"] = new_ml
            v1prw[rn]["mean_reversion_lookback_bars"] = new_mrl
        ...
        msg = (f"feat: P370 per-regime V1 lookback sweep OBJ={reported_obj:.4f} "
