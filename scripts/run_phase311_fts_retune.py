"""
Phase 311 — Per-Regime FTS Retune after P307-P310 Convergence [VECTORIZED]
============================================================================
Baseline: v2.49.29, OBJ=5.9744 (Phase 307 vol+disp retune, LOYO=3/5)

P307-P310 showed deep convergence (all Δ≈0). Re-sweep FTS overlay
in case the vol/disp changes from P307 shifted the FTS landscape.

Current FTS (from P290, confirmed in P296, P301, P306):
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase311_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase311_fts_report.json").write_text(json.dumps(_partial, indent=2))
...
RS_GRID  = [0.005, 0.008, 0.01, 0.015, 0.02, 0.03, 0.05, 0.08, 0.10, 0.15, 0.20, 0.30]
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 311 — FTS Retune after P307-P310 Convergence [VECTORIZED]")
    print("Baseline: v2.49.29 OBJ=5.9744")
    print("=" * 70)
    ...
    for pass_num in [1, 2]:
        ...
        for rname in RNAMES:
            ...
            print(f"    {rname}: rs={bst['rs']:.3f} rt={bst['rt']:.2f} bs={bst['bs']:.2f} "
    ...
    result = {
        "phase": "311", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    out = Path("artifacts/phase311_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase311_fts_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase311_fts/phase311_fts_report.json")
    ...
    if validated:
        ...
        old_ver = cfg.get("_version", "2.49.29")
        parts = old_ver.split(".")
        parts[-1] = str(int(parts[-1]) + 1)
        new_ver = ".".join(parts)
        cfg["_version"] = new_ver
        old_vver = cfg.get("version", "v2.49.29").lstrip("v")
        ...
        msg = (f"feat: P311 FTS retune OBJ={reported_obj} "
