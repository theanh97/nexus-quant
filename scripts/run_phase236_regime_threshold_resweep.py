"""
Phase 236 — Regime Threshold Re-Sweep (P_LOW × P_HIGH)
========================================================
P235: BRD_LB/PCT_WIN confirmed at exact peak (NO IMPROVEMENT).
P217 confirmed P_LOW=0.30, P_HIGH=0.60 — but that was with UNIFORM overlays.
Now we have highly differentiated per-regime overlay stack (P229-P232):
  - Per-regime FTS RS/BS/RT/BT (large discontinuity at regime boundary)
  - Per-regime VOL scale
  - Per-regime DISP threshold/scale (including AMPLIFY in MID)

With these overlay discontinuities, the optimal regime boundaries may shift.
E.g., if MID DISP_SCALE=1.50 (amplify) and LOW DISP_SCALE=0.50 (dampen),
the transition zone at P_LOW boundary has high impact.

Pure NumPy sweep (reuse pre-computed signals, only vary breadth percentile thresholds).

P_LOW sweep:  [0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45]
P_HIGH sweep: [0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85]

Constraint: P_LOW < P_HIGH (enforced).
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase236"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase236_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
P_HIGH_SWEEP = [0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85]
...
def main():
    t0 = time.time()
    print("=" * 65)
    print("Phase 236 — Regime Threshold Re-Sweep (P_LOW × P_HIGH)")
    ...
    base_obj_val, base_yearly = evaluate(P_LOW_BASE, P_HIGH_BASE, all_data)
    print(f"\n  Baseline OBJ = {base_obj_val:.4f}  (expected ≈ {BASELINE_OBJ})")
    _partial.update({"baseline_obj": float(base_obj_val)})

    print("\n[2/3] 2D Sweep (P_LOW × P_HIGH)...")
    best_obj_val = base_obj_val; best_pl = P_LOW_BASE; best_ph = P_HIGH_BASE
    results = []
    for pl in P_LOW_SWEEP:
        for ph in P_HIGH_SWEEP:
            ...
            if o > best_obj_val:
                best_obj_val = o; best_pl = pl; best_ph = ph

    print(f"\n  → Best: P_LOW={best_pl}  P_HIGH={best_ph}  OBJ={best_obj_val:.4f}")

    print(f"\n[3/3] LOYO validation...")
    joint_obj_val, joint_yearly = evaluate(best_pl, best_ph, all_data)
    ...
    result = {
        "phase": 236, "baseline_obj": float(base_obj_val), "best_obj": float(joint_obj_val),
    ...
    out = Path("artifacts/phase236"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase236_report.json").write_text(json.dumps(result, indent=2))
