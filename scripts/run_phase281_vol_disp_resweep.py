"""
Phase 281 — Per-Regime Vol + Disp Overlay Re-Sweep [VECTORIZED]
================================================================
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase281"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase281_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 281 — Vol + Disp Overlay Re-Sweep [VECTORIZED]")
    print("=" * 70)
    ...
    out = Path("artifacts/phase281"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase281_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        msg = (f"feat: P281 vol+disp resweep OBJ={reported_obj:.4f} "
