"""
Phase 207 — I460 Beta Window Sweep
=====================================
I460 is 28% HIGH + 7% LOW. In HIGH regime, idio = 79% total.
Last tuned: bw=168 (P194). Re-sweep with current regime weights.
Pre-compute V1, I415, F168; only vary I460 beta_window.

Baseline: v2.32.0, OBJ=3.0860
Current: i460 beta_window_bars=168
Sweep: [96, 120, 144, 168, 192, 240, 288, 336]
"""
...
def _on_timeout(signum, frame):
    ...
    (out / "phase207_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
I460_BASE = {"k_per_side": 4, "lookback_bars": 460,
             "target_gross_leverage": 0.3, "rebalance_interval_bars": 48}

BASELINE_OBJ  = 3.0860
BW_BASE       = 168
BW_SWEEP      = [96, 120, 144, 168, 192, 240, 288, 336]
...
def ensemble_rets(i460_rets, fixed_rets, btc_vol, fund_std_pct, ts_spread_pct, brd_pct):
    RNAMES = ["LOW", "MID", "HIGH"]
    sk_all = ["v1", "i460", "i415", "f168"]
    all_rets = {**fixed_rets, "i460": i460_rets}
...
def evaluate_bw(bw, all_data):
    ...
    for yr, (dataset, btc_vol, fsp, tsp, brd_pct, fixed_rets, n) in all_data.items():
        i460r = run_i460(dataset, bw)
        ens = ensemble_rets(i460r, fixed_rets, btc_vol, fsp, tsp, brd_pct)
...
def main():
    t0 = time.time()
    print("=" * 65)
    print("Phase 207 — I460 Beta Window Sweep")
    print(f"Baseline: v2.32.0  OBJ={BASELINE_OBJ}")
    print(f"Current: i460 bw={BW_BASE}  (28% HIGH, HIGH=79% idio)")
    print(f"Sweep: {BW_SWEEP}")
    print("=" * 65)

    print("\n[1/3] Loading per-year data & pre-compute V1/I415/F168...")
    all_data = {}
    ...
    print(f"\n  → Best: i460_bw={best_bw}  OBJ={best_obj:.4f}")
    ...
    print(f"\n{'✅ VALIDATED' if validated else '❌ NO IMPROVEMENT'} — i460_bw={best_bw}")

    result = {
        "phase": 207, "baseline_obj": float(base_obj), "best_obj": float(joint_obj),
    ...
    out = Path("artifacts/phase207"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase207_report.json").write_text(json.dumps(result, indent=2))
