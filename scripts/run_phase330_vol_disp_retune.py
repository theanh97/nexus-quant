"""
Phase 330 — Per-Regime Vol + Disp Overlay Re-Tune [VECTORIZED]
================================================================
Baseline: v2.49.37, OBJ=6.4749 (P329 V1 weight resweep — no change)

After full cycle P325-P329 with P328 validated (regime weights),
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase330"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase330_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 330 — Vol + Disp Overlay Re-Tune [VECTORIZED]")
    print("=" * 70)
    ...
    result = {
        "phase": "330", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    out = Path("artifacts/phase330"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase330_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        old_vver = cfg.get("version", "v2.49.37").lstrip("v")
        ...
        msg = (f"feat: P330 vol+disp retune OBJ={reported_obj:.4f} "
