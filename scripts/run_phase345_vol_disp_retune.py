"""
Phase 345 — Per-Regime Vol + Disp Overlay Re-Tune [VECTORIZED]
================================================================
Baseline: v2.49.40, OBJ=6.7382 (P344 V1 weight resweep — no change)

After full cycle P340-P344 with P340-P344 full convergence,
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase345"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase345_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 345 — Vol + Disp Overlay Re-Tune [VECTORIZED]")
    print("=" * 70)
    ...
    result = {
        "phase": "345", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    out = Path("artifacts/phase345"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase345_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        msg = (f"feat: P345 vol+disp retune OBJ={reported_obj:.4f} "
