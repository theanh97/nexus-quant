"""
Phase 208 — V1 Mean Reversion Lookback Extended Sweep
=======================================================
V1: mr_lb was tuned at P187 (72→84h, LOYO 4/5 Δ=+0.1158 OBJ=2.7103).
Since then regime weights changed significantly:
  - V1 now has weight: LOW=0.46, MID=0.20, HIGH=0.04
  - V1 is primarily a LOW-regime signal now (breadth-defensive)
  - Mean reversion lookback may have a different optimal in this context

Sweep mr_lb ∈ [72, 78, 84, 90, 96, 108, 120, 144, 168]
  - Load f168, i460, i415 signals once (fixed)
  - Re-run V1 for each mr_lb value
  - Use current regime weights and all overlays
...
def _on_timeout(signum, frame):
    ...
    (out / "phase208_report.json").write_text(json.dumps(_partial, indent=2, default=str))
    sys.exit(0)
...
MR_LB_SWEEP = [72, 78, 84, 90, 96, 108, 120, 144, 168]
...
def get_config():
    ...
    v1_params = ens_cfg.get("signals", {}).get("v1", {}).get("params", {})
    params["v1_vol_lb"] = v1_params.get("vol_lookback_bars", 192)
    params["v1_mr_lb"] = v1_params.get("mean_reversion_lookback_bars", 84)
    params["v1_wc"] = v1_params.get("w_carry", 0.35)
    params["v1_wm"] = v1_params.get("w_mom", 0.40)
    params["v1_wr"] = v1_params.get("w_mean_reversion", 0.25)
...
def load_year_fixed_signals(year, params):
    """Load data + fixed signals (i460, i415, f168). V1 loaded per mr_lb."""
    p = params
...
def load_v1_for_mr_lb(dataset, mr_lb, params):
    p = params
    sp = {
        "k_per_side": 2, "w_carry": p["v1_wc"], "w_mom": p["v1_wm"],
        "w_mean_reversion": p["v1_wr"],
        "momentum_lookback_bars": 336, "mean_reversion_lookback_bars": mr_lb,
...
# ── MAIN ────────────────────────────────────────────────────────────────────
print("=" * 60)
print("Phase 208 — V1 Mean Reversion Lookback Extended Sweep")
print("=" * 60)
_start = time.time()

prod_cfg, WEIGHTS, PARAMS, baseline_obj = get_config()
ver_now = prod_cfg.get("_version", "2.32.0")
curr_mr_lb = PARAMS["v1_mr_lb"]
print(f"  Config: {ver_now}  baseline_obj={baseline_obj:.4f}")
print(f"  V1 now: mr_lb={curr_mr_lb}  vol_lb={PARAMS['v1_vol_lb']}  wc={PARAMS['v1_wc']}  wm={PARAMS['v1_wm']}  wr={PARAMS['v1_wr']}")
...
for mr_lb in MR_LB_SWEEP:
    ...
    for yr in YEARS:
        ...
        v1_rets = load_v1_for_mr_lb(dataset, mr_lb, PARAMS)
        ens = compute_ensemble(year_data[yr], WEIGHTS, PARAMS, v1_rets)
        yr_sharpes[yr] = sharpe(ens, year_data[yr][-1])
    obj = obj_fn(yr_sharpes)
    results.append((mr_lb, obj, yr_sharpes))
    print(f"  mr_lb={mr_lb:3d}h  OBJ={obj:.4f}")

best = max(results, key=lambda x: x[1])
best_mr_lb, best_obj, best_yr_sharpes = best
delta = best_obj - baseline_obj
print(f"\n  Best mr_lb: {best_mr_lb}h  OBJ={best_obj:.4f}  Δ={delta:+.4f}")

# Baseline yr_sharpes (current mr_lb)
curr_result = next((r for r in results if r[0] == curr_mr_lb), None)
...
loyo_wins = 0
for yr in YEARS:
    win = best_yr_sharpes.get(yr, 0) > base_yr.get(yr, 0)
    if win: loyo_wins += 1
    print(f"  {yr}: best={best_yr_sharpes.get(yr,0):.4f} curr={base_yr.get(yr,0):.4f} {'WIN' if win else 'LOSS'}")
...
report = {
    "phase": 208,
    "description": "V1 mean_reversion_lookback_bars extended sweep",
    "elapsed_seconds": round(time.time() - _start, 1),
    "baseline_obj": baseline_obj, "best_obj": best_obj, "delta": delta,
    "curr_mr_lb": curr_mr_lb, "best_mr_lb": best_mr_lb,
    "loyo_wins": loyo_wins, "validated": validated,
    "yearly_best": best_yr_sharpes, "yearly_base": base_yr,
    "sweep_results": [(r[0], r[1]) for r in results],
}
(out / "phase208_report.json").write_text(json.dumps(report, indent=2, default=str))
...
if validated:
    ...
    v1_sig.get("params", {})["mean_reversion_lookback_bars"] = best_mr_lb
    ...
    cfg["_validated"] = old_val + (
        "; V1 mr_lb P208: %d->%d LOYO %d/5 delta=%+.4f OBJ=%.4f — PRODUCTION %s" % (
            curr_mr_lb, best_mr_lb, loyo_wins, delta, best_obj, new_ver))
    Path("configs/production_p91b_champion.json").write_text(json.dumps(cfg, indent=2))
    print(f"\n✅ VALIDATED → {new_ver} OBJ={best_obj:.4f}")
    cm = "phase208: V1 mr_lb sweep -- VALIDATED mr_lb=%d->%d LOYO %d/5 delta=%+.4f OBJ=%.4f" % (
        curr_mr_lb, best_mr_lb, loyo_wins, delta, best_obj)
else:
    new_ver = ver_now
    print(f"\n⚠️ NO IMPROVEMENT — mr_lb={curr_mr_lb} near-optimal (LOYO {loyo_wins}/5 delta={delta:+.4f})")
    cm = "phase208: V1 mr_lb sweep -- WEAK LOYO %d/5 delta=%+.4f best_mr_lb=%d" % (
        loyo_wins, delta, best_mr_lb)

os.system("git add configs/production_p91b_champion.json scripts/run_phase208_v1_mr_lb_sweep.py "
          "artifacts/phase208/ 2>/dev/null || true")
os.system('git commit -m "' + cm + '"')
os.system("git stash && git pull --rebase && git stash pop && git push")

print(f"\n[DONE] Phase 208 complete.")
