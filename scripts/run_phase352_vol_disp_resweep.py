"""
Phase 352 — Per-Regime Vol + Disp Overlay Re-Sweep [VECTORIZED]
================================================================
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase352"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase352_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 352 — Vol + Disp Overlay Re-Tune [VECTORIZED]")
    print("=" * 70)
    ...
    result = {
        "phase": "352", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    out = Path("artifacts/phase352"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase352_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        msg = (f"feat: P352 vol+disp retune OBJ={reported_obj:.4f} "
