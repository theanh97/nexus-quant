"""
Phase 279 — Per-Regime FTS Resweep against P271 Baseline [VECTORIZED]
=====================================================================
Baseline: v2.49.1, OBJ=4.5951 (Phase 271 regime weight reopt12, LOYO=5/5)

P268 FTS v3 found LOYO 5/5 improvement (rs→0.02, different bs/bt) but was
tested against lower baseline (OBJ=4.26). Re-running against P271 baseline.

Key P268 findings to seed search:
  - rs→0.02 across all regimes (nearly disable FTS reduce leg)
  - MID: bs 2.5→1.5, bt 0.22→0.22
  - HIGH: bs 2.25→3.5, bt 0.22→0.15, pct_win 400→288

Sweep: 2-pass coordinate descent (rs→rt→bs→bt→pct_win per regime)
Grids centered around both current baseline AND P268 findings.
"""
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase279_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase279_fts_report.json").write_text(json.dumps(_partial, indent=2))
...
# Grids — include P268 optimal values to ensure they're tested
RS_GRID  = [0.02, 0.05, 0.08, 0.10, 0.15, 0.20, 0.25, 0.30, 0.40, 0.50]
RT_GRID  = [0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90]
BS_GRID  = [1.00, 1.50, 2.00, 2.50, 3.00, 3.50, 4.00, 4.50, 5.00]
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 279 — FTS Resweep against P271 Baseline [VECTORIZED]")
    print("Baseline: P271 OBJ=4.5951 (LOYO 5/5)")
    print("=" * 70)
    ...
    result = {
        "phase": "279_fts", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    out = Path("artifacts/phase279_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase279_fts_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase279_fts/phase279_fts_report.json")
    ...
    if validated:
        ...
        print(f"\n  Config updated → v{new_ver}  OBJ={round(final_obj, 4)}")
        subprocess.run(["git", "add", str(CFG_PATH)], cwd=ROOT)
        msg = (f"feat: P279 FTS resweep OBJ={round(final_obj,4)} "
