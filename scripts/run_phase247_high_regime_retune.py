#!/usr/bin/env python3
"""
Phase 247 — HIGH Regime Comprehensive Re-tune
Current HIGH: v1=0.06 f168=0.00 i460=0.282 i415=0.658
Sweep: A) f168, B) v1, C) idio_ratio fine-tune
"""
...
def compute_ensemble(data, params, weights):
    """Compute ensemble returns applying breadth regime + FTS + vol overlays."""
    n   = data["n"]
    p   = params
    w   = weights
    brd = data["brd_pct"]
    tsp = data["ts_spread_pct"]
    vol = data["btc_vol"]
...
def eval_weights(year_data, params, weights):
    sharpes = []
    for yr in YEARS:
        s = yearly_sharpe(year_data[yr], params, weights)
        sharpes.append(s)
    arr = np.array(sharpes)
    return float(np.mean(arr)) - 0.5 * float(np.std(arr))

def make_high_w(base, f168_w, v1_w, idio_ratio):
    """Build HIGH weights: v1=v1_w, f168=f168_w, remainder→idio split by ratio"""
...
def main():
    t0 = time.time()
    print("=" * 60)
    print("Phase 247 — HIGH Regime Comprehensive Re-tune")
    print("=" * 60)

    cfg, weights, params, v1p, i460p, i415p, f168p = get_config()

    cur_high = weights["HIGH"]
    cur_v1   = cur_high["v1"]
    cur_f168 = cur_high["f168"]
    cur_i460 = cur_high["i460"]
    cur_i415 = cur_high["i415"]
    cur_idio_total = cur_i460 + cur_i415
    cur_idio_ratio = (cur_i460 / cur_idio_total) if cur_idio_total > 1e-9 else 0.3

    print(f"  Current HIGH: v1={cur_v1} f168={cur_f168} i460={cur_i460} i415={cur_i415}")
    print(f"  idio_total={cur_idio_total:.4f}  idio_ratio={cur_idio_ratio:.3f}")
    ...
    # ── Part A: HIGH f168 sweep ──────────────────────────────────────────────
    print("\n[2] Part A: HIGH f168 sweep (v1 fixed, idio_ratio=0.30) ...")
    f168_cands = [0.00, 0.03, 0.05, 0.08, 0.10, 0.12, 0.15, 0.18, 0.20]
    best_f168 = cur_f168; best_f168_obj = base_obj
    for f168_w in f168_cands:
        w_test = make_high_w(weights, f168_w, cur_v1, cur_idio_ratio)
        o = eval_weights(year_data, params, w_test)
        marker = " <-" if o > best_f168_obj + 1e-6 else ""
        hi = w_test["HIGH"]
        print(f"  f168={f168_w:.2f}  v1={hi['v1']:.2f}  i460={hi['i460']:.4f}  "
              f"i415={hi['i415']:.4f}  OBJ={o:.4f}{marker}")
        if o > best_f168_obj + 1e-6:
            best_f168_obj = o; best_f168 = f168_w
    print(f"  Best f168: {best_f168}  OBJ={best_f168_obj:.4f}")

    # ── Part B: HIGH v1 sweep ────────────────────────────────────────────────
    print(f"\n[3] Part B: HIGH v1 sweep (f168={best_f168} fixed, idio_ratio={cur_idio_ratio:.2f}) ...")
    v1_cands = [0.00, 0.02, 0.04, 0.06, 0.08, 0.10, 0.12, 0.15, 0.18, 0.20]
    best_v1 = cur_v1; best_v1_obj = best_f168_obj
    for v1_w in v1_cands:
        w_test = make_high_w(weights, best_f168, v1_w, cur_idio_ratio)
        o = eval_weights(year_data, params, w_test)
        marker = " <-" if o > best_v1_obj + 1e-6 else ""
        hi = w_test["HIGH"]
        print(f"  v1={v1_w:.2f}  f168={best_f168:.2f}  i460={hi['i460']:.4f}  "
              f"i415={hi['i415']:.4f}  OBJ={o:.4f}{marker}")
        if o > best_v1_obj + 1e-6:
            best_v1_obj = o; best_v1 = v1_w
    print(f"  Best v1: {best_v1}  OBJ={best_v1_obj:.4f}")

    # ── Part C: HIGH idio_ratio fine-tune ────────────────────────────────────
    print(f"\n[4] Part C: HIGH idio_ratio fine-tune (v1={best_v1}, f168={best_f168}) ...")
    ratio_cands = [0.00, 0.10, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50,
                   0.55, 0.60, 0.65, 0.70, 0.80, 0.90, 1.00]
    best_ratio = cur_idio_ratio; best_ratio_obj = best_v1_obj
    for ratio in ratio_cands:
        w_test = make_high_w(weights, best_f168, best_v1, ratio)
        o = eval_weights(year_data, params, w_test)
        hi = w_test["HIGH"]
        marker = " <-" if o > best_ratio_obj + 1e-6 else ""
        print(f"  ratio={ratio:.2f}  i460={hi['i460']:.4f}  i415={hi['i415']:.4f}  "
              f"OBJ={o:.4f}{marker}")
        if o > best_ratio_obj + 1e-6:
            best_ratio_obj = o; best_ratio = ratio
    delta = best_ratio_obj - base_obj
    print(f"  Best ratio: {best_ratio}  OBJ={best_ratio_obj:.4f}  D={delta:+.4f}")

    # ── LOYO validation ──────────────────────────────────────────────────────
    best_w = make_high_w(weights, best_f168, best_v1, best_ratio)
    best_hi = best_w["HIGH"]
    print(f"\n[5] LOYO validation ...")
    loyo_wins = 0
    for yr in YEARS:
        s_base = yearly_sharpe(year_data[yr], params, weights)
        s_best = yearly_sharpe(year_data[yr], params, best_w)
    ...
    print(f"  Best HIGH: v1={best_v1}  f168={best_f168}  i460={best_hi['i460']}  i415={best_hi['i415']}")

    if loyo_wins >= 3 and delta > 0.005:
        print("\n  VALIDATED — updating config ...")
        cfg["breadth_regime_switching"]["regime_weights"]["HIGH"]["v1"]        = best_v1
        cfg["breadth_regime_switching"]["regime_weights"]["HIGH"]["f168"]       = best_f168
        cfg["breadth_regime_switching"]["regime_weights"]["HIGH"]["i460bw168"]  = best_hi["i460"]
        cfg["breadth_regime_switching"]["regime_weights"]["HIGH"]["i415bw216"]  = best_hi["i415"]
        # Version bump
        old_ver = cfg.get("version", "v2.42.4")
        ...
        try:
            ...
            msg = (f"feat: {new_ver} Phase247 HIGH retune — "
