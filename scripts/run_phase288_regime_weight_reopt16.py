"""
Phase 288 — Regime Weight Re-opt (16th Pass) [VECTORIZED]
=========================================================
Baseline: v2.49.12, OBJ=5.3345 (Phase 287 V1 weight resweep)

After P287 changed V1 internal weights (carry/mom/mr) for all regimes,
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase288"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase288_report.json").write_text(json.dumps(_partial, indent=2))
...
if __name__ == "__main__":
    t0 = time.time()
    print("=" * 65)
    print("Phase 288 — Regime Weight Re-opt (16th Pass) [VECTORIZED]")
    print("After P287 V1 internal weight changes")
    ...
    report = {
        ...
        "context": "After P287 V1 internal weight changes",
        "runtime_seconds": round(time.time() - t0, 1),
    }
    out = Path("artifacts/phase288"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase288_report.json").write_text(json.dumps(report, indent=2))
    ...
    if validated:
        ...
        msg = (f"feat: P288 regime weight reopt16 OBJ={reported_obj:.4f} "
