"""
Phase 320 — Per-Regime Vol + Disp Overlay Re-Tune [VECTORIZED]
================================================================
Baseline: v2.49.33, OBJ=6.2248 (P319 V1 weight resweep — no change)

After full cycle P315-P319 with zero validated improvements,
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase320"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase320_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 320 — Vol + Disp Overlay Re-Tune [VECTORIZED]")
    print("=" * 70)
    ...
    result = {
        "phase": "320", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    out = Path("artifacts/phase320"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase320_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        old_vver = cfg.get("version", "v2.49.33").lstrip("v")
        ...
        msg = (f"feat: P320 vol+disp retune OBJ={reported_obj:.4f} "
