"""
Phase 205 — V1 Mean Reversion Lookback Sweep
==============================================
After P203: w_mr raised from 0.25→0.30. MR contribution increased.
Optimal mean_reversion_lookback_bars may shift from current 84h.

Baseline: v2.32.0, OBJ=3.0860
Current: mean_reversion_lookback_bars=84
Sweep: [24, 36, 48, 60, 72, 84, 96, 120, 144, 168]
"""
...
def _on_timeout(signum, frame):
    ...
    (out / "phase205_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
BASELINE_OBJ = 3.0860
MR_LB_BASE   = 84
MR_LB_SWEEP  = [24, 36, 48, 60, 72, 84, 96, 120, 144, 168]
...
def run_v1(dataset, mr_lb):
    params = dict(V1_BASE, mean_reversion_lookback_bars=mr_lb)
    result = BacktestEngine(BacktestConfig(costs=COST_MODEL)).run(
...
def main():
    t0 = time.time()
    print("=" * 65)
    print("Phase 205 — V1 Mean Reversion Lookback Sweep")
    print(f"Baseline: v2.32.0  OBJ={BASELINE_OBJ}")
    print(f"Current: mr_lb={MR_LB_BASE}  (w_mr=0.30, up from 0.25)")
    print(f"Sweep: {MR_LB_SWEEP}")
    print("=" * 65)
    ...
    base_obj, base_yearly = evaluate_lb(MR_LB_BASE, all_data)
    print(f"\n  Baseline OBJ = {base_obj:.4f}  (expected ≈ {BASELINE_OBJ})")

    print("\n[2/3] Sweep...")
    best_obj = base_obj; best_lb = MR_LB_BASE
    results = []
    for lb in MR_LB_SWEEP:
        o, _ = evaluate_lb(lb, all_data)
        delta = o - base_obj
        marker = " ← BEST" if o > best_obj else ""
        print(f"  mr_lb={lb:4d}  OBJ={o:.4f}  Δ={delta:+.4f}{marker}")
        results.append((lb, o))
        if o > best_obj:
            best_obj = o; best_lb = lb

    print(f"\n  → Best: mr_lb={best_lb}  OBJ={best_obj:.4f}")
    ...
    print(f"\n{'✅ VALIDATED' if validated else '❌ NO IMPROVEMENT'} — mr_lb={best_lb}")

    result = {
        "phase": 205, "baseline_obj": float(base_obj), "best_obj": float(joint_obj),
        "delta": float(delta), "loyo_wins": loyo_wins, "loyo_total": len(joint_yearly),
        "validated": validated, "best_mr_lb": best_lb,
        "sweep_results": results,
    }
    out = Path("artifacts/phase205"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase205_report.json").write_text(json.dumps(result, indent=2))
