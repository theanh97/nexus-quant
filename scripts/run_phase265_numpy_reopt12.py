"""
Phase 265 NumPy — Regime Weight Re-opt (12th Pass) [VECTORIZED]
================================================================
Baseline: v2.49.0, OBJ=4.3173 (P252 per-regime DISP resweep)

P252 updated DISP params:
  LOW:  thr=0.50 scale=0.70 (was 0.70/0.50)
  MID:  thr=0.70 scale=1.80 (was 0.70/1.50)
  HIGH: thr=0.40 scale=0.50 (unchanged)

With new DISP params, optimal ensemble regime weights may shift.
Re-sweep from P263n baseline weights.
"""
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase265n"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase265n_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
# Baseline: P263n regime weights (v2.48.0)
REGIME_WEIGHTS_BASE = {
...
def main():
    t0 = time.time()
    print("="*65)
    print("Phase 265n — Regime Weight Re-opt (12th Pass) [VEC]")
    print(f"Baseline: v2.49.0  OBJ={BASELINE_OBJ}  (P252 DISP resweep)")
    print("="*65)
    ...
    result = {"phase":"265n","baseline_obj":float(base_obj),"best_obj":float(final_obj),
              "delta":float(delta),"loyo_wins":wins,"loyo_total":5,
              "validated":validated,"best_regime_weights":cur_w}
    _partial.update(result)
    out = Path("artifacts/phase265n"); out.mkdir(parents=True, exist_ok=True)
    (out/"phase265n_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        print(f"\n  Config → v2.50.0  OBJ={round(final_obj,4)}", flush=True)
        subprocess.run(["git","add",str(cfg_path)],cwd=ROOT)
        msg=f"feat: P265n regime weight reopt12 OBJ={round(final_obj,4)} LOYO={wins}/5 D={delta:+.4f} [v2.50.0]"
