"""
Phase 239 — F168 Funding Lookback Bars Sweep
=============================================
P238 validated per-regime ts_pct_win (Δ=+0.0207, LOYO 3/5, OBJ=3.7924).
F168 signal (contrarian funding momentum alpha) has never had its
core lookback parameter swept:
  - funding_lookback_bars = 168h (7 days) — always fixed

This is the dominant signal in MID regime (0.78 weight) and important
in LOW (0.37 weight). Getting the lookback right is material.

Pre-compute V1/I460/I415 (fixed), only vary F168 lookback_bars.

F168 funding_lookback sweep: [48, 72, 96, 120, 144, 168, 192, 240, 288, 336]
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase239"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase239_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
F168_LOOKBACK_BASE = 168
F168_BASE_PARAMS = {"k_per_side": 2, "funding_lookback_bars": 168, "direction": "contrarian",
                    "target_gross_leverage": 0.25, "rebalance_interval_bars": 36}

BASELINE_OBJ = 3.7924
F168_LB_SWEEP = [48, 72, 96, 120, 144, 168, 192, 240, 288, 336]
...
def load_year_data_fixed(year):
    ...
    # Per-regime ts_spread_pct
    tsp_cache = {}
    for r, win in TS_PCT_WINS.items():
        if win not in tsp_cache:
    ...
    # Fixed signals (no F168 yet — will add per combo)
    fixed_rets = {}
    ...
    return btc_vol, fund_std_pct, tsp_cache, brd_pct, fixed_rets, n, dataset

def compute_f168_rets(dataset, lookback_bars):
    params = dict(F168_BASE_PARAMS); params["funding_lookback_bars"] = lookback_bars
    result = BacktestEngine(BacktestConfig(costs=COST_MODEL)).run(
        dataset, make_strategy({"name": "funding_momentum_alpha", "params": params}))
    return np.array(result.returns)

def ensemble_rets(f168_rets, base_data):
    btc_vol, fund_std_pct, tsp_cache, brd_pct, fixed_rets, n, _ = base_data
    RNAMES = ["LOW", "MID", "HIGH"]
...
def evaluate_f168_lb(lb, all_data):
    yearly = {}
    for yr, bd in all_data.items():
        f168_rets = compute_f168_rets(bd[6], lb)
        yearly[yr] = sharpe(ensemble_rets(f168_rets, bd))
    return obj(yearly), yearly

def main():
    t0 = time.time()
    print("=" * 65)
    print("Phase 239 — F168 Funding Lookback Bars Sweep")
    print(f"Baseline: v2.42.0  OBJ={BASELINE_OBJ}")
    print(f"Current F168 lookback: {F168_LOOKBACK_BASE}h")
    print(f"Sweep: {F168_LB_SWEEP}")
    print("=" * 65)

    print("\n[1/3] Loading per-year data & pre-computing V1/I460/I415...")
    all_data = {}
    for yr in sorted(YEAR_RANGES):
        print(f"  {yr}: ", end="", flush=True)
        all_data[yr] = load_year_data_fixed(yr)

    # Baseline
    base_f168_rets = {yr: compute_f168_rets(all_data[yr][6], F168_LOOKBACK_BASE) for yr in all_data}
    base_yearly = {yr: sharpe(ensemble_rets(base_f168_rets[yr], all_data[yr])) for yr in all_data}
    ...
    print("\n[2/3] F168 lookback sweep...")
    best_lb = F168_LOOKBACK_BASE; best_obj_val = base_obj_val
    results = []
    for lb in F168_LB_SWEEP:
        o, _ = evaluate_f168_lb(lb, all_data)
        delta = o - base_obj_val
        marker = " ← BEST" if o > best_obj_val else ""
        print(f"  lb={lb:3d}h  OBJ={o:.4f}  Δ={delta:+.4f}{marker}")
        results.append((lb, o))
        if o > best_obj_val: best_obj_val = o; best_lb = lb

    print(f"\n  → Best: lb={best_lb}h  OBJ={best_obj_val:.4f}")

    print(f"\n[3/3] LOYO validation...")
    joint_obj_val, joint_yearly = evaluate_f168_lb(best_lb, all_data)
    ...
    result = {
        "phase": 239, "baseline_obj": float(base_obj_val), "best_obj": float(joint_obj_val),
        "delta": float(delta), "loyo_wins": loyo_wins, "loyo_total": len(joint_yearly),
        "validated": validated, "best_f168_lookback": best_lb,
        "all_results": results,
    }
    _partial.update(result)
    out = Path("artifacts/phase239"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase239_report.json").write_text(json.dumps(result, indent=2))

    if validated:
        cfg_path = ROOT / "configs" / "production_p91b_champion.json"
        cfg = json.load(open(cfg_path))
        for sk, sv in cfg["ensemble"]["signals"].items():
            if sv.get("name") == "funding_momentum_alpha":
                sv["params"]["funding_lookback_bars"] = best_lb
                break
        cfg["_version"] = "2.43.0"
