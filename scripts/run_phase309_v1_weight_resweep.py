"""
Phase 309 — Per-Regime V1 Internal Weight Re-Sweep [VECTORIZED]
================================================================
Baseline: v2.49.28, OBJ=5.9612 (Phase 304 regime weight reopt)

V1 internal weights (carry/mom/mean_reversion) last optimized in P304.
Since then: P305 vol/disp, P306 FTS, P307 breadth, P308 regime weights.
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase309"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase309_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 309 — Per-Regime V1 Internal Weight Re-Sweep")
    print("Baseline: v2.49.28, OBJ=5.9612")
    print("=" * 70)
    ...
    # [7] Save report
    result = {
        "phase": "309", "title": "Per-Regime V1 Weight Re-Sweep",
    ...
    out = Path("artifacts/phase309"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase309_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase309/phase309_report.json")
    ...
    if validated:
        ...
        msg = (f"feat: P309 V1 weight resweep OBJ={reported_obj:.4f} "
