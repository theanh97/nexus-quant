"""
Phase 354 — Per-Regime V1 Internal Weight Re-Sweep [VECTORIZED]
================================================================
Baseline: v2.49.40, OBJ=6.7382 (P353 regime weight reopt VALIDATED)

V1 internal weights (carry/mom/mean_reversion) — re-sweep after
P350-P353 confirmed all other params optimal.

Current (from P353 validated):
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase354"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase354_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 354 — Per-Regime V1 Internal Weight Re-Sweep")
    print("=" * 70)
    ...
    result = {
        "phase": "354", "title": "Per-Regime V1 Weight Re-Sweep",
    ...
    out = Path("artifacts/phase354"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase354_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase354/phase354_report.json")
