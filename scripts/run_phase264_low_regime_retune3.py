#!/usr/bin/env python3
"""
Phase 264 — LOW Regime Retune Round 3
Current LOW: v1=0.40, i460bw168=0.00, i415bw216=0.20, f168=0.40
LOW boundary: brd_pct < 0.30 (unchanged by P261)
Sweep: A) f168, B) v1, C) i415 weight
"""
...
def make_low_w(base_weights, f168_w, v1_w, i415_w):
    remainder = max(0.0, 1.0 - v1_w - f168_w - i415_w)
    i460_w = round(max(0.0, remainder), 4)
...
def main():
    t0 = time.time()
    print("=" * 60)
    print("Phase 264 — LOW Regime Retune Round 3")
    print("Config: v2.42.14 (or updated by P263)")
    print("=" * 60)
    ...
    print(f"  Current LOW: v1={curr_low['v1']:.2f} i460={curr_low['i460']:.2f} "
          f"i415={curr_low['i415']:.2f} f168={curr_low['f168']:.2f}")
    ...
    for fw in f168_candidates:
        w_test = make_low_w(base_weights, fw, curr_low["v1"], curr_low["i415"])
    ...
    # [3] v1 sweep
    print("\n[3] v1 sweep ...")
    v1_candidates = [0.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50, 0.60]
    best_v1 = curr_low["v1"]; best_v1_obj = best_f168_obj
    for vw in v1_candidates:
        if vw + best_f168 > 1.0: continue
        w_test = make_low_w(base_weights, best_f168, vw, curr_low["i415"])
        obj = eval_weights(year_data, params, w_test)
    ...
    # [4] i415 sweep
    print("\n[4] i415 sweep ...")
    i415_candidates = [0.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50]
    best_i415 = curr_low["i415"]; best_i415_obj = best_v1_obj
    for iw in i415_candidates:
        if best_v1 + best_f168 + iw > 1.0: continue
        w_test = make_low_w(base_weights, best_f168, best_v1, iw)
        obj = eval_weights(year_data, params, w_test)
    ...
    # [5] LOYO validation
    print("\n[5] LOYO validation ...")
    best_weights = make_low_w(base_weights, best_f168, best_v1, best_i415)
    wins = 0
    for yr in YEARS:
    ...
    print(f"  Best LOW: v1={bw['v1']:.2f} i460={bw['i460']:.3f} i415={bw['i415']:.2f} f168={bw['f168']:.2f}")

    if wins >= 3 and delta > 0.005:
        print(f"\n  VALIDATED  (LOYO {wins}/5 delta={delta:+.4f})")
        _update_config(cfg, bw, best_i415_obj)
    else:
...
def _update_config(cfg, bw, new_obj):
    ...
    subprocess.run(["git", "commit", "-m",
        f"quant: {new_ver} — LOW regime retune r3 (v1={bw['v1']:.2f} f168={bw['f168']:.2f}) OBJ={new_obj:.4f}"],
