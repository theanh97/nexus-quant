"""
Phase 212 — V1 k_per_side + Target Leverage Sweep
====================================================
V1 uses k=2 (top 2 long + bottom 2 short = 4 positions from 10 symbols).
That's only 40% of universe coverage — quite concentrated.

Also: target_gross_leverage=0.35 for V1. Could be sub-optimal.

Part A: V1 k_per_side ∈ [1, 2, 3, 4, 5]
  - Load I460, I415, F168 once (fixed)
  - Re-run V1 for each k value
Part B: V1 target_gross_leverage ∈ [0.25, 0.30, 0.35, 0.40, 0.45]
  - Using best k from Part A
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase212"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase212_report.json").write_text(json.dumps(_partial, indent=2, default=str))
    sys.exit(0)
...
V1_K_SWEEP  = [1, 2, 3, 4, 5]
V1_LEV_SWEEP = [0.25, 0.30, 0.35, 0.40, 0.45]
...
def get_config():
    ...
    params["v1_k"] = v1_p.get("k_per_side", 2)
    params["v1_lev"] = v1_p.get("target_gross_leverage", 0.35)
    ...
    baseline_obj = float(m[-1]) if m else 3.0860
...
def run_v1(dataset, k, lev, params):
    p = params
    sp = {
        "k_per_side": k, "w_carry": p["v1_wc"], "w_mom": p["v1_wm"],
        "w_mean_reversion": p["v1_wr"],
        "momentum_lookback_bars": p["v1_mom_lb"],
        "mean_reversion_lookback_bars": p["v1_mr_lb"],
        "vol_lookback_bars": p["v1_vol_lb"],
...
# ── MAIN ────────────────────────────────────────────────────────────────────
print("=" * 60)
print("Phase 212 — V1 k_per_side + Target Leverage Sweep")
print("=" * 60)
_start = time.time()

prod_cfg, WEIGHTS, PARAMS, baseline_obj = get_config()
ver_now = prod_cfg.get("_version", "2.32.0")
curr_v1_k = PARAMS["v1_k"]
curr_v1_lev = PARAMS["v1_lev"]
print(f"  Config: {ver_now}  baseline_obj={baseline_obj:.4f}")
print(f"  V1 now: k={curr_v1_k}  lev={curr_v1_lev}")
...
# Part A: V1 k sweep (lev fixed)
print(f"\n[2] Part A: V1 k_per_side sweep (lev={curr_v1_lev} fixed) ...")
results_a = []
for k in V1_K_SWEEP:
    yr_sharpes = {}
    for yr in YEARS:
        v1_rets = run_v1(year_data[yr][0], k, curr_v1_lev, PARAMS)
    ...
    print(f"  V1 k={k}  OBJ={obj:.4f}")

best_a = max(results_a, key=lambda x: x[1])
best_v1_k, best_obj_a, _ = best_a
print(f"  Best V1 k: {best_v1_k}  OBJ={best_obj_a:.4f}")

# Part B: V1 leverage sweep (using best k)
print(f"\n[3] Part B: V1 leverage sweep (k={best_v1_k}) ...")
results_b = []
for lev in V1_LEV_SWEEP:
    yr_sharpes = {}
    for yr in YEARS:
        v1_rets = run_v1(year_data[yr][0], best_v1_k, lev, PARAMS)
        ens = compute_ensemble(year_data[yr], WEIGHTS, PARAMS, v1_rets)
        yr_sharpes[yr] = sharpe(ens, year_data[yr][-1])
    obj = obj_fn(yr_sharpes)
    results_b.append((lev, obj, yr_sharpes))
    print(f"  V1 lev={lev:.2f}  OBJ={obj:.4f}")

best_b = max(results_b, key=lambda x: x[1])
best_v1_lev, best_obj_b, best_yr = best_b
delta = best_obj_b - baseline_obj
print(f"  Best V1 lev: {best_v1_lev:.2f}  OBJ={best_obj_b:.4f}  Δ={delta:+.4f}")

curr_a = next((r for r in results_a if r[0] == curr_v1_k), None)
curr_b = next((r for r in results_b if r[0] == curr_v1_lev), None)
...
out = Path("artifacts/phase212"); out.mkdir(parents=True, exist_ok=True)
report = {
    "phase": 212, "description": "V1 k_per_side + target leverage sweep",
    "elapsed_seconds": round(time.time() - _start, 1),
    "baseline_obj": baseline_obj, "best_obj": best_obj_b, "delta": delta,
    "curr_v1_k": curr_v1_k, "best_v1_k": best_v1_k,
    "curr_v1_lev": curr_v1_lev, "best_v1_lev": best_v1_lev,
    "loyo_wins": loyo_wins, "validated": validated,
    "yearly_best": best_yr, "yearly_base": base_yr,
    "k_sweep": [(r[0], r[1]) for r in results_a],
    "lev_sweep": [(r[0], r[1]) for r in results_b],
}
(out / "phase212_report.json").write_text(json.dumps(report, indent=2, default=str))
...
if validated:
    ...
    v1_sig.get("params", {})["k_per_side"] = best_v1_k
    v1_sig.get("params", {})["target_gross_leverage"] = best_v1_lev
    ...
    cfg["_validated"] = old_val + (
        "; V1 k+lev P212: k=%d->%d lev=%.2f->%.2f LOYO %d/5 delta=%+.4f OBJ=%.4f — PRODUCTION %s" % (
            curr_v1_k, best_v1_k, curr_v1_lev, best_v1_lev,
    ...
    cm = "phase212: V1 k+lev -- VALIDATED k=%d lev=%.2f LOYO %d/5 delta=%+.4f OBJ=%.4f" % (
        best_v1_k, best_v1_lev, loyo_wins, delta, best_obj_b)
else:
    new_ver = ver_now
    print(f"\n⚠️ NO IMPROVEMENT — V1 k+lev near-optimal (LOYO {loyo_wins}/5 delta={delta:+.4f})")
    cm = "phase212: V1 k+lev -- WEAK LOYO %d/5 delta=%+.4f V1_k=%d lev=%.2f" % (
        loyo_wins, delta, best_v1_k, best_v1_lev)

os.system("git add configs/production_p91b_champion.json scripts/run_phase212_v1_k_leverage_sweep.py "
          "artifacts/phase212/ 2>/dev/null || true")
os.system('git commit -m "' + cm + '"')
os.system("git stash && git pull --rebase && git stash pop && git push")

print(f"\n[DONE] Phase 212 complete.")
