"""
Phase 213 — DISP Overlay Sweep (DISP_THR × DISP_SCALE)
=========================================================
Currently: DISP_THR=0.60, DISP_SCALE=1.0 → overlay DISABLED (no effect).
With F168 now 78% MID: high funding dispersion = more cross-sectional
spread = better contrarian F168 opportunity. Boosting (DISP_SCALE>1.0)
when fsp > THR might help. Also try dampening (scale<1.0).
All signals pre-computed — pure NumPy sweep (very fast).

Baseline: v2.32.0, OBJ=3.0860
Sweep: scale ∈ [0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 1.00, 1.10, 1.20, 1.30, 1.40, 1.50]
...
def _on_timeout(signum, frame):
    ...
    (out / "phase213_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
DISP_THR_BASE   = 0.60; DISP_SCALE_BASE = 1.0  # currently disabled
...
BASELINE_OBJ  = 3.0860
SCALE_SWEEP   = [0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 1.00, 1.10, 1.20, 1.30, 1.40, 1.50]
THR_SWEEP     = [0.45, 0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80]
...
def main():
    t0 = time.time()
    print("=" * 65)
    print("Phase 213 — DISP Overlay Sweep (scale × threshold)")
    print(f"Baseline: v2.32.0  OBJ={BASELINE_OBJ}")
    print(f"Current: DISP_THR={DISP_THR_BASE} DISP_SCALE={DISP_SCALE_BASE} (DISABLED)")
    print(f"Scale: {SCALE_SWEEP}")
    print(f"THR:   {THR_SWEEP}")
    print("=" * 65)
    ...
    base_obj, base_yearly = evaluate(DISP_THR_BASE, DISP_SCALE_BASE, all_data)
    print(f"\n  Baseline OBJ = {base_obj:.4f}  (expected ≈ {BASELINE_OBJ})")
    _partial.update({"baseline_obj": float(base_obj)})

    print("\n[2/3] 2D Sweep...")
    best_obj = base_obj; best_thr = DISP_THR_BASE; best_scale = DISP_SCALE_BASE
    results = []
    for scale in SCALE_SWEEP:
        for thr in THR_SWEEP:
            o, _ = evaluate(thr, scale, all_data)
            delta = o - base_obj
    ...
    print("\n[3/3] LOYO validation...")
    joint_obj, joint_yearly = evaluate(best_thr, best_scale, all_data)
    ...
    result = {
        "phase": 213, "baseline_obj": float(base_obj), "best_obj": float(joint_obj),
        "delta": float(delta), "loyo_wins": loyo_wins, "loyo_total": len(joint_yearly),
        "validated": validated, "best_disp_scale": best_scale, "best_disp_thr": best_thr,
        "top5": sorted(results, key=lambda x: -x[2])[:5],
    }
    _partial.update(result)
    out = Path("artifacts/phase213"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase213_report.json").write_text(json.dumps(result, indent=2))

    if validated:
        cfg_path = ROOT / "configs" / "production_p91b_champion.json"
        cfg = json.load(open(cfg_path))
        do = cfg.setdefault("disp_overlay_params", {})
        do["disp_thr"]   = best_thr
        do["disp_scale"] = best_scale
        cfg["_version"] = "2.33.0"
