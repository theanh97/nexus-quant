"""
Phase 314 — Per-Regime V1 Internal Weight Re-Sweep [VECTORIZED]
================================================================
Baseline: v2.49.31, OBJ=6.0391 (Phase 309 parallel session)

V1 internal weights (carry/mom/mean_reversion) — re-sweep after
P311-P313 confirmed all other params optimal.

Current (from P308 parallel session):
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase314"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase314_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 314 — Per-Regime V1 Internal Weight Re-Sweep")
    print("=" * 70)
    ...
    result = {
        "phase": "314", "title": "Per-Regime V1 Weight Re-Sweep",
    ...
    out = Path("artifacts/phase314"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase314_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase314/phase314_report.json")
    ...
    if validated:
        ...
        old_ver = cfg.get("_version", "2.49.31")
        parts = old_ver.split(".")
        parts[-1] = str(int(parts[-1]) + 1)
        new_ver = ".".join(parts)
        cfg["_version"] = new_ver
        old_vver = cfg.get("version", "v2.49.31").lstrip("v")
        ...
        msg = (f"feat: P314 V1 weight resweep OBJ={reported_obj:.4f} "
