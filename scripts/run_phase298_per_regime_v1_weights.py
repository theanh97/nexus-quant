"""
Phase 298 — Per-Regime V1 Internal Weights Re-Sweep [VECTORIZED, MEM-EFFICIENT]
================================================================================
Baseline: v2.49.21, OBJ=5.6221 (P293-P297 cycle complete)

New cycle P298-P302 after P293-P297 advanced OBJ 5.4545→5.6221. V1 internals
may have shifted after regime weights (P294), vol+disp (P295-ext), V1 (P293-ext).

Current V1 weights:
  LOW:  wc=0.15, wm=0.45, wmr=0.40  (balanced)
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase298"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase298_report.json").write_text(json.dumps(_partial, indent=2))
...
if __name__ == "__main__":
    t0 = time.time()
    print("=" * 70)
    print("Phase 298 — Per-Regime V1 Weights Re-Sweep [MEM-EFFICIENT]")
    print("Baseline: v2.49.21 OBJ=5.6221 (P293-P297 cycle)")
    print("=" * 70)
    ...
    result = {
        "phase": "298", "baseline_obj": float(obj_base), "stored_baseline": float(base_obj),
    ...
    out = Path("artifacts/phase298"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase298_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        msg = (f"feat: P298 per-regime V1 weights OBJ={reported_obj:.4f} "
