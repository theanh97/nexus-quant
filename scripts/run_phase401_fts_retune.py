"""
Phase 401 — Per-Regime FTS Retune [VECTORIZED]
============================================================================
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase401_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase401_fts_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 401 — FTS Retune [VECTORIZED]")
    print("=" * 70)
    ...
    out = Path("artifacts/phase401_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase401_fts_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase401_fts/phase401_fts_report.json")
    ...
    if validated:
        ...
        msg = (f"feat: P401 FTS retune OBJ={reported_obj} "
