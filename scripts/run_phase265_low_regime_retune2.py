#!/usr/bin/env python3
"""
Phase 265 — LOW Regime Re-retune Round 2
Current LOW: v1=0.4, i460=0.0, i415=0.2, f168=0.4 (from parallel P260)
LOW boundary unchanged (brd_pct < 0.30).
Sweep: A) f168, B) v1, C) i415 weight, D) i460 weight
"""
...
def make_low_w(base_weights, v1_w, f168_w, i415_w, i460_w):
    """Build LOW weights; total must be <= 1.0"""
    total = v1_w + f168_w + i415_w + i460_w
    if total > 1.001:
        return None
    return {"MID": base_weights["MID"], "HIGH": base_weights["HIGH"],
            "LOW": {"v1": v1_w, "i460": i460_w, "i415": i415_w, "f168": f168_w}}

def main():
    t0 = time.time()
    print("=" * 60)
    print("Phase 265 — LOW Regime Re-retune Round 2")
    print("Re-optimizing under current config state")
    print("=" * 60)

    cfg, weights, params, v1p, i460p, i415p, f168p = get_config()
    base_weights = weights
    curr_low = weights["LOW"]

    print(f"\n  Config: {cfg.get('version')}")
    print(f"  Current LOW: v1={curr_low['v1']:.3f} i460={curr_low['i460']:.3f} "
          f"i415={curr_low['i415']:.3f} f168={curr_low['f168']:.3f}")
    ...
    # [2] f168 sweep
    print("\n[2] f168 sweep ...")
    f168_candidates = [0.00, 0.10, 0.20, 0.30, 0.35, 0.40, 0.45, 0.50,
                       0.55, 0.60, 0.65, 0.70, 0.80, 0.90, 1.00]
    best_f168 = curr_low["f168"]; best_f168_obj = baseline_obj
    for fw in f168_candidates:
        total = curr_low["v1"] + fw + curr_low["i415"] + curr_low["i460"]
        if total > 1.001: continue
        w_test = make_low_w(base_weights, curr_low["v1"], fw, curr_low["i415"], curr_low["i460"])
        if w_test is None: continue
    ...
    print(f"  Best f168: {best_f168:.2f}  OBJ={best_f168_obj:.4f}  D={best_f168_obj-baseline_obj:+.4f}")

    # [3] v1 sweep
    print("\n[3] v1 sweep ...")
    v1_candidates = [0.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50, 0.55, 0.60]
    best_v1 = curr_low["v1"]; best_v1_obj = best_f168_obj
    for vw in v1_candidates:
        total = vw + best_f168 + curr_low["i415"] + curr_low["i460"]
        if total > 1.001: continue
        w_test = make_low_w(base_weights, vw, best_f168, curr_low["i415"], curr_low["i460"])
        if w_test is None: continue
        obj = eval_weights(year_data, params, w_test)
    ...
    print(f"  Best v1: {best_v1:.2f}  OBJ={best_v1_obj:.4f}  D={best_v1_obj-baseline_obj:+.4f}")

    # [4] i415 sweep
    print("\n[4] i415 sweep ...")
    i415_candidates = [0.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40]
    best_i415 = curr_low["i415"]; best_i415_obj = best_v1_obj
    for iw in i415_candidates:
        total = best_v1 + best_f168 + iw + curr_low["i460"]
        if total > 1.001: continue
        w_test = make_low_w(base_weights, best_v1, best_f168, iw, curr_low["i460"])
        if w_test is None: continue
        obj = eval_weights(year_data, params, w_test)
        marker = " <-" if obj > best_i415_obj else ""
        print(f"  i415={iw:.2f}  OBJ={obj:.4f}{marker}")
        if obj > best_i415_obj:
            best_i415_obj = obj; best_i415 = iw
    print(f"  Best i415: {best_i415:.2f}  OBJ={best_i415_obj:.4f}  D={best_i415_obj-baseline_obj:+.4f}")

    # [5] LOYO validation
    print("\n[5] LOYO validation ...")
    best_weights = make_low_w(base_weights, best_v1, best_f168, best_i415, 0.0)
    wins = 0
    for yr in YEARS:
    ...
    delta = best_i415_obj - baseline_obj
    bl = best_weights["LOW"]
    print(f"\n  LOYO: {wins}/5 wins  delta={delta:+.4f}")
    print(f"  Best LOW: v1={bl['v1']:.3f} i460={bl['i460']:.3f} i415={bl['i415']:.3f} f168={bl['f168']:.3f}")

    if wins >= 3 and delta > 0.005:
        print(f"\n  VALIDATED  (LOYO {wins}/5 delta={delta:+.4f})")
        _update_config(cfg, bl, best_i415_obj)
    else:
        print(f"\n  CONFIRMED OPTIMAL  (LOYO {wins}/5 delta={delta:+.4f})")

    print(f"\n[DONE] Phase 265 complete.  ({time.time()-t0:.1f}s)")

def _update_config(cfg, bl, new_obj):
    old_ver = cfg.get("version", "v2.42.15")
    parts = old_ver.lstrip("v").split(".")
    parts[-1] = str(int(parts[-1]) + 1)
    new_ver = "v" + ".".join(parts)
    rw = cfg["breadth_regime_switching"]["regime_weights"]["LOW"]
    rw["v1"]        = round(bl["v1"], 4)
    rw["i460bw168"] = round(bl["i460"], 4)
    rw["i415bw216"] = round(bl["i415"], 4)
    rw["f168"]      = round(bl["f168"], 4)
    cfg["version"] = new_ver
    p = Path("configs/production_p91b_champion.json")
    p.write_text(json.dumps(cfg, indent=2))
    print(f"  Config updated: {old_ver} → {new_ver}")
    print(f"  LOW: v1={bl['v1']:.3f} i460={bl['i460']:.3f} i415={bl['i415']:.3f} f168={bl['f168']:.3f}")
    print(f"  New OBJ={new_obj:.4f}")
    subprocess.run(["git", "add", str(p)], cwd="/Users/truonglys/projects/quant")
    subprocess.run(["git", "commit", "-m",
        f"quant: {new_ver} — LOW retune r2 (v1={bl['v1']:.2f} f168={bl['f168']:.2f} i415={bl['i415']:.2f}) OBJ={new_obj:.4f}"],
