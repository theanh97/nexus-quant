"""
Phase 315 — Per-Regime Vol + Disp Overlay Re-Tune [VECTORIZED]
================================================================
Baseline: v2.49.32, OBJ=6.1979 (P314 V1 weight resweep)

After P314 changed LOW V1 weights (wc=0.15 wm=0.45 wmr=0.40),
re-tune vol/disp overlays with finer grids around current values.

Current:
  Vol thresh: LOW=0.54, MID=0.50, HIGH=0.57
  Vol scale:  LOW=0.45, MID=0.18, HIGH=0.003
  Disp thresh: LOW=0.75, MID=0.40, HIGH=0.30
  Disp scale:  LOW=0.60, MID=2.00, HIGH=0.40

Approach: 2-pass coordinate descent (vol_thr→vol_scale→disp_thr→disp_scale per regime)
          + disp percentile window sweep
"""
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase315"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase315_report.json").write_text(json.dumps(_partial, indent=2))
...
# Grids — finer around current values
# Current: vol_thr LOW=0.54 MID=0.50 HIGH=0.57, vol_scale LOW=0.45 MID=0.18 HIGH=0.003
VOL_THR_GRID   = [0.46, 0.48, 0.50, 0.51, 0.52, 0.53, 0.54, 0.55, 0.56, 0.57, 0.58, 0.59, 0.60]
VOL_SCALE_GRID = [0.001, 0.003, 0.005, 0.01, 0.03, 0.05, 0.08, 0.10, 0.15, 0.18, 0.20, 0.25, 0.30, 0.40, 0.45, 0.50]
# Current: disp_thr LOW=0.75 MID=0.40 HIGH=0.30, disp_scale LOW=0.60 MID=2.00 HIGH=0.40
DISP_THR_GRID  = [0.25, 0.30, 0.35, 0.40, 0.45, 0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85]
DISP_SCALE_GRID = [0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.50, 0.60, 0.70, 0.80, 1.0, 1.5, 1.8, 2.0, 2.5, 3.0]
...
def load_config_params():
    ...
    return {
        "p_low": float(brs.get("p_low", 0.30)), "p_high": float(brs.get("p_high", 0.60)),
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 315 — Vol + Disp Overlay Re-Tune [VECTORIZED]")
    print("=" * 70)
    ...
    for pass_num in [1, 2]:
        ...
        for rn in RNAMES:
            ...
            print(f"    {rn}: vt={cur_vt[rn]:.2f} vs={cur_vs[rn]:.3f} "
                  f"dt={cur_dt[rn]:.2f} ds={cur_ds[rn]:.2f}  "
    ...
    result = {
        "phase": "315", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    out = Path("artifacts/phase315"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase315_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        old_ver = cfg.get("_version", "2.49.32")
        parts = old_ver.split(".")
        parts[-1] = str(int(parts[-1]) + 1)
        new_ver = ".".join(parts)
        cfg["_version"] = new_ver
        old_vver = cfg.get("version", "v2.49.32").lstrip("v")
        ...
        msg = (f"feat: P315 vol+disp retune OBJ={reported_obj:.4f} "
