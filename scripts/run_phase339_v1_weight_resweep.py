"""
Phase 339 — Per-Regime V1 Internal Weight Re-Sweep [VECTORIZED]
================================================================
Baseline: v2.49.40, OBJ=6.7382 (P338 regime weight reopt — no change)

V1 internal weights (carry/mom/mean_reversion) — re-sweep after
P335-P338 confirmed all other params optimal.

Current (from P338 confirmed optimal):
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase339"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase339_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 339 — Per-Regime V1 Internal Weight Re-Sweep")
    print("=" * 70)
    ...
    result = {
        "phase": "339", "title": "Per-Regime V1 Weight Re-Sweep",
    ...
    out = Path("artifacts/phase339"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase339_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase339/phase339_report.json")
