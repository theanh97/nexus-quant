"""
Phase 172 — I460 Beta Window Fine-Tune
========================================
I460's beta_window_bars=168 has never been swept independently.
P163 confirmed I415's optimal bw=216 (much larger than initial bw=168).
Since I460 has a longer lookback (460 vs 415), its optimal beta
de-trending window may also differ from the default bw=168.

Sweep: beta_window_bars ∈ [96, 120, 144, 168, 192, 216, 240]
Baseline (v2.12.0): I460bw168, OBJ=2.4148

Design: V1, I415bw216, F168 run once per year (shared).
        I460 re-run for each beta_window variant.
"""
...
# Fixed overlay params (v2.12.0)
VOL_WINDOW     = 168
VOL_THRESHOLD  = 0.5
VOL_SCALE      = 0.4
VOL_F168_BOOST = 0.10  # updated v2.13.0
BRD_LOOKBACK   = 192
PCT_WINDOW     = 336
...
BASELINE_BW = 168
BW_SWEEP    = [96, 120, 144, 168, 192, 216, 240]
...
def _on_timeout(signum, frame):
    ...
    (OUT_DIR / "phase172_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
print("=" * 68)
print("PHASE 172 — I460 Beta Window Fine-Tune")
print("=" * 68)
print(f"  Baseline (v2.13.0): I460 bw={BASELINE_BW}, OBJ≈2.4153")
print(f"  Sweep bw: {BW_SWEEP}\n")

print("[1/3] Loading data + signals + shared strategy returns (V1, I415bw216, F168)...")
shared_rets: dict = {"v1": {}, "i415bw216": {}, "f168": {}}
i460_rets_by_bw: dict = {bw: {} for bw in BW_SWEEP}
signals_data: dict = {}
datasets: dict = {}
...
for year, (start, end) in YEAR_RANGES.items():
    ...
    # I460 variants for all bw values
    for bw in BW_SWEEP:
        bt_cfg = BacktestConfig(costs=COST_MODEL)
        ...
        result = BacktestEngine(bt_cfg).run(dataset, strat)
        i460_rets_by_bw[bw][year] = np.array(result.returns)
    print(f". ✓ [{len(BW_SWEEP)} I460 variants]")

_partial.update({"phase": 172, "description": "I460 Beta Window Fine-Tune", "partial": True})
...
for bw in BW_SWEEP:
    ...
    for year in YEARS:
        sr = {
            ...
            "i460bw168": i460_rets_by_bw[bw][year],  # varies
...
# Now compute deltas relative to baseline
...
for bw in BW_SWEEP:
    ...
    print(f"  {sym} bw={bw}: OBJ={r['obj']:.4f} (Δ={d:+.4f}){tag}")
...
print("\n" + "=" * 68)
if validated:
    print(f"✅ VALIDATED — I460 bw={best_bw}: OBJ={best_obj:.4f} (Δ={best_delta:+.4f}) LOYO {loyo_wins}/5")
else:
    print(f"❌ NO IMPROVEMENT — I460 bw={BASELINE_BW} OBJ={baseline_obj:.4f} optimal")
    print(f"   Best: bw={best_bw} Δ={best_delta:+.4f} | LOYO {loyo_wins}/5")
print("=" * 68)

report = {
    "phase": 172,
    "description": "I460 Beta Window Fine-Tune",
    ...
    "validated": validated,
    "verdict": (
        f"VALIDATED — I460 bw={best_bw} OBJ={best_obj:.4f} Δ={best_delta:+.4f} LOYO {loyo_wins}/5"
        if validated else
        f"NO IMPROVEMENT — I460 bw={BASELINE_BW} OBJ={baseline_obj:.4f} optimal"
    ),
    "partial": False,
    "timestamp": datetime.datetime.now(datetime.UTC).isoformat(),
}

out_path = OUT_DIR / "phase172_report.json"
