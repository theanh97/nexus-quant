"""
Phase 411 — Per-Regime FTS Retune [VECTORIZED]
============================================================================
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase411_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase411_fts_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 411 — FTS Retune [VECTORIZED]")
    print("=" * 70)
    ...
    out = Path("artifacts/phase411_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase411_fts_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase411_fts/phase411_fts_report.json")
    ...
    if validated:
        ...
        msg = (f"feat: P411 FTS retune OBJ={reported_obj} "
