"""
Phase 193 — I415 Lookback Re-Sweep (post regime weight changes P189-P190)
=========================================================================
Motivation: I415 is the dominant signal in HIGH regime (54%) and significant
in MID (19%). With V1 now dominant in LOW/MID, I415 optimal lookback
may have shifted from 415h.

Sweep:
  lookback_bars ∈ [300, 350, 380, 415, 460, 500, 540, 600]
...
def _on_timeout(signum, frame):
    ...
    (out / "phase193_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
I415_LB_BASE = 415
BASELINE_OBJ = 2.8660
SWEEP_LBS    = [300, 350, 380, 415, 460, 500, 540, 600]
...
def load_year_data(year: int):
    ...
    for sk, sname, params in [
        ...
        ("i415_base", "idio_momentum_alpha", {
            "k_per_side": 4, "lookback_bars": I415_LB_BASE, "beta_window_bars": 144,
            "target_gross_leverage": 0.3, "rebalance_interval_bars": 48,
        }),
...
def run_i415_lb(base_data, lb: int) -> np.ndarray:
    dataset, btc_vol, fund_std_pct, ts_spread_pct, regime, sig_rets, n = base_data
    if lb == I415_LB_BASE:
        i415_rets = sig_rets["i415_base"]
    else:
        result = BacktestEngine(BacktestConfig(costs=COST_MODEL)).run(
            dataset,
            make_strategy({"name": "idio_momentum_alpha", "params": {
                "k_per_side": 4, "lookback_bars": lb, "beta_window_bars": 144,
                "target_gross_leverage": 0.3, "rebalance_interval_bars": 48,
            }}))
        i415_rets = np.array(result.returns)

    all_rets = {"v1": sig_rets["v1"], "i460": sig_rets["i460"],
                "i415": i415_rets, "f168": sig_rets["f168"]}
...
def main():
    t0 = time.time()
    print("=" * 65)
    print("Phase 193 — I415 Lookback Re-Sweep")
    print(f"Baseline: v2.22.0  OBJ={BASELINE_OBJ}  i415_lb={I415_LB_BASE}")
    ...
    bl_yr = {yr: sharpe(run_i415_lb(data_by_yr[yr], I415_LB_BASE)) for yr in YEARS}
    bl_obj = obj(bl_yr)
    print(f"  Baseline OBJ = {bl_obj:.4f}  (expected ≈ {BASELINE_OBJ})")

    print(f"\n[2/3] Sweeping I415 lookback_bars ...")
    sweep_results: dict = {}
    best_lb, best_obj_val = I415_LB_BASE, bl_obj
    best_yr_s = bl_yr

    for lb in SWEEP_LBS:
        yr_s = {yr: sharpe(run_i415_lb(data_by_yr[yr], lb)) for yr in YEARS}
        ...
        flag = ("⭐" if (o > best_obj_val and loyo_wins >= 3 and d > 0.005) else
                ...
                ("⚠️ " if lb == I415_LB_BASE else "❌")))
        base_m = " ← baseline" if lb == I415_LB_BASE else ""
        ...
        if o > best_obj_val and loyo_wins >= 3 and d > 0.005:
            best_lb, best_obj_val, best_yr_s = lb, o, yr_s

    print(f"\n[3/3] LOYO validation — best_lb={best_lb}  OBJ={best_obj_val:.4f}")
    if best_lb == I415_LB_BASE:
        print("  ❌ NO IMPROVEMENT — i415_lb=415 already optimal")
        ...
        for loyo_yr in YEARS:
            ...
            d = (obj({y: best_yr_s[y] for y in train_yrs}) -
    ...
    if validated:
        print(f"\n✅ VALIDATED — i415_lb={best_lb}  OBJ={best_obj_val:.4f}")
    else:
        print(f"\n❌ NO IMPROVEMENT — i415_lb={I415_LB_BASE}  OBJ={bl_obj:.4f} optimal")

    report = {
        "phase": 193, "target": "i415_lookback_bars",
        "sweep": sweep_results,
        "baseline": {"obj": bl_obj, "lb": I415_LB_BASE},
        "best": {"obj": best_obj_val, "lb": best_lb},
        "loyo": {"wins": loyo_wins_final if best_lb != I415_LB_BASE else 0,
                 "avg_delta": loyo_avg_d if best_lb != I415_LB_BASE else 0.0},
        "validated": validated, "runtime_s": round(time.time() - t0, 1),
    }
    out_dir = ROOT / "artifacts" / "phase193"
    out_dir.mkdir(parents=True, exist_ok=True)
    (out_dir / "phase193_report.json").write_text(json.dumps(report, indent=2))
    print(f"\nReport → artifacts/phase193/phase193_report.json")
    ...
    if validated:
        ...
        cfg["_version"] = new_v
        cfg["_validated"] += (
            f"; I415 lb re-sweep P193: {I415_LB_BASE}→{best_lb} LOYO {loyo_wins_final}/5 "
