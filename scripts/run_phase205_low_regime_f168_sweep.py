"""
Phase 205 — LOW Regime f168 Sweep + v1 Fine-Tune
==================================================
Current LOW: v1=0.46, i460=0.0716, i415=0.1184, f168=0.35
MID has f168=0.78 (dominant). LOW_f168=0.35 — is this optimal?

In LOW breadth regime (<20th pct), market is risk-off / defensive.
Funding signal may be more/less useful vs momentum signals.

Part A: LOW_f168 ∈ [0.25, 0.30, 0.35, 0.40, 0.45, 0.50, 0.55]
  - v1=0.46 fixed, i460/i415 scaled proportionally from base
Part B: LOW_v1 sweep using best LOW_f168
  - v1 ∈ [0.36, 0.40, 0.44, 0.46, 0.50, 0.54, 0.58]
  - i460/i415 scaled proportionally
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase205"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase205_report.json").write_text(json.dumps(_partial, indent=2, default=str))
    sys.exit(0)
...
LOW_F168_SWEEP = [0.25, 0.30, 0.35, 0.40, 0.45, 0.50, 0.55]
LOW_V1_SWEEP   = [0.36, 0.40, 0.44, 0.46, 0.50, 0.54, 0.58]
...
def make_low_weights(base_weights, low_v1, low_f168):
...
# ── MAIN ────────────────────────────────────────────────────────────────────
print("=" * 60)
print("Phase 205 — LOW Regime f168 Sweep + v1 Fine-Tune")
print("=" * 60)
_start = time.time()
...
print(f"  LOW now: v1={curr_low_v1:.3f}  i460={BASE_WEIGHTS['LOW']['i460']:.4f}"
      f"  i415={BASE_WEIGHTS['LOW']['i415']:.4f}  f168={curr_low_f168:.3f}")
...
# Part A: LOW_f168 sweep (v1 fixed)
print(f"\n[2] Part A: LOW f168 sweep (v1={curr_low_v1:.3f} fixed) ...")
results_a = []
for low_f168 in LOW_F168_SWEEP:
    w_test = make_low_weights(BASE_WEIGHTS, curr_low_v1, low_f168)
    if w_test is None:
        print(f"  LOW f168={low_f168:.2f}  SKIP (infeasible)")
        continue
    obj, _ = eval_weights(w_test, year_data, PARAMS)
    results_a.append((low_f168, obj, w_test))
    print(f"  LOW f168={low_f168:.2f} (i460={w_test['LOW']['i460']:.4f}, i415={w_test['LOW']['i415']:.4f})  OBJ={obj:.4f}")

best_a = max(results_a, key=lambda x: x[1])
best_low_f168, best_obj_a, _ = best_a
print(f"  Best LOW f168: {best_low_f168:.2f}  OBJ={best_obj_a:.4f}")

# Part B: LOW_v1 sweep (using best f168)
print(f"\n[3] Part B: LOW v1 sweep (f168={best_low_f168:.3f} fixed) ...")
results_b = []
for low_v1 in LOW_V1_SWEEP:
    w_test = make_low_weights(BASE_WEIGHTS, low_v1, best_low_f168)
    if w_test is None:
        print(f"  LOW v1={low_v1:.2f}  SKIP (infeasible)")
        continue
    obj, _ = eval_weights(w_test, year_data, PARAMS)
    results_b.append((low_v1, obj, w_test))
    print(f"  LOW v1={low_v1:.2f} (i460={w_test['LOW']['i460']:.4f}, i415={w_test['LOW']['i415']:.4f})  OBJ={obj:.4f}")

best_b = max(results_b, key=lambda x: x[1])
best_low_v1, best_obj_b, best_weights = best_b
delta = best_obj_b - baseline_obj
print(f"  Best LOW v1: {best_low_v1:.2f}  OBJ={best_obj_b:.4f}  Δ={delta:+.4f}")
...
report = {
    "phase": 205,
    "description": "LOW regime f168 + v1 sweep",
    "elapsed_seconds": round(time.time() - _start, 1),
    "baseline_obj": baseline_obj, "best_obj": best_obj_b, "delta": delta,
    "best_low_v1": best_low_v1, "best_low_f168": best_low_f168,
    "best_weights": best_weights,
    "loyo_wins": loyo_wins, "validated": validated,
    "yearly_best": best_yr, "yearly_base": base_yr,
}
(out / "phase205_report.json").write_text(json.dumps(report, indent=2, default=str))
...
if validated:
    ...
    rw["LOW"]["v1"] = best_weights["LOW"]["v1"]
    rw["LOW"]["i460bw168"] = round(best_weights["LOW"]["i460"], 6)
    rw["LOW"]["i415bw216"] = round(best_weights["LOW"]["i415"], 6)
    rw["LOW"]["f168"] = best_weights["LOW"]["f168"]
    ...
    cfg["_validated"] = old_val + (
        "; LOW regime P205: v1=%.2f->%.2f f168=%.2f->%.2f LOYO %d/5 delta=%+.4f OBJ=%.4f — PRODUCTION %s" % (
            curr_low_v1, best_low_v1, curr_low_f168, best_low_f168,
    ...
    cm = "phase205: LOW regime weight tune -- VALIDATED LOW_v1=%.2f LOW_f168=%.2f LOYO %d/5 delta=%+.4f OBJ=%.4f" % (
        best_low_v1, best_low_f168, loyo_wins, delta, best_obj_b)
else:
    new_ver = ver_now
    print(f"\n⚠️ NO IMPROVEMENT — LOW weights near-optimal (LOYO {loyo_wins}/5 delta={delta:+.4f})")
    cm = "phase205: LOW regime weight tune -- WEAK LOYO %d/5 delta=%+.4f LOW_v1=%.2f f168=%.2f" % (
        loyo_wins, delta, best_low_v1, best_low_f168)

os.system("git add configs/production_p91b_champion.json scripts/run_phase205_low_regime_f168_sweep.py "
          "artifacts/phase205/ 2>/dev/null || true")
os.system('git commit -m "' + cm + '"')
os.system("git stash && git pull --rebase && git stash pop && git push")

print(f"\n[DONE] Phase 205 complete.")
