"""
Phase 319 — Per-Regime V1 Internal Weight Re-Sweep [VECTORIZED]
================================================================
Baseline: v2.49.33, OBJ=6.2248 (parallel session advances)

V1 internal weights (carry/mom/mean_reversion) — re-sweep after
P315-P318 confirmed all other params optimal.

Current (from P314 + parallel session):
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase319"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase319_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 319 — Per-Regime V1 Internal Weight Re-Sweep")
    print("=" * 70)
    ...
    result = {
        "phase": "319", "title": "Per-Regime V1 Weight Re-Sweep",
    ...
    out = Path("artifacts/phase319"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase319_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase319/phase319_report.json")
