"""
Phase 192 — I460 Lookback Re-Sweep (post regime weight changes)
===============================================================
Motivation: P189-P190 significantly changed regime weights:
  LOW regime: i460 weight reduced 0.1607→0.0642 (V1 increased)
  MID regime: i460 weight reduced 0.1906→0.1119
  HIGH regime: i460 weight still 0.30 (dominant contributor)
With i460 now primarily a HIGH-regime signal (30%), optimal lookback may differ.

Sweep:
  lookback_bars ∈ [336, 380, 415, 460, 500, 540, 600, 672]

Baseline: v2.22.0, OBJ=2.8660
...
def _on_timeout(signum, frame):
    ...
    (out / "phase192_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
I460_LB_BASE = 460
BASELINE_OBJ = 2.8660
SWEEP_LBS    = [336, 380, 415, 460, 500, 540, 600, 672]
...
def load_year_data(year: int):
    ...
    # Pre-compute v1, i415, f168 (fixed)
    sig_rets = {}
    for sk, sname, params in [
        ...
        ("i460_base", "idio_momentum_alpha", {
            "k_per_side": 4, "lookback_bars": I460_LB_BASE, "beta_window_bars": 120,
...
def run_i460_lb(base_data, lb: int) -> np.ndarray:
    dataset, btc_vol, fund_std_pct, ts_spread_pct, regime, sig_rets, n = base_data
    if lb == I460_LB_BASE:
        i460_rets = sig_rets["i460_base"]
    else:
        result = BacktestEngine(BacktestConfig(costs=COST_MODEL)).run(
            dataset,
            make_strategy({"name": "idio_momentum_alpha", "params": {
                "k_per_side": 4, "lookback_bars": lb, "beta_window_bars": 120,
                "target_gross_leverage": 0.3, "rebalance_interval_bars": 48,
            }}))
        i460_rets = np.array(result.returns)

    all_rets = {"v1": sig_rets["v1"], "i460": i460_rets,
    ...
    for i in range(min_len):
        w = BASE_WEIGHTS[["LOW", "MID", "HIGH"][int(reg[i])]]
...
def main():
    t0 = time.time()
    print("=" * 65)
    print("Phase 192 — I460 Lookback Re-Sweep (post P189-P190)")
    print(f"Baseline: v2.22.0  OBJ={BASELINE_OBJ}  i460_lb={I460_LB_BASE}")
    print(f"Sweep:    lookback_bars ∈ {SWEEP_LBS}")
    print("=" * 65)
    ...
    bl_yr = {yr: sharpe(run_i460_lb(data_by_yr[yr], I460_LB_BASE)) for yr in YEARS}
    bl_obj = obj(bl_yr)
    print(f"  Baseline OBJ = {bl_obj:.4f}  (expected ≈ {BASELINE_OBJ})")

    print(f"\n[2/3] Sweeping I460 lookback_bars ...")
    sweep_results: dict = {}
    best_lb, best_obj_val = I460_LB_BASE, bl_obj
    best_yr_sharpes = bl_yr

    for lb in SWEEP_LBS:
        yr_s = {yr: sharpe(run_i460_lb(data_by_yr[yr], lb)) for yr in YEARS}
        ...
        flag = ("⭐" if (o > best_obj_val and loyo_wins >= 3 and d > 0.005) else
                ...
                ("⚠️ " if lb == I460_LB_BASE else "❌")))
        base_m = " ← baseline" if lb == I460_LB_BASE else ""
        print(f"  lb={lb:3d} → OBJ={o:.4f}  Δ={d:+.4f}  LOYO {loyo_wins}/5  {flag}{base_m}")
        sweep_results[lb] = {"obj": o, "delta": d, "loyo_wins": loyo_wins}
        if o > best_obj_val and loyo_wins >= 3 and d > 0.005:
            best_lb, best_obj_val, best_yr_sharpes = lb, o, yr_s

    print(f"\n[3/3] LOYO validation — best_lb={best_lb}  OBJ={best_obj_val:.4f}")
    if best_lb == I460_LB_BASE:
        print("  ❌ NO IMPROVEMENT — i460_lb=460 already optimal")
        ...
        for loyo_yr in YEARS:
            ...
            d = (obj({y: best_yr_sharpes[y] for y in train_yrs}) -
    ...
    if validated:
        print(f"\n✅ VALIDATED — i460_lb={best_lb}  OBJ={best_obj_val:.4f}")
    else:
        print(f"\n❌ NO IMPROVEMENT — i460_lb={I460_LB_BASE}  OBJ={bl_obj:.4f} optimal")

    report = {
        "phase": 192, "target": "i460_lookback_bars",
        "sweep": sweep_results,
        "baseline": {"obj": bl_obj, "lb": I460_LB_BASE},
        "best": {"obj": best_obj_val, "lb": best_lb},
        "loyo": {"wins": loyo_wins_final if best_lb != I460_LB_BASE else 0,
                 "avg_delta": loyo_avg_d if best_lb != I460_LB_BASE else 0.0},
        "validated": validated, "runtime_s": round(time.time() - t0, 1),
    }
    out_dir = ROOT / "artifacts" / "phase192"
    out_dir.mkdir(parents=True, exist_ok=True)
    (out_dir / "phase192_report.json").write_text(json.dumps(report, indent=2))
    print(f"\nReport → artifacts/phase192/phase192_report.json")
    print(f"Runtime: {report['runtime_s']:.0f}s")

    if validated:
        cfg_path = ROOT / "configs" / "production_p91b_champion.json"
        cfg = json.loads(cfg_path.read_text())
        cfg["ensemble"]["signals"]["i460bw168"]["params"]["lookback_bars"] = best_lb
        old_v = cfg["_version"]; parts = old_v.split(".")
        new_v = f"{parts[0]}.{int(parts[1]) + 1}.0" if len(parts) >= 2 else old_v
        cfg["_version"] = new_v
        cfg["_validated"] += (
            f"; I460 lb re-sweep P192: {I460_LB_BASE}→{best_lb} LOYO {loyo_wins_final}/5 "
