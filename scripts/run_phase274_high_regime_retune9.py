#!/usr/bin/env python3
"""
Phase 274 — HIGH Regime Retune Round 9
Supervisor set HIGH = v1=0.50, i460=0.50, i415=0.00, f168=0.00 (pure momentum).
My prior findings consistently showed f168~0.30 helps HIGH regime.
LOYO-validate optimal HIGH weights under current full config (v2.49.3+).
...
def try_high(yearly_data, params, base_weights, hv1, hi460, hi415, hf168):
    w = {
        "HIGH": {"v1": hv1, "i460": hi460, "i415": hi415, "f168": hf168},
...
def main():
    ...
    print(f"    Current HIGH: v1={cv1} i460={ci460} i415={ci415} f168={cf168}")
    print(f"    MID: {weights['MID']}  LOW: {weights['LOW']}")
    ...
    # [3] Sweep f168 — key question: does HIGH need contrarian component?
    print("\n[3] f168 sweep ...")
    for f in [0.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50]:
        o = try_high(yearly_data, params, weights, best_v1, best_i460, best_i415, f)
    ...
    # [4] Sweep v1
    print("\n[4] v1 sweep ...")
    for v in [0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50, 0.55]:
        o = try_high(yearly_data, params, weights, v, best_i460, best_i415, best_f168)
        marker = " <--" if o > best_obj else ""
    ...
    # [5] Sweep i460
    print("\n[5] i460 sweep ...")
    for a in [0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50, 0.55]:
        o = try_high(yearly_data, params, weights, best_v1, a, best_i415, best_f168)
        marker = " <--" if o > best_obj else ""
    ...
    # [6] Sweep i415
    print("\n[6] i415 sweep ...")
    for a in [0.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35]:
        o = try_high(yearly_data, params, weights, best_v1, best_i460, a, best_f168)
        marker = " <--" if o > best_obj else ""
    ...
    print("\n[8] LOYO validation ...")
    best_weights = {
        "HIGH": {"v1": best_v1, "i460": best_i460, "i415": best_i415, "f168": best_f168},
        "MID":  weights["MID"],
        "LOW":  weights["LOW"],
    }
    wins = 0; deltas = []
    ...
    if wins >= 3 and delta > 0.005:
        ...
        high = brs["regime_weights"]["HIGH"]
        high["v1"]        = round(float(best_v1), 4)
        high["i460bw168"] = round(float(best_i460), 4)
        high["i415bw216"] = round(float(best_i415), 4)
        high["f168"]      = round(float(best_f168), 4)
        for k in ["i460", "i415"]: high.pop(k, None)
        ...
        msg = (f"quant: {new_ver} — HIGH regime retune r9 "
        ...
        print(f"  Keeping HIGH: v1={cv1} i460={ci460} i415={ci415} f168={cf168}")

    print(f"\n[DONE] Phase 274 complete.  ({time.time()-t0:.1f}s)")
