#!/usr/bin/env python3
"""
Phase 264 — LOW Regime Re-retune Round 2
After supervisor's regime reopt9 + P261 breadth threshold change
Current LOW: v1=0.40, i460=0.00, i415=0.20, f168=0.40
LOW = brd_pct < 0.30 (unchanged boundary, but regime compositions changed)
Sweep: A) f168, B) v1, C) i415 fraction
"""
...
def make_low_w(base_weights, f168_w, v1_w, i415_frac):
...
def main():
    t0 = time.time()
    print("=" * 60)
    print("Phase 264 — LOW Regime Re-retune Round 2")
    print("LOW = brd_pct < 0.30 (defensive)")
    print("=" * 60)
    ...
    print(f"  Current LOW: v1={curr_low['v1']:.2f} i460={curr_low['i460']:.3f} "
          f"i415={curr_low['i415']:.3f} f168={curr_low['f168']:.2f}")
    curr_rem = curr_low["i415"] + curr_low["i460"]
    curr_i415_frac = curr_low["i415"] / curr_rem if curr_rem > 0 else 1.0

    # [2] f168 sweep
    print("\n[2] f168 sweep ...")
    f168_candidates = [0.00, 0.10, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 1.00]
    best_f168 = curr_low["f168"]; best_f168_obj = baseline_obj
    for fw in f168_candidates:
        w_test = make_low_w(base_weights, fw, curr_low["v1"], curr_i415_frac)
    ...
    # [3] v1 sweep
    print("\n[3] v1 sweep ...")
    v1_candidates = [0.00, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50, 0.55, 0.60]
    best_v1 = curr_low["v1"]; best_v1_obj = best_f168_obj
    for vw in v1_candidates:
        if vw + best_f168 > 1.0: continue
        w_test = make_low_w(base_weights, best_f168, vw, curr_i415_frac)
        obj = eval_weights(year_data, params, w_test)
    ...
    # [4] i415 fraction sweep
    print("\n[4] i415 fraction sweep ...")
    frac_candidates = [0.00, 0.10, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 1.00]
    best_frac = curr_i415_frac; best_frac_obj = best_v1_obj
    for frac in frac_candidates:
        w_test = make_low_w(base_weights, best_f168, best_v1, frac)
        obj = eval_weights(year_data, params, w_test)
        marker = " <-" if obj > best_frac_obj else ""
        print(f"  i415_frac={frac:.2f}  OBJ={obj:.4f}{marker}")
        if obj > best_frac_obj:
            best_frac_obj = obj; best_frac = frac
    print(f"  Best i415_frac: {best_frac:.2f}  OBJ={best_frac_obj:.4f}  D={best_frac_obj-baseline_obj:+.4f}")

    # [5] LOYO validation
    print("\n[5] LOYO validation ...")
    best_weights = make_low_w(base_weights, best_f168, best_v1, best_frac)
    wins = 0
    for yr in YEARS:
    ...
    delta = best_frac_obj - baseline_obj
    bw = best_weights["LOW"]
    print(f"\n  LOYO: {wins}/5 wins  delta={delta:+.4f}")
    print(f"  Best LOW: v1={bw['v1']:.2f} i460={bw['i460']:.3f} i415={bw['i415']:.3f} f168={bw['f168']:.2f}")

    if wins >= 3 and delta > 0.005:
        print(f"\n  VALIDATED  (LOYO {wins}/5 delta={delta:+.4f})")
        _update_config(cfg, bw, best_frac_obj)
    else:
...
def _update_config(cfg, bw, new_obj):
    ...
    print(f"  LOW: v1={bw['v1']:.3f} i460={bw['i460']:.3f} i415={bw['i415']:.3f} f168={bw['f168']:.3f}")
    print(f"  New OBJ={new_obj:.4f}")
    subprocess.run(["git", "add", str(p)], cwd="/Users/truonglys/projects/quant")
    subprocess.run(["git", "commit", "-m",
        f"quant: {new_ver} — LOW regime retune r2 (v1={bw['v1']:.2f} f168={bw['f168']:.2f}) OBJ={new_obj:.4f}"],
