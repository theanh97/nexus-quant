"""
Phase 217 — Regime Threshold Sweep (P_LOW × P_HIGH)
=====================================================
The breadth percentile signal switches regime:
  LOW  if breadth_pct < P_LOW
  HIGH if breadth_pct > P_HIGH
  MID  otherwise

Currently P_LOW=0.30, P_HIGH=0.60. Never swept since introduced.
With F168-heavy MID regime (78%) and the recent rebalance change,
the optimal boundary placement may have shifted.

All signals pre-computed — pure NumPy 2D sweep (very fast).

Baseline: v2.33.0, OBJ=3.1190
Sweep: p_low  ∈ [0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45]
       p_high ∈ [0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90]
"""
...
def _on_timeout(signum, frame):
    ...
    (out / "phase217_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
BASELINE_OBJ = 3.1190
P_LOW_SWEEP  = [0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45]
P_HIGH_SWEEP = [0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90]
...
def main():
    t0 = time.time()
    print("=" * 65)
    print("Phase 217 — Regime Threshold Sweep (P_LOW × P_HIGH)")
    print(f"Baseline: v2.33.0  OBJ={BASELINE_OBJ}")
    print(f"Current: P_LOW={P_LOW_BASE}  P_HIGH={P_HIGH_BASE}")
    print(f"P_LOW:  {P_LOW_SWEEP}")
    print(f"P_HIGH: {P_HIGH_SWEEP}")
    print("=" * 65)
    ...
    print("\n[2/3] 2D Sweep...")
    best_obj = base_obj; best_pl = P_LOW_BASE; best_ph = P_HIGH_BASE
    results = []
    for pl in P_LOW_SWEEP:
        for ph in P_HIGH_SWEEP:
            if ph <= pl: continue  # enforce P_LOW < P_HIGH
            o, _ = evaluate(pl, ph, all_data)
            delta = o - base_obj
            marker = " ← BEST" if o > best_obj else ""
            print(f"  p_low={pl:.2f}  p_high={ph:.2f}  OBJ={o:.4f}  Δ={delta:+.4f}{marker}")
            results.append((pl, ph, o))
            if o > best_obj:
                best_obj = o; best_pl = pl; best_ph = ph

    print(f"\n  → Best: p_low={best_pl}  p_high={best_ph}  OBJ={best_obj:.4f}")

    print("\n[3/3] LOYO validation...")
    joint_obj, joint_yearly = evaluate(best_pl, best_ph, all_data)
    ...
    print(f"\n{'✅ VALIDATED' if validated else '❌ NO IMPROVEMENT'} — p_low={best_pl}  p_high={best_ph}")

    result = {
        "phase": 217, "baseline_obj": float(base_obj), "best_obj": float(joint_obj),
        "delta": float(delta), "loyo_wins": loyo_wins, "loyo_total": len(joint_yearly),
        "validated": validated, "best_p_low": best_pl, "best_p_high": best_ph,
        "top5": sorted(results, key=lambda x: -x[2])[:5],
    }
    _partial.update(result)
    out = Path("artifacts/phase217"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase217_report.json").write_text(json.dumps(result, indent=2))
