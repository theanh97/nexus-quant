"""
Phase 197 — Breadth Regime Threshold Re-Sweep
===============================================
P181 set p_low=0.20, p_high=0.60 at v2.17.0. Since then regime weights
changed drastically (V1 dominant LOW, F168 dominant MID, I415 dominant HIGH).
Also FTS params updated to rt=0.65/rs=0.45/bt=0.25/bs=1.70 by parallel P197.

Re-sweep p_low × p_high with full current v2.26.0 config.

Sweep: p_low  ∈ [0.10, 0.15, 0.20, 0.25, 0.30]
       p_high ∈ [0.50, 0.55, 0.60, 0.65, 0.70]
Constraint: p_low < p_high - 0.20 (keep ≥20pp spread)

Baseline: v2.26.0, OBJ ≈ 2.9142
"""
...
def _on_timeout(signum, frame):
    ...
    (out / "phase197_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
# Overlay constants from v2.26.0 config (including parallel session's FTS update)
VOL_WINDOW    = 168
BRD_LB        = 192
PCT_WINDOW    = 336
...
VOL_SCALE     = 0.40
VOL_F168_BOOST= 0.10
# Updated FTS params (parallel session phase197 validated rt=0.65/rs=0.45/bt=0.25/bs=1.70)
TS_RT = 0.65; TS_RS = 0.45; TS_BT = 0.25; TS_BS = 1.70
DISP_THR = 0.60; DISP_SCALE = 1.0

# Current p_low/p_high (baseline)
P_LOW_BASE  = 0.20
P_HIGH_BASE = 0.60

# v2.26.0 signal params
V1_PARAMS = {
...
P_LOW_SWEEP  = [0.10, 0.15, 0.20, 0.25, 0.30]
P_HIGH_SWEEP = [0.50, 0.55, 0.60, 0.65, 0.70]
...
def load_year_data(year):
    ...
    # Breadth + percentile — pre-compute for all thresholds (store raw breadth pct)
    ...
    return dataset, btc_vol, fund_std_pct, ts_spread_pct, brd_pct, sig_rets, n


def ensemble_returns_with_thresholds(p_low, p_high, base_data):
    dataset, btc_vol, fund_std_pct, ts_spread_pct, brd_pct, sig_rets, n = base_data
    ...
    for i in range(min_len):
        bp = bpct[i]
        if bp < p_low:   regime_idx = 0  # LOW
        elif bp > p_high: regime_idx = 2  # HIGH
        else:             regime_idx = 1  # MID
        w = REGIME_WEIGHTS[REGIME_NAMES[regime_idx]]
...
def evaluate_thresholds(p_low, p_high, all_data):
    yearly = {}
    for yr, base_data in all_data.items():
        ens = ensemble_returns_with_thresholds(p_low, p_high, base_data)
...
def main():
    t0 = time.time()
    print("=" * 65)
    print("Phase 197 — Breadth Regime Threshold Re-Sweep")
    print(f"Baseline: v2.26.0  OBJ={BASELINE_OBJ}")
    print(f"Sweep: p_low × p_high  current={P_LOW_BASE}/{P_HIGH_BASE}")
    print("=" * 65)

    print("\n[1/3] Loading per-year data ...")
    all_data = {}
    ...
    base_obj, base_yearly = evaluate_thresholds(P_LOW_BASE, P_HIGH_BASE, all_data)
    print(f"\n  Baseline OBJ = {base_obj:.4f}  (expected ≈ {BASELINE_OBJ})")

    print("\n[2/3] Sweeping p_low × p_high ...")
    best_obj   = base_obj
    best_p_low  = P_LOW_BASE
    best_p_high = P_HIGH_BASE
    results = []

    for p_low in P_LOW_SWEEP:
        for p_high in P_HIGH_SWEEP:
            if p_high - p_low < 0.20: continue  # need ≥20pp spread
            o, _ = evaluate_thresholds(p_low, p_high, all_data)
            delta = o - base_obj
            marker = " ← BEST" if o > best_obj else ""
            print(f"  p_low={p_low:.2f}  p_high={p_high:.2f}  OBJ={o:.4f}  Δ={delta:+.4f}{marker}")
            results.append((p_low, p_high, o, delta))
            if o > best_obj:
                best_obj   = o
                best_p_low  = p_low
                best_p_high = p_high
    ...
    print("\n[3/3] LOYO validation ...")
    joint_obj, joint_yearly = evaluate_thresholds(best_p_low, best_p_high, all_data)
    ...
    print(f"\n{status} — p_low={best_p_low:.2f}  p_high={best_p_high:.2f}")
    ...
    result = {
        ...
        "validated": validated,
        "best_p_low": best_p_low,
        "best_p_high": best_p_high,
        "sweep_results": [(r[0], r[1], r[2], r[3]) for r in results],
    }
    out = Path("artifacts/phase197"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase197_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        print(f"\n  Config → v2.27.0  OBJ={round(joint_obj,4)}")

    print(f"\nReport → artifacts/phase197/phase197_report.json")
