#!/usr/bin/env python3
"""
Phase 269 — HIGH Regime Re-retune Round 6
After convergence sweep (P265-P268), re-check HIGH regime allocation.
Sweep: A) f168 fine-grid, B) v1 fine-grid, C) idio_ratio fine-grid
Current HIGH: v1=0.20, i460=0.135, i415=0.315, f168=0.35
"""
...
def main():
    t0 = time.time()
    print("=" * 60)
    print("Phase 269 — HIGH Regime Re-retune Round 6 (Fine Grid)")
    print("=" * 60)
    ...
    curr_ratio = curr_high["i460"] / curr_idio if curr_idio > 0 else 0.30
    ...
    # [2] f168 fine-grid sweep
    print("\n[2] f168 fine-grid sweep ...")
    f168_cands = [0.20, 0.25, 0.28, 0.30, 0.32, 0.35, 0.38, 0.40, 0.42, 0.45, 0.48, 0.50]
    ...
    # [3] v1 fine-grid sweep
    print("\n[3] v1 fine-grid sweep ...")
    v1_cands = [0.12, 0.14, 0.16, 0.18, 0.20, 0.22, 0.24, 0.26, 0.28, 0.30]
    ...
    # [4] idio_ratio fine-grid sweep
    print("\n[4] idio_ratio fine-grid sweep ...")
    ratio_cands = [0.20, 0.25, 0.28, 0.30, 0.33, 0.35, 0.38, 0.40, 0.43, 0.45, 0.50]
    ...
    print(f"  Best idio_ratio: {best_ratio:.2f}  D={best_ratio_obj-baseline_obj:+.4f}")
    ...
    print(f"\n[DONE] Phase 269 complete.  ({time.time()-t0:.1f}s)")
...
def _update_config(cfg, bw, new_obj):
    ...
    subprocess.run(["git", "commit", "-m",
        f"quant: {new_ver} — HIGH retune r6 fine (v1={bw['v1']:.2f} f168={bw['f168']:.2f}) OBJ={new_obj:.4f}"],
