"""
Phase 336 — Per-Regime FTS Retune [VECTORIZED]
============================================================================
Baseline: v2.49.40, OBJ=6.7382 (P335 vol/disp retune — no change)

Continue R&D cycle: re-sweep FTS overlay params.

Current FTS (confirmed optimal in P296, P301, P306, P311, P316, P321, P326, P331):
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase336_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase336_fts_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 336 — FTS Retune [VECTORIZED]")
    print("=" * 70)
    ...
    result = {
        "phase": "336", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    out = Path("artifacts/phase336_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase336_fts_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase336_fts/phase336_fts_report.json")
    ...
    if validated:
        ...
        msg = (f"feat: P336 FTS retune OBJ={reported_obj} "
