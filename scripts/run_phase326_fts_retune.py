"""
Phase 326 — Per-Regime FTS Retune [VECTORIZED]
============================================================================
Baseline: v2.49.36, OBJ=6.4480 (P325 vol/disp retune — no change)

Continue R&D cycle: re-sweep FTS overlay params.

Current FTS (confirmed optimal in P296, P301, P306, P311, P316, P321):
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase326_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase326_fts_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 326 — FTS Retune [VECTORIZED]")
    print("=" * 70)
    ...
    result = {
        "phase": "326", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    out = Path("artifacts/phase326_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase326_fts_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase326_fts/phase326_fts_report.json")
    ...
    if validated:
        ...
        msg = (f"feat: P326 FTS retune OBJ={reported_obj} "
