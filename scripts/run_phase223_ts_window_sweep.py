"""
Phase 223 — TS_SHORT × TS_LONG Sweep
======================================
The FTS (Funding Term Structure) overlay uses:
  ts_spread = rolling_mean(cross_sectional_funding, TS_SHORT) - rolling_mean(..., TS_LONG)
  When ts_spread_pct > TS_RT → scale by TS_RS (dampen)
  When ts_spread_pct < TS_BT → scale by TS_BS (boost, 1.70x)

TS_SHORT=16h, TS_LONG=72h were set early and NEVER swept.
With BS=1.70 boost being the dominant overlay, the window timescales
of the signal generator could significantly affect performance.

All signals pre-computed — pure NumPy 2D sweep (very fast).
Rebuild ts_spread_pct on-the-fly for each (TS_SHORT, TS_LONG) combo.

Sweep: TS_SHORT ∈ [4, 8, 12, 16, 20, 24, 32, 48]
       TS_LONG  ∈ [48, 60, 72, 84, 96, 120, 144, 168, 192]
Constraint: TS_SHORT < TS_LONG
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase223"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase223_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
BASELINE_OBJ = 3.1534
TS_SHORT_SWEEP = [4, 8, 12, 16, 20, 24, 32, 48]
TS_LONG_SWEEP  = [48, 60, 72, 84, 96, 120, 144, 168, 192]
...
def load_year_data(year):
    ...
    # Pre-compute funding dispersion pct (fixed, doesn't depend on TS windows)
    fund_std_raw = np.std(fund_rates, axis=1)
    fund_std_pct = np.full(n, 0.5)
    for i in range(FUND_DISP_PCT, n):
        fund_std_pct[i] = float(np.mean(fund_std_raw[i-FUND_DISP_PCT:i] <= fund_std_raw[i]))
    # Cross-sectional funding mean (used to compute ts_raw for different windows)
    ...
    # Pre-compute all strategy returns (fixed)
    sig_rets = {}
    ...
    # Return xsect_mean so we can recompute ts_spread_pct for different windows
    return btc_vol, fund_std_pct, xsect_mean, brd_pct, sig_rets, n

def compute_ts_spread_pct(xsect_mean, n, ts_short, ts_long):
...
def ensemble_rets(ts_spread_pct, base_data):
    btc_vol, fund_std_pct, xsect_mean, brd_pct, sig_rets, n = base_data
    RNAMES = ["LOW", "MID", "HIGH"]
...
def evaluate(ts_short, ts_long, all_data):
    yearly = {}
    for yr, d in all_data.items():
        btc_vol, fund_std_pct, xsect_mean, brd_pct, sig_rets, n = d
        tsp = compute_ts_spread_pct(xsect_mean, n, ts_short, ts_long)
...
def main():
    t0 = time.time()
    print("=" * 65)
    print("Phase 223 — TS_SHORT × TS_LONG Sweep (FTS signal windows)")
    print(f"Baseline: v2.34.0  OBJ={BASELINE_OBJ}")
    print(f"Current: TS_SHORT={TS_SHORT_BASE}  TS_LONG={TS_LONG_BASE}")
    print(f"TS_SHORT: {TS_SHORT_SWEEP}")
    print(f"TS_LONG:  {TS_LONG_SWEEP}")
    print("=" * 65)
    ...
    base_obj_val, base_yearly = evaluate(TS_SHORT_BASE, TS_LONG_BASE, all_data)
    print(f"\n  Baseline OBJ = {base_obj_val:.4f}  (expected ≈ {BASELINE_OBJ})")
    _partial.update({"baseline_obj": float(base_obj_val)})

    print("\n[2/3] 2D Sweep...")
    best_obj_val = base_obj_val; best_tss = TS_SHORT_BASE; best_tsl = TS_LONG_BASE
    results = []
    for tss in TS_SHORT_SWEEP:
        for tsl in TS_LONG_SWEEP:
            if tss >= tsl: continue  # enforce TS_SHORT < TS_LONG
            o, _ = evaluate(tss, tsl, all_data)
            delta = o - base_obj_val
            marker = " ← BEST" if o > best_obj_val else ""
            print(f"  ts_short={tss:3d}  ts_long={tsl:4d}  OBJ={o:.4f}  Δ={delta:+.4f}{marker}")
            results.append((tss, tsl, o))
            if o > best_obj_val:
                best_obj_val = o; best_tss = tss; best_tsl = tsl

    print(f"\n  → Best: ts_short={best_tss}  ts_long={best_tsl}  OBJ={best_obj_val:.4f}")

    print(f"\n[3/3] LOYO validation...")
    joint_obj_val, joint_yearly = evaluate(best_tss, best_tsl, all_data)
    ...
    print(f"\n{'✅ VALIDATED' if validated else '❌ NO IMPROVEMENT'} — ts_short={best_tss}  ts_long={best_tsl}")

    result = {
        "phase": 223, "baseline_obj": float(base_obj_val), "best_obj": float(joint_obj_val),
        "delta": float(delta), "loyo_wins": loyo_wins, "loyo_total": len(joint_yearly),
        "validated": validated, "best_ts_short": best_tss, "best_ts_long": best_tsl,
        "top5": sorted(results, key=lambda x: -x[2])[:5],
    }
    _partial.update(result)
    out = Path("artifacts/phase223"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase223_report.json").write_text(json.dumps(result, indent=2))

    if validated:
        cfg_path = ROOT / "configs" / "production_p91b_champion.json"
        cfg = json.load(open(cfg_path))
        fts = cfg.setdefault("fts_overlay_params", {})
        fts["ts_short_bars"] = best_tss
        fts["ts_long_bars"]  = best_tsl
        cfg["_version"] = "2.35.0"
