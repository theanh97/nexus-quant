"""
Phase 200 — Regime Weight Extension v2
========================================
P199 found ceilings:
  - MID  f168=0.55  (v1=0.30 fixed, still ↑)
  - HIGH v1=0.08    (f168=0.15 fixed, still ↑)

Extensions:
  A) MID  f168: [0.55, 0.57, 0.60, 0.63, 0.65, 0.68] with v1=0.30
  B) HIGH v1:   [0.08, 0.10, 0.12, 0.15, 0.18]       with f168=0.15
  C) LOW fine-tune: confirm v1=0.48 f168=0.35 vs neighbors
  D) Joint LOYO

Baseline: v2.28.0, OBJ=2.9956
"""
...
def _on_timeout(signum, frame):
    ...
    (out / "phase200_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
BASE_WEIGHTS = {
    "LOW":  {"v1": 0.48,   "i460": 0.0640, "i415": 0.1060, "f168": 0.35},
    "MID":  {"v1": 0.30,   "i460": 0.0559, "i415": 0.0941, "f168": 0.55},
    "HIGH": {"v1": 0.08,   "i460": 0.2750, "i415": 0.4950, "f168": 0.15},
}
...
def main():
    t0 = time.time()
    print("=" * 65)
    print("Phase 200 — Regime Weight Extension v2")
    print(f"Baseline: v2.28.0  OBJ={BASELINE_OBJ}")
    print("=" * 65)
    ...
    print("\n[2/4] Part A: Extend MID f168 (v1=0.30 fixed) ...")
    best_mid_f168 = 0.55; best_obj_mid = base_obj
    for f168_w in [0.55, 0.57, 0.60, 0.63, 0.65, 0.68, 0.70]:
        w_r = make_weights("MID", 0.30, f168_w)
        ...
        print(f"  MID v1=0.30 f168={f168_w:.2f}  OBJ={o:.4f}  Δ={delta:+.4f}{marker}")
        if o > best_obj_mid:
            best_obj_mid = o; best_mid_f168 = f168_w
    print(f"  → Best MID f168={best_mid_f168:.2f}  OBJ={best_obj_mid:.4f}")

    print(f"\n[2b] Part B: Extend HIGH v1 (f168=0.15 fixed) ...")
    best_high_v1 = 0.08; best_obj_high = base_obj
    for v1_w in [0.08, 0.10, 0.12, 0.15, 0.18, 0.20]:
        w_r = make_weights("HIGH", v1_w, 0.15)
        if w_r is None: continue
        wbd = {"LOW": BASE_WEIGHTS["LOW"], "MID": BASE_WEIGHTS["MID"], "HIGH": w_r}
        o, _ = evaluate_weights(wbd, all_data)
        delta = o - base_obj
        marker = " ← BEST" if o > best_obj_high else ""
        print(f"  HIGH v1={v1_w:.2f} f168=0.15  OBJ={o:.4f}  Δ={delta:+.4f}{marker}")
        if o > best_obj_high:
            best_obj_high = o; best_high_v1 = v1_w
    print(f"  → Best HIGH v1={best_high_v1:.2f}  OBJ={best_obj_high:.4f}")

    print(f"\n[3/4] Part C: LOW fine-tune ...")
    best_low_v1 = 0.48; best_low_f168 = 0.35; best_obj_low = base_obj
    for v1_w in [0.44, 0.46, 0.48, 0.50, 0.52]:
        for f168_w in [0.30, 0.32, 0.35, 0.37, 0.40]:
            ...
            if delta > -0.01:  # only print near-best
                print(f"  LOW v1={v1_w:.2f} f168={f168_w:.2f}  OBJ={o:.4f}  Δ={delta:+.4f}{marker}")
            if o > best_obj_low:
                best_obj_low = o; best_low_v1 = v1_w; best_low_f168 = f168_w
    print(f"  → Best LOW v1={best_low_v1:.2f} f168={best_low_f168:.2f}  OBJ={best_obj_low:.4f}")

    # Build best weights
    best_weights = {
        "LOW":  make_weights("LOW", best_low_v1, best_low_f168) or BASE_WEIGHTS["LOW"].copy(),
        "MID":  make_weights("MID", 0.30, best_mid_f168) or BASE_WEIGHTS["MID"].copy(),
        "HIGH": make_weights("HIGH", best_high_v1, 0.15) or BASE_WEIGHTS["HIGH"].copy(),
    }
    ...
    result = {"phase": 200, "baseline_obj": float(base_obj), "best_obj": float(joint_obj),
    ...
    out = Path("artifacts/phase200"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase200_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        print(f"\n  Config → v2.29.0  OBJ={round(joint_obj,4)}")
