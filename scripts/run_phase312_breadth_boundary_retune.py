"""
Phase 312 — Breadth Boundary Re-Tune (p_low / p_high)
=======================================================
Baseline: v2.49.30, OBJ=6.0005 (parallel session advances)

After P311 FTS confirmed optimal, re-sweep breadth regime boundaries.
Current: p_low=0.30, p_high=0.60 (confirmed 3x in P275, P302, P307).
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase312"); out.mkdir(parents=True, exist_ok=True)
...
if __name__ == "__main__":
    t0 = time.time()
    print("=" * 65)
    print("Phase 312 — Regime Boundary Re-Sweep (p_low / p_high)")
    ...
    if delta < MIN_DELTA:
        ...
        result = {
            "phase": "312", "baseline_obj": float(obj_base), "stored_baseline": base_obj,
    ...
    if validated:
        ...
        old_ver = cfg.get("_version", "2.49.30")
        parts = old_ver.split(".")
        ...
        msg = (f"feat: P312 boundary resweep p_low={best_plow:.2f} p_high={best_phigh:.2f} "
