"""
Phase 204 — V1 Momentum Lookback Sweep
========================================
After P203: w_mom raised from 0.40→0.45. Optimal momentum_lookback_bars may shift.
Pre-compute non-V1 returns, sweep V1 momentum_lb only.

Baseline: v2.32.0, OBJ=3.0860
Current: momentum_lookback_bars=336
Sweep: [168, 240, 288, 336, 400, 460, 500, 560]
"""
...
def _on_timeout(signum, frame):
    ...
    (out / "phase204_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
# V1 params (P203 optimized weights, non-lookback components fixed)
V1_W_CARRY = 0.25; V1_W_MOM = 0.45; V1_W_MR = 0.30
V1_MR_LB   = 84   # will vary separately in P205
V1_BASE = {
    "k_per_side": 2, "w_carry": V1_W_CARRY, "w_mom": V1_W_MOM, "w_mean_reversion": V1_W_MR,
    "mean_reversion_lookback_bars": V1_MR_LB,
    "vol_lookback_bars": 192, "target_gross_leverage": 0.35, "rebalance_interval_bars": 60,
}

BASELINE_OBJ  = 3.0860
MOM_LB_BASE   = 336
MOM_LB_SWEEP  = [168, 240, 288, 336, 400, 460, 500, 560]
...
def run_v1(dataset, mom_lb):
    params = dict(V1_BASE, momentum_lookback_bars=mom_lb)
    result = BacktestEngine(BacktestConfig(costs=COST_MODEL)).run(
        dataset, make_strategy({"name": "nexus_alpha_v1", "params": params}))
    return np.array(result.returns)

def ensemble_rets(v1_rets, fixed_rets, btc_vol, fund_std_pct, ts_spread_pct, brd_pct):
    RNAMES = ["LOW", "MID", "HIGH"]
...
def evaluate_lb(mom_lb, all_data):
    yearly = {}
    for yr, (dataset, btc_vol, fsp, tsp, brd_pct, fixed_rets, n) in all_data.items():
        v1r = run_v1(dataset, mom_lb)
        ens = ensemble_rets(v1r, fixed_rets, btc_vol, fsp, tsp, brd_pct)
...
def main():
    t0 = time.time()
    print("=" * 65)
    print("Phase 204 — V1 Momentum Lookback Sweep")
    print(f"Baseline: v2.32.0  OBJ={BASELINE_OBJ}")
    print(f"Current: mom_lb={MOM_LB_BASE}  (w_carry=0.25 w_mom=0.45 w_mr=0.30)")
    print(f"Sweep: {MOM_LB_SWEEP}")
    print("=" * 65)
    ...
    print(f"\n{'✅ VALIDATED' if validated else '❌ NO IMPROVEMENT'} — mom_lb={best_lb}")

    result = {
        "phase": 204, "baseline_obj": float(base_obj), "best_obj": float(joint_obj),
        "delta": float(delta), "loyo_wins": loyo_wins, "loyo_total": len(joint_yearly),
        "validated": validated, "best_mom_lb": best_lb,
        "sweep_results": results,
    }
    _partial.update(result)
    out = Path("artifacts/phase204"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase204_report.json").write_text(json.dumps(result, indent=2))
