"""
Phase 274 — Regime Weight Re-Optimization Round 13 [VECTORIZED]
================================================================
Baseline: v2.49.2 hybrid (P271 weights + P267 p_high=0.60)
OBJ=4.2602

p_high changed from 0.68 → 0.60 (Phase 267). This affects the proportion
of bars in each regime and potentially the optimal signal blend weights.
Re-optimize all three regime weight vectors.

Approach: vectorized sweep, W_VALS grid of 0.05 steps, f168=1-v1-i460-i415.
Sequential 2-pass (LOW→MID→HIGH repeated twice).
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase274"); out.mkdir(parents=True, exist_ok=True)
...
# Grid: steps of 0.05
W_VALS = [round(x * 0.05, 2) for x in range(21)]  # 0.00 to 1.00
...
def build_precomp(p, all_base_data, all_regime_weights):
    """Build fast precomp for given regime_weights dict (and fixed other params from p)."""
    fast_data = {}
...
def fast_eval(all_regime_weights, fast_data):
    yearly = []
    for yr, fd in sorted(fast_data.items()):
        sc = fd["scaled"]; masks = fd["masks"]; ml = fd["ml"]
        ens = np.zeros(ml)
        for ri, rn in enumerate(RNAMES):
            m = masks[ri]; w = all_regime_weights[rn]
...
def sweep_regime_weights(target_regime, current_weights, fast_data, best_obj):
    """Sweep all valid (v1, i460, i415, f168) combos for target_regime."""
    all_weights = {r: dict(w) for r, w in current_weights.items()}
    ...
    for v1 in W_VALS:
        for i460 in W_VALS:
            for i415 in W_VALS:
                ...
                obj = fast_eval(all_weights, fast_data)
                if obj > best_obj + 1e-6:
                    best_obj = obj; best_w = dict(w)
    all_weights[target_regime] = best_w
    return best_w, best_obj, all_weights


if __name__ == "__main__":
    t0 = time.time()
    print("=" * 65)
    print("Phase 274 — Regime Weight Re-Optimization Round 13 [VECTORIZED]")

    P = load_config_params()
    base_obj = P["baseline_obj"]
    print(f"Baseline: v{P['config_version']}  OBJ={base_obj:.4f}")
    print(f"p_low={P['p_low']:.2f}  p_high={P['p_high']:.2f}")
    ...
    # [2] Build baseline precomp
    print("\n[2] Building baseline fast data...", flush=True)
    baseline_fd = build_precomp(P, all_base_data, P["regime_weights"])
    obj_base = fast_eval(P["regime_weights"], baseline_fd)
    print(f"  Measured baseline OBJ = {obj_base:.4f}  (config says {base_obj:.4f})")

    # [3] 2-pass regime weight sweep
    print("\n[3] Regime weight sweep (2 passes)...", flush=True)
    ...
    for pass_num in range(1, 3):
        print(f"\n  === Pass {pass_num} ===")
        for rname in RNAMES:
            current_weights[rname], best_obj, current_weights = sweep_regime_weights(
                rname, current_weights, baseline_fd, best_obj)
            w = current_weights[rname]
            print(f"    {rname}: v1={w['v1']:.2f} i460={w['i460']:.2f} i415={w['i415']:.2f} "
                  f"f168={w['f168']:.2f}  OBJ={best_obj:.4f}  Δ={best_obj-obj_base:+.4f}")
    ...
    for yr in sorted(YEAR_RANGES.keys()):
        ...
        base_loyo = subset_obj(P["regime_weights"], baseline_fd, others)
        cand_loyo = subset_obj(current_weights, baseline_fd, others)
    ...
    if validated:
        ...
        for rn in RNAMES:
            w = current_weights[rn]
            rw[rn]["v1"]        = w["v1"]
            rw[rn]["i460bw168"] = w["i460"]
            rw[rn]["i415bw216"] = w["i415"]
            rw[rn]["f168"]      = w["f168"]
        ...
        for rn in RNAMES:
            w = current_weights[rn]
            print(f"  {rn}: v1={w['v1']:.2f} i460={w['i460']:.2f} i415={w['i415']:.2f} f168={w['f168']:.2f}")
        msg = (f"feat: P274 regime weight reopt13 OBJ={reported_obj:.4f} "
