"""
Phase 296 — Per-Regime FTS Retune after P293-P295 Signal Shift [VECTORIZED]
============================================================================
Baseline: v2.49.21, OBJ=5.6221 (Phase 295 vol+disp retune, LOYO=4/5)

After P293 (V1 internal weights), P294 (regime weights), P295 (vol+disp),
the signal landscape has shifted. Re-tune FTS overlay params.

Current FTS (from P290):
...
Include wider rs grid to test if reduce leg should be re-enabled with new signals.
"""
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase296_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase296_fts_report.json").write_text(json.dumps(_partial, indent=2))
...
# Grids — finer around current P290 values + test re-enabling reduce leg
# Current: rs=0.01, rt=0.78/0.58/0.55, bs=3.5/1.5/2.5, bt=0.30/0.22/0.22, pw=240/288/400
RS_GRID  = [0.005, 0.01, 0.02, 0.03, 0.05, 0.08, 0.10, 0.15, 0.20, 0.30, 0.50]
RT_GRID  = [0.50, 0.53, 0.55, 0.58, 0.60, 0.63, 0.65, 0.70, 0.75, 0.78, 0.80, 0.85]
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 296 — FTS Retune after P293-P295 Signal Shift [VECTORIZED]")
    print("Baseline: v2.49.21 OBJ=5.6221")
    print("=" * 70)
    ...
    result = {
        "phase": "296", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    out = Path("artifacts/phase296_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase296_fts_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase296_fts/phase296_fts_report.json")
    ...
    if validated:
        ...
        old_ver = cfg.get("_version", "2.49.21")
        parts = old_ver.split(".")
        parts[-1] = str(int(parts[-1]) + 1)
        new_ver = ".".join(parts)
        cfg["_version"] = new_ver
        old_vver = cfg.get("version", "v2.49.21").lstrip("v")
        ...
        msg = (f"feat: P296 FTS retune OBJ={reported_obj} "
