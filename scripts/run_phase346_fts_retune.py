"""
Phase 346 — Per-Regime FTS Retune [VECTORIZED]
============================================================================
Baseline: v2.49.40, OBJ=6.7382 (P345 vol/disp retune — no change)

Continue R&D cycle: re-sweep FTS overlay params.

Current FTS (confirmed optimal in P296, P301, P306, P311, P316, P321, P326, P331, P336, P341):
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase346_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase346_fts_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 346 — FTS Retune [VECTORIZED]")
    print("=" * 70)
    ...
    result = {
        "phase": "346", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    out = Path("artifacts/phase346_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase346_fts_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase346_fts/phase346_fts_report.json")
    ...
    if validated:
        ...
        old_ver = cfg.get("_version", "2.49.34")
        parts = old_ver.split(".")
        ...
        msg = (f"feat: P346 FTS retune OBJ={reported_obj} "
