"""
Phase 232 — Per-Regime DISP Overlay Sweep
===========================================
P231 validated per-regime VOL scale (Δ=+0.0369, LOYO 4/5, OBJ=3.5261).
Currently DISP overlay is disabled: DISP_SCALE=1.0 (no-op) for all regimes.
The DISP overlay fires when fund_std_pct > DISP_THR (currently 0.60):
  funding rate dispersion across symbols is unusually high.

Rationale for per-regime DISP:
  - LOW breadth + high DISP = divergent stress? Scale down more (< 1.0)
  - MID breadth + high DISP = confused market → dampen (< 1.0) or ignore
  - HIGH breadth + high DISP = some symbols lagging → amplify or dampen?

Also explore DISP_SCALE > 1.0 (amplify when high dispersion).

DISP_SCALE sweep: [0.30, 0.50, 0.70, 0.80, 0.90, 1.00, 1.10, 1.20, 1.50, 2.00]
DISP_THR sweep:   [0.40, 0.50, 0.60, 0.70, 0.80]

Sequential: optimize LOW, MID, HIGH (2 passes).

Baseline: v2.39.0, OBJ=3.5261
"""
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase232"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase232_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
BASELINE_OBJ = 3.5261
DISP_SCALE_SWEEP = [0.30, 0.50, 0.70, 0.80, 0.90, 1.00, 1.10, 1.20, 1.50, 2.00]
...
def ensemble_rets(disp_thr, disp_scale, base_data):
    """disp_thr / disp_scale are dicts keyed by regime (LOW/MID/HIGH)."""
...
def main():
    t0 = time.time()
    print("=" * 65)
    print("Phase 232 — Per-Regime DISP Overlay Sweep")
    print(f"Baseline: v2.39.0  OBJ={BASELINE_OBJ}")
    print(f"Current DISP: THR=0.60 (uniform), SCALE=1.0 (disabled)")
    ...
    base_disp_thr   = {r: 0.60 for r in ["LOW", "MID", "HIGH"]}
    base_disp_scale = {r: 1.00 for r in ["LOW", "MID", "HIGH"]}
    ...
    print("\n[2/3] Per-regime DISP_THR × DISP_SCALE sweep (2 passes)...")
    ...
    result = {
        "phase": 232, "baseline_obj": float(base_obj_val), "best_obj": float(joint_obj_val),
    ...
    out = Path("artifacts/phase232"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase232_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        print(f"\n  Config → v2.40.0  OBJ={round(joint_obj_val,4)}")
