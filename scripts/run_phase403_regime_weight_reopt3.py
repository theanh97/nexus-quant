"""
Phase 403 — Regime Weight Re-opt (31st Pass) [VECTORIZED]
=========================================================
Baseline: v2.49.40, OBJ=6.7382 (P357 boundary resweep — no change)

After P355-P357 confirmed overlays/boundaries optimal,
re-sweep regime ensemble weights.
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase403"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase403_report.json").write_text(json.dumps(_partial, indent=2))
...
def load_config_params():
    ...
    return {
        ...
        "fts_rs": fts.get("per_regime_rs", {"LOW": 0.01, "MID": 0.01, "HIGH": 0.01}),
        "fts_bs": fts.get("per_regime_bs", {"LOW": 3.5,  "MID": 1.5,  "HIGH": 2.5}),
        "fts_rt": fts.get("per_regime_rt", {"LOW": 0.78, "MID": 0.58, "HIGH": 0.55}),
        "fts_bt": fts.get("per_regime_bt", {"LOW": 0.30, "MID": 0.22, "HIGH": 0.22}),
        "ts_pct_win": fts.get("per_regime_ts_pct_win", {"LOW": 240, "MID": 288, "HIGH": 400}),
        "vol_thr":   vol.get("per_regime_threshold", {"LOW": 0.54, "MID": 0.50, "HIGH": 0.57}),
        "vol_scale": vol.get("per_regime_scale",     {"LOW": 0.45, "MID": 0.18, "HIGH": 0.003}),
        "disp_thr":  disp.get("per_regime_threshold", {"LOW": 0.75, "MID": 0.40, "HIGH": 0.30}),
        "disp_scale":disp.get("per_regime_scale",     {"LOW": 0.60, "MID": 2.00, "HIGH": 0.40}),
...
if __name__ == "__main__":
    t0 = time.time()
    print("=" * 65)
    print("Phase 403 — Regime Weight Re-opt (31st Pass) [VECTORIZED]")
    print("After P355-P357 confirmed overlays/boundaries optimal")
    ...
    out = Path("artifacts/phase403"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase403_report.json").write_text(json.dumps(report, indent=2))
    ...
    if validated:
        ...
        msg = (f"feat: P403 regime weight reopt24 OBJ={reported_obj:.4f} "
