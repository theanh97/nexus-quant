#!/usr/bin/env python3
"""
Phase 260 — Vol Overlay Retune Round 2
Sweep vol_thr + vol_scale with updated v2.42.9 config.
Not retested since P249 (before V1 weights/regime changes in P255-257).
"""
...
def eval_vol(year_data, params, weights, vol_thr, vol_scale):
    p2 = dict(params); p2["vol_thr"] = vol_thr; p2["vol_scale"] = vol_scale
...
def main():
    t0 = time.time()
    print("=" * 60)
    print("Phase 260 — Vol Overlay Retune Round 2")
    print("Config: v2.42.9")
    print("=" * 60)
    ...
    print(f"  Current: vol_thr={params['vol_thr']:.2f}, vol_scale={params['vol_scale']:.2f}")

    # [2] vol_thr sweep
    print("\n[2] vol_thr sweep ...")
    thr_candidates = [0.30, 0.35, 0.40, 0.45, 0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80]
    best_thr = params["vol_thr"]; best_thr_obj = baseline_obj
    for thr in thr_candidates:
        obj = eval_vol(year_data, params, weights, thr, params["vol_scale"])
        marker = " <-" if obj > best_thr_obj else ""
        print(f"  vol_thr={thr:.2f}  OBJ={obj:.4f}{marker}")
        if obj > best_thr_obj:
            best_thr_obj = obj; best_thr = thr
    print(f"  Best vol_thr: {best_thr:.2f}  OBJ={best_thr_obj:.4f}  D={best_thr_obj-baseline_obj:+.4f}")

    # [3] vol_scale sweep (with best vol_thr)
    print("\n[3] vol_scale sweep ...")
    scale_candidates = [0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45,
                        0.50, 0.55, 0.60, 0.65, 0.70, 0.80, 1.00]
    best_scale = params["vol_scale"]; best_scale_obj = best_thr_obj
    for scale in scale_candidates:
        obj = eval_vol(year_data, params, weights, best_thr, scale)
        marker = " <-" if obj > best_scale_obj else ""
        print(f"  vol_scale={scale:.2f}  OBJ={obj:.4f}{marker}")
        if obj > best_scale_obj:
            best_scale_obj = obj; best_scale = scale
    print(f"  Best vol_scale: {best_scale:.2f}  OBJ={best_scale_obj:.4f}  D={best_scale_obj-baseline_obj:+.4f}")

    # [4] LOYO validation
    print("\n[4] LOYO validation ...")
    p_best = dict(params); p_best["vol_thr"] = best_thr; p_best["vol_scale"] = best_scale
    wins = 0
    for yr in YEARS:
    ...
    delta = best_scale_obj - baseline_obj
    print(f"\n  LOYO: {wins}/5 wins  delta={delta:+.4f}")
    print(f"  Best Vol: thr={best_thr:.2f} scale={best_scale:.2f}")

    if wins >= 3 and delta > 0.005:
        print(f"\n  VALIDATED  (LOYO {wins}/5 delta={delta:+.4f})")
        _update_config(cfg, best_thr, best_scale, best_scale_obj)
    else:
        print(f"\n  CONFIRMED OPTIMAL  (LOYO {wins}/5 delta={delta:+.4f})")

    print(f"\n[DONE] Phase 260 complete.  ({time.time()-t0:.1f}s)")

def _update_config(cfg, best_thr, best_scale, new_obj):
    old_ver = cfg.get("version", "v2.42.9")
    parts = old_ver.lstrip("v").split(".")
    parts[-1] = str(int(parts[-1]) + 1)
    new_ver = "v" + ".".join(parts)
    cfg["vol_regime_overlay"]["threshold"]    = round(best_thr, 3)
    cfg["vol_regime_overlay"]["scale_factor"] = round(best_scale, 3)
    cfg["version"] = new_ver
    p = Path("configs/production_p91b_champion.json")
    p.write_text(json.dumps(cfg, indent=2))
    print(f"  Config updated: {old_ver} → {new_ver}")
    print(f"  vol_thr={best_thr:.2f}, vol_scale={best_scale:.2f}")
    print(f"  New OBJ={new_obj:.4f}")
    subprocess.run(["git", "add", str(p)], cwd="/Users/truonglys/projects/quant")
    subprocess.run(["git", "commit", "-m",
        f"quant: {new_ver} — vol overlay retune r2 (thr={best_thr:.2f} scale={best_scale:.2f}) OBJ={new_obj:.4f}"],
