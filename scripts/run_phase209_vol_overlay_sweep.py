"""
Phase 209 — VOL Overlay Re-Sweep (scale × threshold)
======================================================
VOL overlay scales entire portfolio in high-vol regimes.
With F168 now 78% MID and V1 momentum-weighted (not carry),
optimal vol damping might differ from parallel P198 values.
All signals pre-computed — sweep is pure NumPy (fast).

Baseline: v2.32.0, OBJ=3.0860
Current: VOL_SCALE=0.30, VOL_THRESHOLD=0.50
Sweep: scale ∈ [0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.50, 0.60, 0.80, 1.00]
...
def _on_timeout(signum, frame):
    ...
    (out / "phase209_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
# Overlay constants (v2.32.0 correct — except vol which we sweep)
...
VOL_SCALE_BASE = 0.30; VOL_THRESHOLD_BASE = 0.50
...
BASELINE_OBJ = 3.0860
SCALE_SWEEP = [0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.50, 0.60, 0.80, 1.00]
THR_SWEEP   = [0.30, 0.40, 0.50, 0.60, 0.70, 0.80]
...
def ensemble_rets(vol_scale, vol_thr, base_data):
...
def evaluate(vol_scale, vol_thr, all_data):
    yearly = {yr: sharpe(ensemble_rets(vol_scale, vol_thr, d)) for yr, d in all_data.items()}
    return obj(yearly), yearly

def main():
    t0 = time.time()
    print("=" * 65)
    print("Phase 209 — VOL Overlay Re-Sweep (scale × threshold)")
    print(f"Baseline: v2.32.0  OBJ={BASELINE_OBJ}")
    print(f"Current: scale={VOL_SCALE_BASE}  thr={VOL_THRESHOLD_BASE}")
    print(f"Scale sweep: {SCALE_SWEEP}")
    print(f"Thr sweep:   {THR_SWEEP}")
    print("=" * 65)
    ...
    base_obj, base_yearly = evaluate(VOL_SCALE_BASE, VOL_THRESHOLD_BASE, all_data)
    print(f"\n  Baseline OBJ = {base_obj:.4f}  (expected ≈ {BASELINE_OBJ})")
    _partial.update({"baseline_obj": float(base_obj)})

    print("\n[2/3] 2D Sweep...")
    best_obj = base_obj; best_scale = VOL_SCALE_BASE; best_thr = VOL_THRESHOLD_BASE
    results = []
    for scale in SCALE_SWEEP:
        for thr in THR_SWEEP:
            o, _ = evaluate(scale, thr, all_data)
            delta = o - base_obj
            marker = " ← BEST" if o > best_obj else ""
            print(f"  scale={scale:.2f}  thr={thr:.2f}  OBJ={o:.4f}  Δ={delta:+.4f}{marker}")
            results.append((scale, thr, o))
            if o > best_obj:
                best_obj = o; best_scale = scale; best_thr = thr

    print(f"\n  → Best: scale={best_scale}  thr={best_thr}  OBJ={best_obj:.4f}")

    print("\n[3/3] LOYO validation...")
    joint_obj, joint_yearly = evaluate(best_scale, best_thr, all_data)
    ...
    print(f"\n{'✅ VALIDATED' if validated else '❌ NO IMPROVEMENT'} — scale={best_scale}  thr={best_thr}")

    result = {
        "phase": 209, "baseline_obj": float(base_obj), "best_obj": float(joint_obj),
        "delta": float(delta), "loyo_wins": loyo_wins, "loyo_total": len(joint_yearly),
        "validated": validated,
        "best_vol_scale": best_scale, "best_vol_thr": best_thr,
        "top5": sorted(results, key=lambda x: -x[2])[:5],
    }
    _partial.update(result)
    out = Path("artifacts/phase209"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase209_report.json").write_text(json.dumps(result, indent=2))

    if validated:
        cfg_path = ROOT / "configs" / "production_p91b_champion.json"
        cfg = json.load(open(cfg_path))
        vo = cfg.setdefault("vol_overlay_params", {})
        vo["vol_scale"]     = best_scale
        vo["vol_threshold"] = best_thr
        cfg["_version"] = "2.33.0"
