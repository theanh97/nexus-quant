#!/usr/bin/env python3
"""
Phase 262 — HIGH Regime Re-retune Round 3
After P261 changed p_high: 0.60 → 0.68 (HIGH now = brd_pct >= 0.68)
Sweep: A) f168 weight, B) v1 weight, C) idio_ratio (i460 vs i415 split)
Current HIGH: v1=0.20, i460bw168=0.18, i415bw216=0.27, f168=0.35
"""
...
def main():
    t0 = time.time()
    print("=" * 60)
    print("Phase 262 — HIGH Regime Re-retune Round 3")
    ...
    print(f"  idio_total={curr_idio:.3f} i460_ratio={curr_ratio:.3f}")
    ...
    # [4] idio_ratio sweep
    print("\n[4] idio_ratio sweep (i460 fraction of total idio) ...")
    ...
    print(f"\n[DONE] Phase 262 complete.  ({time.time()-t0:.1f}s)")

def _update_config(cfg, bw, new_obj):
    old_ver = cfg.get("version", "v2.42.12")
    ...
    subprocess.run(["git", "commit", "-m",
        f"quant: {new_ver} — HIGH regime retune r3 (v1={bw['v1']:.2f} f168={bw['f168']:.2f}) OBJ={new_obj:.4f}"],
