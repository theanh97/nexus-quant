"""
Phase 217 — V1 Internal Weights Re-Tune
========================================
V1 signal blends 3 sub-factors: carry (funding momentum), momentum (price), mean-reversion.
Last tuned P185 at OBJ≈2.59. Then phase215 changed to wc=0.25/wm=0.45/wr=0.30.
Current OBJ≈3.27. Optimal blend may have shifted significantly.

V1 sub-factor weights constraint: w_carry + w_mom + w_mr = 1.0
Current: wc=0.25, wm=0.45, wr=0.30

Part A: w_carry ∈ [0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40]
  fix w_mr=0.30, w_mom = 1 - w_carry - w_mr

Part B: w_mr ∈ [0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45]
  use best w_carry, w_mom = 1 - best_wc - w_mr

I460/I415/F168 loaded once (fixed). V1 re-run for each weight combo.
LOYO vs current config.
"""
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase217"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase217_report.json").write_text(json.dumps(_partial, indent=2, default=str))
    sys.exit(0)
...
WC_SWEEP = [0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40]
WR_SWEEP = [0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45]
...
def get_config():
    ...
    params["v1_wc"]   = v1p.get("w_carry", 0.25)
    params["v1_wm"]   = v1p.get("w_mom", 0.45)
    params["v1_wr"]   = v1p.get("w_mean_reversion", 0.30)
    params["v1_k"]    = v1p.get("k_per_side", 2)
    params["v1_mlb"]  = v1p.get("momentum_lookback_bars", 336)
    params["v1_mrlb"] = v1p.get("mean_reversion_lookback_bars", 84)
    params["v1_vlb"]  = v1p.get("vol_lookback_bars", 192)
    params["v1_lev"]  = v1p.get("target_gross_leverage", 0.35)
    params["v1_reb"]  = v1p.get("rebalance_interval_bars", 60)
...
def load_year_fixed_other(year, params):
    """Load I460/I415/F168 fixed + overlays. Returns dict with all except V1."""
    p = params
    ...
    return dataset, btc_vol, fund_std_pct, ts_spread_pct, regime, i460_rets, i415_rets, f168_rets, n

def run_v1(dataset, wc, wm, wr, params):
    p = params
    res = BacktestEngine(BacktestConfig(costs=COST_MODEL)).run(
        dataset, make_strategy({"name": "nexus_alpha_v1", "params": {
            "k_per_side": p["v1_k"],
            "w_carry": wc, "w_mom": wm, "w_mean_reversion": wr,
            "momentum_lookback_bars": p["v1_mlb"],
            "mean_reversion_lookback_bars": p["v1_mrlb"],
            "vol_lookback_bars": p["v1_vlb"],
            "target_gross_leverage": p["v1_lev"],
            "rebalance_interval_bars": p["v1_reb"],
        }}))
    return np.array(res.returns)

def compute_ensemble(base_data, v1_rets, weights, params):
    p = params
    _, btc_vol, fund_std_pct, ts_spread_pct, regime, i460_rets, i415_rets, f168_rets, n = base_data
...
# ── MAIN ────────────────────────────────────────────────────────────────────
print("=" * 60)
print("Phase 217 — V1 Internal Weights Re-Tune")
print("=" * 60)
_start = time.time()

prod_cfg, WEIGHTS, PARAMS, baseline_obj = get_config()
ver_now = prod_cfg.get("_version", "2.35.0")
curr_wc = PARAMS["v1_wc"]; curr_wm = PARAMS["v1_wm"]; curr_wr = PARAMS["v1_wr"]
print(f"  Config: {ver_now}  baseline_obj={baseline_obj:.4f}")
print(f"  V1 curr: wc={curr_wc}  wm={curr_wm}  wr={curr_wr}")

print("\n[1] Loading all years (I460/I415/F168 fixed) ...")
year_data = {}
for yr in YEARS:
    print(f"  {yr}", end="", flush=True)
    year_data[yr] = load_year_fixed_other(yr, PARAMS)
    print()

def eval_v1_weights(wc, wm, wr):
    yr_sharpes = {}
    for yr in YEARS:
        ds = year_data[yr][0]
        v1_rets = run_v1(ds, wc, wm, wr, PARAMS)
        ens = compute_ensemble(year_data[yr], v1_rets, WEIGHTS, PARAMS)
        yr_sharpes[yr] = sharpe(ens, year_data[yr][-1])
    return obj_fn(yr_sharpes), yr_sharpes

# Current baseline
curr_obj, curr_yr = eval_v1_weights(curr_wc, curr_wm, curr_wr)
print(f"\n  Baseline OBJ (computed): {curr_obj:.4f}")

# Part A: w_carry sweep (w_mr=0.30 fixed)
fixed_wr = curr_wr
print(f"\n[2] Part A: w_carry sweep (w_mr={fixed_wr:.2f} fixed) ...")
results_a = []
for wc in WC_SWEEP:
    wm = round(1.0 - wc - fixed_wr, 4)
    if wm <= 0 or wm >= 1: continue
    obj, yr_s = eval_v1_weights(wc, wm, fixed_wr)
    results_a.append((wc, wm, fixed_wr, obj, yr_s))
    print(f"  wc={wc:.2f} wm={wm:.2f} wr={fixed_wr:.2f}  OBJ={obj:.4f}")

best_a = max(results_a, key=lambda x: x[3])
best_wc_a, best_wm_a, _, best_obj_a, _ = best_a
print(f"  Best wc: {best_wc_a:.2f} (wm={best_wm_a:.2f})  OBJ={best_obj_a:.4f}")

# Part B: w_mr sweep (best w_carry)
print(f"\n[3] Part B: w_mr sweep (wc={best_wc_a:.2f}) ...")
results_b = []
for wr in WR_SWEEP:
    wm = round(1.0 - best_wc_a - wr, 4)
    if wm <= 0 or wm >= 1: continue
    obj, yr_s = eval_v1_weights(best_wc_a, wm, wr)
    results_b.append((best_wc_a, wm, wr, obj, yr_s))
    print(f"  wc={best_wc_a:.2f} wm={wm:.2f} wr={wr:.2f}  OBJ={obj:.4f}")

best_b = max(results_b, key=lambda x: x[3])
best_wc, best_wm, best_wr, best_obj, best_yr = best_b
delta = best_obj - curr_obj
print(f"  Best: wc={best_wc:.2f} wm={best_wm:.2f} wr={best_wr:.2f}  OBJ={best_obj:.4f}  Δ={delta:+.4f}")
...
out = Path("artifacts/phase217"); out.mkdir(parents=True, exist_ok=True)
report = {
    "phase": 217, "description": "V1 internal weights re-tune",
    "elapsed_seconds": round(time.time() - _start, 1),
    "baseline_obj_string": baseline_obj, "curr_obj_computed": curr_obj,
    "best_obj": best_obj, "delta": delta,
    "curr": {"wc": curr_wc, "wm": curr_wm, "wr": curr_wr},
    "best": {"wc": best_wc, "wm": best_wm, "wr": best_wr},
    "loyo_wins": loyo_wins, "validated": validated,
    "yearly_best": best_yr, "yearly_curr": curr_yr,
    "wc_sweep": [(r[0], r[1], r[3]) for r in results_a],
    "wr_sweep": [(r[0], r[1], r[2], r[3]) for r in results_b],
}
(out / "phase217_report.json").write_text(json.dumps(report, indent=2, default=str))
...
if validated:
    ...
    cfg["_validated"] = old_val + (
        "; V1 weights P217: wc=%.2f->%.2f wm=%.2f->%.2f wr=%.2f->%.2f LOYO %d/5 delta=%+.4f OBJ=%.4f — PRODUCTION %s" % (
            curr_wc, best_wc, curr_wm, best_wm, curr_wr, best_wr,
    ...
    cm = "phase217: V1 internal weights -- VALIDATED wc=%.2f wm=%.2f wr=%.2f LOYO %d/5 delta=%+.4f OBJ=%.4f" % (
        best_wc, best_wm, best_wr, loyo_wins, delta, best_obj)
else:
    new_ver = ver_now
    print(f"\n⚠️ NO IMPROVEMENT — V1 weights near-optimal (LOYO {loyo_wins}/5 delta={delta:+.4f})")
    cm = "phase217: V1 internal weights -- WEAK LOYO %d/5 delta=%+.4f wc=%.2f wm=%.2f wr=%.2f" % (
        loyo_wins, delta, best_wc, best_wm, best_wr)

os.system("git add configs/production_p91b_champion.json scripts/run_phase217_v1_weights_retune.py "
          "artifacts/phase217/ 2>/dev/null || true")
os.system('git commit -m "' + cm + '"')
os.system("git stash && git pull --rebase && git stash pop && git push")

print(f"\n[DONE] Phase 217 complete.")
