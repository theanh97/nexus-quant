"""
Phase 416 — Per-Regime FTS Retune [VECTORIZED]
============================================================================
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase416_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase416_fts_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 416 — FTS Retune [VECTORIZED]")
    print("=" * 70)
    ...
    out = Path("artifacts/phase416_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase416_fts_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase416_fts/phase416_fts_report.json")
    ...
    if validated:
        ...
        msg = (f"feat: P416 FTS retune OBJ={reported_obj} "
