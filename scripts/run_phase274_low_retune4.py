#!/usr/bin/env python3
"""
Phase 274 — LOW Regime Retune Round 4
After HIGH/MID/Vol retune, re-optimize LOW regime weights.
LOW = breadth_pct < p_low (0.30) — defensive regime with scarce bars.
...
def main():
    ...
    print(f"    Current LOW: v1={cl['v1']} i460={cl['i460']} i415={cl['i415']} f168={cl['f168']}")
    ...
    iratio_cur = (cl["i460"] / (cl["i460"] + cl["i415"])) if (cl["i460"] + cl["i415"]) > 0 else 0.33
    ...
    for f168_w in [0.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40,
        ...
        w = make_low_w(base_weights, f168_w, v1_cur, iratio_cur)
    ...
    print(f"  Best f168: {best_f168}  D={best_obj-curr_obj:+.4f}")

    # [4] Sweep v1
    print("\n[4] v1 sweep ...")
    best_v1 = v1_cur
    for v1_w in [0.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50]:
        w = make_low_w(base_weights, best_f168, v1_w, iratio_cur)
        o = eval_weights(yearly_data, params, w)
        marker = " <--" if o > best_obj else ""
        print(f"  v1={v1_w:.2f}  OBJ={o:.4f}{marker}", flush=True)
        if o > best_obj:
            best_obj = o; best_v1 = v1_w
    print(f"  Best v1: {best_v1}  D={best_obj-curr_obj:+.4f}")

    # [5] Sweep idio_ratio
    print("\n[5] idio_ratio sweep ...")
    best_ratio = iratio_cur
    for ratio in [0.00, 0.10, 0.20, 0.30, 0.33, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 1.00]:
    ...
    print(f"  Best ratio: {best_ratio}  D={best_obj-curr_obj:+.4f}")
    ...
    print(f"  Best LOW: v1={bl['v1']} i460={bl['i460']} i415={bl['i415']} f168={bl['f168']}")
    ...
    if wins >= 3 and delta > 0.005:
        ...
        if "i460bw168" in rw2["LOW"]:
            rw2["LOW"]["i460bw168"] = bl["i460"]
        else:
            rw2["LOW"]["i460"] = bl["i460"]
        if "i415bw216" in rw2["LOW"]:
            rw2["LOW"]["i415bw216"] = bl["i415"]
        else:
            rw2["LOW"]["i415"] = bl["i415"]
        rw2["LOW"]["f168"] = bl["f168"]
        p = Path("configs/production_p91b_champion.json")
        p.write_text(json.dumps(cfg2, indent=2))
        print(f"  Config updated: {old_ver} → {new_ver}")
        print(f"  LOW: v1={bl['v1']} i460={bl['i460']} i415={bl['i415']} f168={bl['f168']}")
        print(f"  New OBJ={best_obj:.4f}")
        msg = (f"quant: {new_ver} — LOW retune r4 "
               f"(v1={bl['v1']:.2f} f168={bl['f168']:.2f}) "
        ...
        print(f"  LOW stays: v1={cl['v1']} i460={cl['i460']} i415={cl['i415']} f168={cl['f168']}")
