"""
Phase 335 — Per-Regime Vol + Disp Overlay Re-Tune [VECTORIZED]
================================================================
Baseline: v2.49.40, OBJ=6.7382 (P334 V1 weight resweep — VALIDATED D=+0.1588)

After full cycle P330-P334 with P334 validated (V1 weights),
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase335"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase335_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 335 — Vol + Disp Overlay Re-Tune [VECTORIZED]")
    print("=" * 70)
    ...
    result = {
        "phase": "335", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    out = Path("artifacts/phase335"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase335_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        msg = (f"feat: P335 vol+disp retune OBJ={reported_obj:.4f} "
