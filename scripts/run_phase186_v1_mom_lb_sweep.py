"""
Phase 186 — V1 Momentum Lookback Re-Sweep
==========================================
Motivation: P185 reduced V1 w_mom from 0.45→0.40. With different internal
weight balance, optimal momentum_lookback_bars may have shifted from 336h.

Sweep:
  momentum_lookback_bars ∈ [168, 240, 288, 336, 384, 480, 672]

Baseline: v2.19.0, OBJ=2.5945 (V1: wc=0.35, wm=0.40, wr=0.25)
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase186"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase186_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
# v2.19.0 V1 weights (P185)
V1_W_CARRY = 0.35; V1_W_MOM = 0.40; V1_W_MR = 0.25
V1_MOM_LB_BASE = 336
V1_MR_LB       = 72

BASELINE_OBJ = 2.5945
SWEEP_LBS    = [168, 240, 288, 336, 384, 480, 672]
...
def load_year_data(year: int):
    ...
    # Pre-compute i460, i415, f168 (fixed)
    sig_rets = {}
    for sk, sname, params in [
        ("v1_base", "nexus_alpha_v1", {
            "k_per_side": 2, "w_carry": V1_W_CARRY, "w_mom": V1_W_MOM,
            "w_mean_reversion": V1_W_MR, "momentum_lookback_bars": V1_MOM_LB_BASE,
            "mean_reversion_lookback_bars": V1_MR_LB, "vol_lookback_bars": 168,
...
def run_v1_mom_lb(base_data, mom_lb: int) -> np.ndarray:
    dataset, btc_vol, fund_std_pct, ts_spread_pct, regime, sig_rets, n = base_data
    if mom_lb == V1_MOM_LB_BASE:
...
def main():
    t0 = time.time()
    print("=" * 65)
    print("Phase 186 — V1 Momentum Lookback Re-Sweep")
    print(f"Baseline: v2.19.0  OBJ={BASELINE_OBJ}  mom_lb={V1_MOM_LB_BASE}")
    print(f"Sweep:    momentum_lookback_bars ∈ {SWEEP_LBS}")
    print("=" * 65)
    ...
    bl_yr = {yr: sharpe(run_v1_mom_lb(data_by_yr[yr], V1_MOM_LB_BASE)) for yr in YEARS}
    bl_obj = obj(bl_yr)
    print(f"  Baseline OBJ = {bl_obj:.4f}  (expected ≈ {BASELINE_OBJ})")

    print(f"\n[2/3] Sweeping momentum_lookback_bars ...")
    sweep_results: dict = {}
    best_lb, best_obj_val = V1_MOM_LB_BASE, bl_obj

    for lb in SWEEP_LBS:
        yr_sharpes = {yr: sharpe(run_v1_mom_lb(data_by_yr[yr], lb)) for yr in YEARS}
        ...
        flag = ("⭐" if (o > best_obj_val and loyo_wins >= 3 and d > 0.005) else
                ...
                ("⚠️ " if lb == V1_MOM_LB_BASE else "❌")))
        base_marker = " ← baseline" if lb == V1_MOM_LB_BASE else ""
    ...
    if best_lb == V1_MOM_LB_BASE:
        print("  ❌ NO IMPROVEMENT — lb=336 already optimal")
        ...
        for loyo_yr in YEARS:
            ...
            best_train = {y: sharpe(run_v1_mom_lb(data_by_yr[y], best_lb)) for y in train_yrs}
    ...
    if validated:
        print(f"\n✅ VALIDATED — momentum_lookback_bars={best_lb}  OBJ={best_obj_val:.4f}")
    else:
        print(f"\n❌ NO IMPROVEMENT — momentum_lookback_bars={V1_MOM_LB_BASE}  OBJ={bl_obj:.4f} optimal")

    report = {
        "phase": 186, "target": "v1_momentum_lookback_bars",
        "sweep": {str(lb): {"obj": sweep_results[lb]["obj"], "delta": sweep_results[lb]["delta"],
                             "loyo_wins": sweep_results[lb]["loyo_wins"]}
                  for lb in SWEEP_LBS},
        "baseline": {"obj": bl_obj, "lb": V1_MOM_LB_BASE},
        "best":     {"obj": best_obj_val, "lb": best_lb},
        "loyo":     {"wins": loyo_wins_final if best_lb != V1_MOM_LB_BASE else 0,
                     "avg_delta": loyo_avg_d if best_lb != V1_MOM_LB_BASE else 0.0},
        "validated": validated, "runtime_s": round(time.time() - t0, 1),
    }
    out_dir = ROOT / "artifacts" / "phase186"
    out_dir.mkdir(parents=True, exist_ok=True)
    (out_dir / "phase186_report.json").write_text(json.dumps(report, indent=2))
    print(f"\nReport → artifacts/phase186/phase186_report.json")
    print(f"Runtime: {report['runtime_s']:.0f}s")

    if validated:
        cfg_path = ROOT / "configs" / "production_p91b_champion.json"
        cfg = json.loads(cfg_path.read_text())
        cfg["ensemble"]["signals"]["v1"]["params"]["momentum_lookback_bars"] = best_lb
        old_v = cfg["_version"]; parts = old_v.split(".")
        new_v = f"{parts[0]}.{int(parts[1]) + 1}.0" if len(parts) >= 2 else old_v
        cfg["_version"] = new_v
        cfg["_validated"] += (
            f"; V1 mom_lb re-sweep P186: {V1_MOM_LB_BASE}→{best_lb} LOYO {loyo_wins_final}/5 "
