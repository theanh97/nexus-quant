"""
Phase 306 — Per-Regime FTS Retune after P304-P305 Signal Shift [VECTORIZED]
============================================================================
Baseline: v2.49.27, OBJ=5.9352 (Phase 305 vol+disp retune, LOYO=3/5)

After P304 (V1 weights) and P305 (vol+disp retune),
the signal landscape has shifted. Re-tune FTS overlay params.

Current FTS (from P290, confirmed in P296, P301):
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase306_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase306_fts_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 306 — FTS Retune after P304-P305 Signal Shift [VECTORIZED]")
    print("Baseline: v2.49.27 OBJ=5.9352")
    print("=" * 70)
    ...
    result = {
        "phase": "306", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    out = Path("artifacts/phase306_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase306_fts_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase306_fts/phase306_fts_report.json")
    ...
    if validated:
        ...
        old_ver = cfg.get("_version", "2.49.27")
        parts = old_ver.split(".")
        parts[-1] = str(int(parts[-1]) + 1)
        new_ver = ".".join(parts)
        cfg["_version"] = new_ver
        old_vver = cfg.get("version", "v2.49.27").lstrip("v")
        ...
        msg = (f"feat: P306 FTS retune OBJ={reported_obj} "
