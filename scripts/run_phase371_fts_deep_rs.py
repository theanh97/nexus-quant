"""
Phase 371 — Per-Regime FTS Retune [VECTORIZED]
============================================================================
Baseline: v2.49.40, OBJ=6.7382 (P360 vol/disp retune — no change)

Continue R&D cycle: re-sweep FTS overlay params.

Current FTS (confirmed optimal in P296-P346, P351):
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase371_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase371_fts_report.json").write_text(json.dumps(_partial, indent=2))
...
RS_GRID  = [0.0, 0.001, 0.002, 0.003, 0.004, 0.005, 0.008, 0.01, 0.015, 0.02, 0.03, 0.05, 0.10, 0.20]
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 371 — FTS Retune [VECTORIZED]")
    print("=" * 70)
    ...
    result = {
        "phase": "356", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    out = Path("artifacts/phase371_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase371_fts_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase371_fts/phase371_fts_report.json")
    ...
    if validated:
        ...
        msg = (f"feat: P371 FTS retune OBJ={reported_obj} "
