"""
Phase 301 — Per-Regime FTS Retune after P298-P300 Signal Shift [VECTORIZED]
============================================================================
Baseline: v2.49.25, OBJ=5.8110 (Phase 300 vol+disp retune, LOYO=3/5)

After P298 (V1 weights + regime weights) and P300 (vol+disp retune),
the signal landscape has shifted. Re-tune FTS overlay params.

Current FTS (from P290, confirmed in P296):
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase301_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase301_fts_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 301 — FTS Retune after P298-P300 Signal Shift [VECTORIZED]")
    ...
    result = {
        "phase": "301", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    out = Path("artifacts/phase301_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase301_fts_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase301_fts/phase301_fts_report.json")
    ...
    if validated:
        ...
        msg = (f"feat: P301 FTS retune OBJ={reported_obj} "
