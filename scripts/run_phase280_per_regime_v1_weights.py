"""
Phase 280 — Per-Regime V1 Internal Weights Re-Sweep [VECTORIZED, MEM-EFFICIENT]
================================================================================
Baseline: v2.49.2, OBJ=4.2403 (Phase 279 FTS resweep, LOYO=3/5)

P244 set V1 per-regime weights at OBJ~3.79. Since then regime weights changed
dramatically (P271) and FTS overlay was retuned (P279). The V1 internals
(carry/mom/mean_reversion) likely have a new optimum.

Current V1 weights:
  LOW:  wc=0.10, wm=0.45, wmr=0.45  (mean-reversion heavy)
  MID:  wc=0.25, wm=0.50, wmr=0.25  (momentum default)
  HIGH: wc=0.25, wm=0.50, wmr=0.25  (momentum default)
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase280"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase280_report.json").write_text(json.dumps(_partial, indent=2))
...
if __name__ == "__main__":
    t0 = time.time()
    print("=" * 70)
    print("Phase 280 — Per-Regime V1 Weights Re-Sweep [MEM-EFFICIENT]")
    print("Baseline: P279 FTS resweep")
    print("=" * 70)
    ...
    result = {
        "phase": "280", "baseline_obj": float(obj_base), "stored_baseline": float(base_obj),
    ...
    out = Path("artifacts/phase280"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase280_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        msg = (f"feat: P280 per-regime V1 weights OBJ={reported_obj:.4f} "
