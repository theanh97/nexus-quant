#!/usr/bin/env python3
"""
Phase 253 — I415 Signal Parameter Sweep
Current: i415 lb=415, bw=144 (i415bw216 signal)
Sweep: A) lookback_bars, B) beta_window_bars
"""
...
def load_year(year, params, v1p, i460p, i415p, f168p):
    ...
    # Return preloaded dataset handle for i415 variations
    return dataset, n, btc_vol, ts_spread_pct, brd_pct, v1_r, i460_r, f168_r, i415p

def run_i415(dataset, i415p, lb, bw):
    res = BacktestEngine(BacktestConfig(costs=COST_MODEL)).run(
        dataset, make_strategy({"name": "idio_momentum_alpha", "params": {
...
def compute_obj(base_data, i415_r, params, weights):
    sharpes = []
    for yr in YEARS:
        dataset, n, btc_vol, ts_spread_pct, brd_pct, v1_r, i460_r, f168_r, _ = base_data[yr]
        i415_arr = i415_r[yr]
        p = params; w = weights
        v1r = v1_r; i460r = i460_r; f168r = f168_r
        min_len = min(n, len(v1r), len(i460r), len(i415_arr), len(f168r))
        brd = brd_pct; tsp = ts_spread_pct; vol = btc_vol
        ...
        for i in range(min_len):
            ...
            r = (ww["v1"]*v1r[i] + ww["i460"]*i460r[i] +
                 ww["i415"]*i415_arr[i] + ww["f168"]*f168r[i])
            ts = tsp[i]
...
def main():
    t0 = time.time()
    print("=" * 60)
    print("Phase 253 — I415 Signal Parameter Sweep")
    print("=" * 60)

    cfg, weights, params, v1p, i460p, i415p, f168p = get_config()
    cur_lb = i415p.get("lookback_bars", 415)
    cur_bw = i415p.get("beta_window_bars", 144)
    print(f"  Current i415: lb={cur_lb} bw={cur_bw}")

    print("\n[1] Pre-loading base data (v1, i460, f168) for all years ...")
    base_data = {}
    for yr in YEARS:
    ...
    # Compute baseline (current lb, bw)
    base_i415 = {yr: run_i415(base_data[yr][0], i415p, cur_lb, cur_bw) for yr in YEARS}
    base_obj, base_sharpes = compute_obj(base_data, base_i415, params, weights)
    print(f"  Baseline OBJ: {base_obj:.4f}  (lb={cur_lb} bw={cur_bw})")

    best_obj = base_obj; best_lb = cur_lb; best_bw = cur_bw

    # Part A: lb sweep (bw fixed)
    print(f"\n[2] Part A: i415 lookback_bars sweep (bw={cur_bw} fixed) ...")
    lb_cands = [300, 336, 360, 390, 415, 450, 480, 504, 528, 560, 600]
    lb_results = {}
    for lb in lb_cands:
        i415_r = {yr: run_i415(base_data[yr][0], i415p, lb, cur_bw) for yr in YEARS}
        obj, sharpes = compute_obj(base_data, i415_r, params, weights)
        lb_results[lb] = (obj, sharpes, i415_r)
        marker = " <-" if obj > best_obj + 1e-6 else ""
        print(f"  lb={lb:3d} bw={cur_bw}  OBJ={obj:.4f}{marker}")
        if obj > best_obj + 1e-6:
            best_obj = obj; best_lb = lb
    print(f"  Best lb: {best_lb}  OBJ={best_obj:.4f}")

    # Part B: bw sweep (best lb fixed)
    print(f"\n[3] Part B: i415 beta_window_bars sweep (lb={best_lb} fixed) ...")
    bw_cands = [96, 112, 120, 128, 144, 160, 168, 192, 216, 240]
    bw_results = {}
    for bw in bw_cands:
        i415_r = {yr: run_i415(base_data[yr][0], i415p, best_lb, bw) for yr in YEARS}
        obj, sharpes = compute_obj(base_data, i415_r, params, weights)
        bw_results[bw] = (obj, sharpes, i415_r)
        marker = " <-" if obj > best_obj + 1e-6 else ""
        print(f"  lb={best_lb} bw={bw:3d}  OBJ={obj:.4f}{marker}")
        if obj > best_obj + 1e-6:
            best_obj = obj; best_bw = bw
    delta = best_obj - base_obj
    print(f"  Best bw: {best_bw}  OBJ={best_obj:.4f}  D={delta:+.4f}")

    # LOYO
    if best_bw in bw_results:
        best_sharpes = bw_results[best_bw][1]
    elif best_lb != cur_lb and best_lb in lb_results:
        best_sharpes = lb_results[best_lb][1]
    else:
    ...
    print(f"  Best i415: lb={best_lb} bw={best_bw}")

    if loyo_wins >= 3 and delta > 0.005:
        print("\n  VALIDATED — updating config ...")
        cfg["ensemble"]["signals"]["i415bw216"]["params"]["lookback_bars"]    = best_lb
        ...
        try:
            ...
            msg = (f"feat: {new_ver} Phase253 i415 lb={best_lb} bw={best_bw} — "
