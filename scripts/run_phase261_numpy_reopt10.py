"""
Phase 261 — NumPy Regime Weight Re-opt (10th Pass) [VECTORIZED]
================================================================
Baseline: v2.45.0, OBJ=3.9273

Issue: P260 used mom_lb=312h (Engine track config) instead of NumPy-optimal 336h.
This pass restores mom_lb=336h for V1 and re-sweeps regime weights.

Per-regime V1 weights (P244, NumPy):
  LOW:  wc=0.10, wm=0.45, wmr=0.45, mom_lb=336h, mr_lb=84h
  MID:  wc=0.25, wm=0.50, wmr=0.25, mom_lb=336h, mr_lb=84h
  HIGH: wc=0.25, wm=0.50, wmr=0.25, mom_lb=336h, mr_lb=84h

Baseline weights (P260 result):
  LOW:  v1=0.40, i460=0.00, i415=0.20, f168=0.40
  MID:  v1=0.10, i460=0.00, i415=0.25, f168=0.65
  HIGH: v1=0.20, i460=0.80, i415=0.00, f168=0.00
"""
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase261n"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase261n_report.json").write_text(json.dumps(_partial, indent=2))
    sys.exit(0)
...
# Per-regime FTS overlays (P229-P230, NumPy-specific)
...
# NumPy-optimal V1 params (mom_lb=336h restored, per-regime weights from P244)
V1_LOW_PARAMS = {
...
BASELINE_OBJ = 3.9273  # P260 result
MIN_DELTA = 0.005; MIN_LOYO = 3
W_STEPS = [round(x * 0.05, 2) for x in range(17)]  # 0.00..0.80
...
def precompute_fast_eval_data(all_base_data, all_v1_rets):
    ...
    for yr, base_data in sorted(all_base_data.items()):
        ...
        bpct = brd_pct[:min_len]; bv = btc_vol[:min_len]; fsp = fund_std_pct[:min_len]
        ...
        for ridx, rname in enumerate(RNAMES):
            m = masks[ridx]
            overlay_mult[m & ~np.isnan(bv) & (bv > VOL_THR[rname])] *= VOL_SCALE[rname]
            overlay_mult[m & (fsp > DISP_THR[rname])] *= DISP_SCALE[rname]
...
def main():
    t0 = time.time()
    print("=" * 65)
    print("Phase 261 NumPy — Regime Weight Re-opt (10th Pass) [VEC]")
    print(f"Baseline: v2.45.0  OBJ={BASELINE_OBJ}  (mom_lb restored: 336h)")
    print("=" * 65)
    ...
    print("\n[2] Pre-computing V1 returns (mom_lb=336h, 2 variants)...", flush=True)
    all_v1_rets = {}
    for yr in sorted(YEAR_RANGES):
        ds = all_base_data[yr][6]
        v1_low     = compute_v1(ds, V1_LOW_PARAMS)
        v1_midhigh = compute_v1(ds, V1_MIDHIGH_PARAMS)
        all_v1_rets[yr] = {"LOW": v1_low, "MID": v1_midhigh, "HIGH": v1_midhigh}
    ...
    print(f"\n  Baseline OBJ (with mom_lb=336h) = {base_obj_val:.4f}", flush=True)
    ...
    result = {
        "phase": "261n", "baseline_obj": float(base_obj_val),
    ...
    out = Path("artifacts/phase261n"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase261n_report.json").write_text(json.dumps(result, indent=2))
    ...
    if validated:
        ...
        print(f"\n  Config → v2.46.0  OBJ={round(final_obj_val,4)}", flush=True)

        subprocess.run(["git", "add", str(cfg_path)], cwd=ROOT)
        msg = (f"feat: P261n regime weight reopt10 OBJ={round(final_obj_val,4)} "
               f"LOYO={loyo_wins}/5 D={delta:+.4f} [v2.46.0]")
