#!/usr/bin/env python3
"""
Phase 270 — HIGH Regime Re-retune Round 7
Parallel session set HIGH: v1=0.5 i460=0.5 i415=0.0 f168=0.0
Re-validate funding contrarian contribution under current config.
Comprehensive sweep: f168, v1, idio_ratio.
"""
...
def main():
    t0 = time.time()
    print("=" * 60)
    print("Phase 270 — HIGH Regime Re-retune Round 7")
    print("Re-validating f168 contribution (parallel set f168=0.0)")
    print("=" * 60)
    ...
    curr_ratio = curr_high["i460"] / curr_idio if curr_idio > 0 else 0.50
    ...
    # [2] f168 sweep (wide range — check if 0.0 is really optimal)
    print("\n[2] f168 sweep ...")
    ...
    # [3] v1 sweep
    print("\n[3] v1 sweep ...")
    v1_candidates = [0.10, 0.15, 0.18, 0.20, 0.22, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50]
    ...
    # [4] idio_ratio sweep
    print("\n[4] idio_ratio sweep ...")
    ratio_candidates = [0.10, 0.20, 0.30, 0.35, 0.40, 0.45, 0.50, 0.55, 0.60, 0.70, 0.80, 0.90]
...
def _update_config(cfg, bw, new_obj):
    ...
    subprocess.run(["git", "commit", "-m",
        f"quant: {new_ver} — HIGH retune r7 (v1={bw['v1']:.2f} f168={bw['f168']:.2f}) OBJ={new_obj:.4f}"],
