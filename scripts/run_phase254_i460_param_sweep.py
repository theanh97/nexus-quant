#!/usr/bin/env python3
"""
Phase 254 — I460 Signal Parameter Sweep
Current: i460 lb=480, bw=168
...
def load_year(year, params, v1p, i460p, i415p, f168p):
    ...
    return dataset, n, btc_vol, ts_spread_pct, brd_pct, v1_r, i415_r, f168_r, i460p

def run_i460(dataset, i460p, lb, bw):
    res = BacktestEngine(BacktestConfig(costs=COST_MODEL)).run(
...
def compute_obj(base_data, i460_r, params, weights):
    sharpes = []
    for yr in YEARS:
        dataset, n, btc_vol, ts_spread_pct, brd_pct, v1_r, i415_r, f168_r, _ = base_data[yr]
        i460_arr = i460_r[yr]
        p = params; w = weights
        min_len = min(n, len(v1_r), len(i460_arr), len(i415_r), len(f168_r))
        ...
        for i in range(min_len):
            ...
            r = (ww["v1"]*v1_r[i] + ww["i460"]*i460_arr[i] +
...
def main():
    t0 = time.time()
    print("=" * 60)
    print("Phase 254 — I460 Signal Parameter Sweep")
    print("=" * 60)

    cfg, weights, params, v1p, i460p, i415p, f168p = get_config()
    cur_lb = i460p.get("lookback_bars", 480)
    cur_bw = i460p.get("beta_window_bars", 168)
    print(f"  Current i460: lb={cur_lb} bw={cur_bw}")

    print("\n[1] Pre-loading base data (v1, i415, f168) for all years ...")
    base_data = {}
    for yr in YEARS:
    ...
    # Compute baseline
    base_i460 = {yr: run_i460(base_data[yr][0], i460p, cur_lb, cur_bw) for yr in YEARS}
    base_obj, base_sharpes = compute_obj(base_data, base_i460, params, weights)
    print(f"  Baseline OBJ: {base_obj:.4f}  (lb={cur_lb} bw={cur_bw})")

    best_obj = base_obj; best_lb = cur_lb; best_bw = cur_bw

    # Part A: lb sweep
    print(f"\n[2] Part A: i460 lookback_bars sweep (bw={cur_bw} fixed) ...")
    lb_cands = [360, 390, 420, 450, 480, 504, 528, 560, 600, 650, 700]
    lb_results = {}
    for lb in lb_cands:
        i460_r = {yr: run_i460(base_data[yr][0], i460p, lb, cur_bw) for yr in YEARS}
        obj, sharpes = compute_obj(base_data, i460_r, params, weights)
        lb_results[lb] = (obj, sharpes, i460_r)
    ...
    # Part B: bw sweep
    print(f"\n[3] Part B: i460 beta_window_bars sweep (lb={best_lb} fixed) ...")
    bw_cands = [120, 144, 156, 168, 180, 192, 210, 224, 240, 256]
    bw_results = {}
    for bw in bw_cands:
        i460_r = {yr: run_i460(base_data[yr][0], i460p, best_lb, bw) for yr in YEARS}
        obj, sharpes = compute_obj(base_data, i460_r, params, weights)
        bw_results[bw] = (obj, sharpes, i460_r)
        marker = " <-" if obj > best_obj + 1e-6 else ""
    ...
    # LOYO
    if best_bw != cur_bw and best_bw in bw_results:
    ...
    if loyo_wins >= 3 and delta > 0.005:
        ...
        try:
            ...
            msg = (f"feat: {new_ver} Phase254 i460 lb={best_lb} bw={best_bw} — "
