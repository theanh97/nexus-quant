"""
Phase 316 — Per-Regime FTS Retune [VECTORIZED]
============================================================================
Baseline: v2.49.32, OBJ=6.1979 (P314 V1 weight resweep)

After P314 changed LOW V1 weights (wc=0.15 wm=0.45 wmr=0.40),
re-sweep FTS overlay in case the V1 weight changes shifted the FTS landscape.

Current FTS (confirmed optimal in P296, P301, P306, P311):
...
def _on_timeout(signum, frame):
    ...
    out = Path("artifacts/phase316_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase316_fts_report.json").write_text(json.dumps(_partial, indent=2))
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Phase 316 — FTS Retune [VECTORIZED]")
    print("=" * 70)
    ...
    result = {
        "phase": "316", "baseline_obj": float(base_obj), "stored_baseline": STORED_OBJ,
    ...
    out = Path("artifacts/phase316_fts"); out.mkdir(parents=True, exist_ok=True)
    (out / "phase316_fts_report.json").write_text(json.dumps(result, indent=2))
    print(f"\n  Report → artifacts/phase316_fts/phase316_fts_report.json")
    ...
    if validated:
        ...
        msg = (f"feat: P316 FTS retune OBJ={reported_obj} "
