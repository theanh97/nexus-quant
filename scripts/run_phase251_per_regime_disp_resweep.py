"""
Phase 251 — Per-Regime DISP Overlay Re-Sweep (Vectorized)
==========================================================
Baseline: v2.48.0, OBJ=4.2852

DISP overlay was set in Phase 232. Key discovery: MID SCALE=1.50 (amplify!).
After FTS, regime weight, VOL changes, DISP params may have shifted.

Current DISP params:
  LOW:  THR=0.70, SCALE=0.50
  MID:  THR=0.70, SCALE=1.50  ← amplify when high dispersion
  HIGH: THR=0.40, SCALE=0.50
"""
...
DISP_THR_BASE   = {"LOW": 0.70, "MID": 0.70, "HIGH": 0.40}
DISP_SCALE_BASE = {"LOW": 0.50, "MID": 1.50, "HIGH": 0.50}

BASELINE_OBJ = 4.2852
MIN_DELTA = 0.005; MIN_LOYO = 3

DISP_THR_SWEEP   = [0.25, 0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90]
DISP_SCALE_SWEEP = [0.0, 0.10, 0.20, 0.30, 0.50, 0.70, 1.00, 1.20, 1.50, 1.80, 2.00, 2.50, 3.00]
...
def precompute_disp_fast_data(all_base_data, all_signals):
    """Precompute overlay_no_disp (VOL + FTS only) and fund_std_pct arrays."""
    fast_data = {}
    for yr, base_data in sorted(all_base_data.items()):
        ...
        # overlay_no_disp: VOL + FTS only
        overlay_no_disp = np.ones(min_len)
        for ridx, rname in enumerate(RNAMES):
            m = masks[ridx]
            # VOL
            vol_m = m & ~np.isnan(bv) & (bv > VOL_THR[rname])
            overlay_no_disp[vol_m] *= VOL_SCALE[rname]
            # FTS
            tsp = ts_pct_cache[TS_PCT_WIN[rname]][:min_len]
            overlay_no_disp[m & (tsp > FTS_RT[rname])] *= FTS_RS[rname]
            overlay_no_disp[m & (tsp < FTS_BT[rname])] *= FTS_BS[rname]

        # raw weighted signal per regime
        raw_weighted = {}
        for ridx, rname in enumerate(RNAMES):
        ...
        fast_data[yr] = (masks, overlay_no_disp, raw_weighted, fsp, min_len)
    return fast_data

def fast_evaluate_disp(disp_thr, disp_scale, fast_data):
    yearly = {}
    for yr, (masks, overlay_no_disp, raw_weighted, fsp, min_len) in fast_data.items():
        ens = np.zeros(min_len)
        for ridx, rname in enumerate(RNAMES):
            m = masks[ridx]
            thr = disp_thr[rname]; sc = disp_scale[rname]
            disp_mult_m = np.where(fsp[m] > thr, sc, 1.0)
            ens[m] += raw_weighted[rname][m] * overlay_no_disp[m] * disp_mult_m
        r = ens[~np.isnan(ens)]
...
def main():
    t0 = time.time()
    print("=" * 65)
    print("Phase 251 — Per-Regime DISP Overlay Re-Sweep (Vectorized)")
    print(f"Baseline: v2.48.0  OBJ={BASELINE_OBJ}")
    print(f"Grid: THR={len(DISP_THR_SWEEP)} × SCALE={len(DISP_SCALE_SWEEP)} = "
          f"{len(DISP_THR_SWEEP)*len(DISP_SCALE_SWEEP)} combos per regime")
    print("=" * 65)
    ...
    print("\n[3] Precomputing DISP fast eval data...", flush=True)
    fast_data = precompute_disp_fast_data(all_base_data, all_signals)

    base_obj_val, base_yearly = fast_evaluate_disp(DISP_THR_BASE, DISP_SCALE_BASE, fast_data)
    print(f"\n  Baseline OBJ = {base_obj_val:.4f}  (expected ≈ {BASELINE_OBJ})", flush=True)
    _partial.update({"baseline_obj": float(base_obj_val)})

    print("\n[4] Sequential per-regime DISP sweep (2 passes)...", flush=True)
    cur_thr   = dict(DISP_THR_BASE)
    cur_scale = dict(DISP_SCALE_BASE)
        ...
        for rname in RNAMES:
            ...
            for thr in DISP_THR_SWEEP:
                for sc in DISP_SCALE_SWEEP:
                    ...
                    o, _ = fast_evaluate_disp(t_test, s_test, fast_data)
                    if o > best_obj_this:
                        best_obj_this = o
    ...
    final_obj_val, final_yearly = fast_evaluate_disp(cur_thr, cur_scale, fast_data)
    delta = final_obj_val - base_obj_val
    print(f"\n  → Best per-regime DISP params:")
    for r in RNAMES:
    ...
    result = {
        ...
        "validated": validated,
        "best_disp": {r: {"thr": cur_thr[r], "scale": cur_scale[r]} for r in RNAMES},
    }
    ...
    if validated:
        ...
        if "per_regime_threshold" not in disp: disp["per_regime_threshold"] = {}
        if "per_regime_scale"     not in disp: disp["per_regime_scale"]     = {}
        for r in RNAMES:
            disp["per_regime_threshold"][r] = cur_thr[r]
            disp["per_regime_scale"][r]     = cur_scale[r]
        ...
        print(f"\n  Config → v2.49.0  OBJ={round(final_obj_val,4)}", flush=True)
