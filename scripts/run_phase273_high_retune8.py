#!/usr/bin/env python3
"""
Phase 273 — HIGH Regime Retune Round 8
After MID retune (v2.49.1: MID f168=0.80), re-optimize HIGH regime weights.
Supervisor set HIGH: v1=0.20, i460=0.1575, i415=0.2925, f168=0.35
Re-validate and optimize from current config.
...
def main():
    ...
    best_obj  = curr_obj
    v1_cur    = ch["v1"]
    iratio_cur = (ch["i460"] / (ch["i460"] + ch["i415"])) if (ch["i460"] + ch["i415"]) > 0 else 0.4

    for f168_w in [0.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40,
                   0.45, 0.50, 0.55, 0.60, 0.65, 0.70]:
        w = make_high_w(base_weights, f168_w, v1_cur, iratio_cur)
    ...
    for v1_w in [0.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50]:
        w = make_high_w(base_weights, best_f168, v1_w, iratio_cur)
        o = eval_weights(yearly_data, params, w)
        marker = " <--" if o > best_obj else ""
    ...
    # Fine-tune
    print("\n[6] Fine-tune HIGH ...")
    for f168_w in sorted(set([round(best_f168 + d, 2) for d in [-0.05, -0.03, 0.0, 0.03, 0.05]])):
        if f168_w < 0.0 or f168_w > 0.8: continue
        for v1_w in sorted(set([round(best_v1 + d, 2) for d in [-0.05, 0.0, 0.05]])):
            if v1_w < 0.0 or v1_w + f168_w > 1.0: continue
            w = make_high_w(base_weights, f168_w, v1_w, best_ratio)
            o = eval_weights(yearly_data, params, w)
            marker = " <--" if o > best_obj else ""
            print(f"  f168={f168_w:.2f} v1={v1_w:.2f}  OBJ={o:.4f}{marker}", flush=True)
            if o > best_obj:
                best_obj = o; best_f168 = f168_w; best_v1 = v1_w
    ...
    if wins >= 3 and delta > 0.005:
        ...
        rw2["HIGH"]["v1"] = bh["v1"]
        rw2["HIGH"]["i460bw168"] = bh["i460"] if "i460bw168" in rw2["HIGH"] else None
        if "i460bw168" in rw2["HIGH"]:
            rw2["HIGH"]["i460bw168"] = bh["i460"]
        else:
            rw2["HIGH"]["i460"] = bh["i460"]
        if "i415bw216" in rw2["HIGH"]:
            rw2["HIGH"]["i415bw216"] = bh["i415"]
        else:
            rw2["HIGH"]["i415"] = bh["i415"]
